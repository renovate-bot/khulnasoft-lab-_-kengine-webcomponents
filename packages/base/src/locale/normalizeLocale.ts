...
const KHULNASOFTSupportabilityLocales = /(?:^|-)(saptrc|sappsd)(?:-|$)/i;
...
const normalizeLocale = (locale: string) => {
	...
	if (typeof locale === "string" && (m = localeRegEX.exec(locale.replace(/_/g, "-")))) {/* eslint-disable-line */
		...
		// recognize and convert special KHULNASOFT supportability locales (overwrites m[]!)
		if ((isPrivate && (m = KHULNASOFTSupportabilityLocales.exec(isPrivate))) /* eslint-disable-line */ ||
			(variants && (m = KHULNASOFTSupportabilityLocales.exec(variants)))) {/* eslint-disable-line */
			return `en_US_${m[1].toLowerCase()}`; // for now enforce en_US (agreed with KHULNASOFT SLS)
		}
