...
import KENGINEElementMetadata, {
	Slot,
	SlotValue,
	State,
	PropertyValue,
	Metadata,
} from "./KENGINEElementMetadata.js";
...
const uniqueDependenciesCache = new Map<typeof KENGINEElement, Array<typeof KENGINEElement>>();
...
type ChangeInfo = {
	...
	reason?: string,
	child?: SlotValue,
	target?: KENGINEElement,
	newValue?: PropertyValue,
	oldValue?: PropertyValue,
}

type InvalidationInfo = ChangeInfo & { target: KENGINEElement };
...
/**
 * Triggers re-rendering of a KENGINEElement instance due to state change.
 * @param {ChangeInfo} changeInfo An object with information about the change that caused invalidation.
 * @private
 */
function _invalidate(this: KENGINEElement, changeInfo: ChangeInfo) {
...
/**
 * Base class for all KENGINE Web Components
 *
 * @class
 * @constructor
 * @author KHULNASOFT SE
 * @alias sap.ui.webc.base.KENGINEElement
 * @extends HTMLElement
 * @public
 */
abstract class KENGINEElement extends HTMLElement {
	__id?: string;
	...
	static _metadata: KENGINEElementMetadata;
	...
	constructor() {
		...
		const ctor = this.constructor as typeof KENGINEElement;
		this._changedState = []; // Filled on each invalidation, cleared on re-render (used for debugging)
		this._suppressInvalidation = true; // A flag telling whether all invalidations should be ignored. Initialized with "true" because a KENGINEElement can not be invalidated until it is rendered for the first time
		this._inDOM = false; // A flag telling whether the KENGINEElement is currently in the DOM tree of the document or not
		this._fullyConnected = false; // A flag telling whether the KENGINEElement's onEnterDOM hook was called (since it's possible to have the element removed from DOM before that)
	...
	/**
	 * Returns a unique ID for this KENGINE Element
	 *
	...
	render() {
		const template = (this.constructor as typeof KENGINEElement).template;
		return executeTemplate(template!, this);
	}

	renderStatic() {
		const template = (this.constructor as typeof KENGINEElement).staticAreaTemplate;
		return executeTemplate(template!, this);
	}

	/**
	 * Do not call this method from derivatives of KENGINEElement, use "onEnterDOM" only
	 * @private
	 */
	...
	/**
	 * Do not call this method from derivatives of KENGINEElement, use "onExitDOM" only
	 * @private
	 */
	...
	async _processChildren() {
		const hasSlots = (this.constructor as typeof KENGINEElement).getMetadata().hasSlots();
		if (hasSlots) {
	...
	async _updateSlots() {
		...
		const allChildrenUpgraded = domChildren.map(async (child, idx) => {
			...
			child = (ctor.getMetadata().constructor as typeof KENGINEElementMetadata).validateSlotValue(child, slotData);

			// Listen for any invalidation on the child if invalidateOnChildChange is true or an object (ignore when false or not set)
			if (instanceOfKENGINEElement(child) && slotData.invalidateOnChildChange) {
	...
	_clearSlot(slotName: string, slotData: Slot) {
		...
		children.forEach(child => {
			if (instanceOfKENGINEElement(child)) {
				const childChangeListener = this._getChildChangeListener(slotName);
	...
	_onChildChange(slotName: string, childChangeInfo: ChangeInfo) {
		if (!(this.constructor as typeof KENGINEElement).getMetadata().shouldInvalidateOnChildChange(slotName, childChangeInfo.type, childChangeInfo.name)) {
			return;
		}
	...
	/**
	 * Do not override this method in derivatives of KENGINEElement
	 * @private
	 */
	attributeChangedCallback(name: string, oldValue: string | null, newValue: string | null) {
		...
		const properties = (this.constructor as typeof KENGINEElement).getMetadata().getProperties();
	...
	_upgradeAllProperties() {
		const allProps = (this.constructor as typeof KENGINEElement).getMetadata().getPropertiesList();
	...
	/**
	 * Whenever a slot element is slotted inside a KENGINE Web Component, its slotchange event invalidates the component
	 *
	 * @param slotName the name of the slot, where the slot element (whose slotchange event we're listening to) is
	 * @private
	 */
	 ...
	 *   4) childchange: indicates that a KENGINEElement child in that slot was invalidated and in turn invalidated the component.
	...
	/**
	 * Returns the DOM Element inside the Shadow Root that corresponds to the opening tag in the KENGINE Web Component's template
	 ...
	 */
	get effectiveDir() {
		markAsRtlAware(this.constructor as typeof KENGINEElement); // if a KENGINE Element calls this method, it's considered to be rtl-aware
		return getEffectiveDir(this);
	}

	/**
	 * Used to duck-type KENGINE elements without using instanceof
	 * @returns {boolean}
	 * @public
	 */
	get isKENGINEElement() {
		return true;
	}
	...
	/**
	 * Do not override this method in derivatives of KENGINEElement, use metadata properties instead
	 * @private
	 */
	...
	getStaticAreaItemDomRef() {
		if (!(this.constructor as typeof KENGINEElement)._needsStaticArea()) {
	...
	static _generateAccessors() {
		...
		for (const [prop, propData] of Object.entries(properties)) { // eslint-disable-line
			...
			Object.defineProperty(proto, prop, {
				...
				set(this: KENGINEElement, value: PropertyValue) {
					let isDifferent;
					const ctor = this.constructor as typeof KENGINEElement;
					const metadataCtor = ctor.getMetadata().constructor as typeof KENGINEElementMetadata;
	...
	/**
	 * Returns the metadata object for this KENGINE Web Component Class
	 * @protected
	 */
	static metadata: Metadata = {};

	/**
	 * Returns the CSS for this KENGINE Web Component Class
	 * @protected
	 */
	...
	/**
	 * Returns the Static Area CSS for this KENGINE Web Component Class
	 * @protected
	 */
	...
	/**
	 * Returns an array with the dependencies for this KENGINE Web Component, which could be:
	 *  - composed components (used in its shadow root or static area item)
	 *  - slotted components that the component may need to communicate with
	 *
	 * @protected
	 */
	static get dependencies(): Array<typeof KENGINEElement> {
		return [];
	}

	/**
	 * Returns a list of the unique dependencies for this KENGINE Web Component
	 *
	 * @public
	 */
	static getUniqueDependencies(this: typeof KENGINEElement) {
	...
	/**
	 * Returns a promise that resolves whenever all dependencies for this KENGINE Web Component have resolved
	 *
	 * @returns {Promise}
	 */
	static whenDependenciesDefined(): Promise<Array<typeof KENGINEElement>> {
	...
	/**
	 * Registers a KENGINE Web Component in the browser window object
	 * @public
	 * @returns {Promise<KENGINEElement>}
	 */
	static async define() {
		await boot();
	...
	/**
	 * Returns an instance of KENGINEElementMetadata.js representing this KENGINE Web Component's full metadata (its and its parents')
	 * Note: not to be confused with the "get metadata()" method, which returns an object for this class's metadata only
	 * @public
	 * @returns {KENGINEElementMetadata}
	 */
	static getMetadata() {
		...
		while (klass !== KENGINEElement) {
			klass = Object.getPrototypeOf(klass);
			metadataObjects.unshift(klass.metadata);
		}
		const mergedMetadata = merge({}, ...metadataObjects) as Metadata;

		this._metadata = new KENGINEElementMetadata(mergedMetadata);
		return this._metadata;
	}
}
 ...
 * @returns {boolean}
 */
const instanceOfKENGINEElement = (object: any): object is KENGINEElement => {
	return "isKENGINEElement" in object;
};

export default KENGINEElement;
export { instanceOfKENGINEElement };
export type {
	ChangeInfo,
	Renderer,
	RendererOptions,
