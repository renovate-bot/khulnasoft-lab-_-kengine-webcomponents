...
attachCustomCSSChange((tag: string) => {
	if (!skipRerender) {
		reRenderAllKENGINEElements({ tag });
	}
});
...
const addCustomCSS = (tag: string, css: string) => {
	...
	try {
		...
		// The skipRerender flag will be used to avoid calling reRenderAllKENGINEElements twice when it is this copy
	...
	return reRenderAllKENGINEElements({ tag });
};
