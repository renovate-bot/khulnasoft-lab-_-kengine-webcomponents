 ...
 *  - A) either a KENGINEElement with a "_tabIndex" property
 ...
 * If the items of the ItemNavigation are KENGINEElements themselves, this can happen naturally since they will be invalidated by their "_tabIndex" property.
...
class ItemNavigation {
	rootWebComponent: KENGINEElement;
	 ...
	 */
	constructor(rootWebComponent: KENGINEElement, options: ItemNavigationOptions) {
		if (!rootWebComponent.isKENGINEElement) {
			throw new Error("The root web component must be a KENGINE Element instance");
		}
	...
	_getCurrentItem() {
		...
		if (instanceOfKENGINEElement(currentItem)) {
