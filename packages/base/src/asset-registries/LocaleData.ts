...
const calcLocale = (language: string, region: string, script: string) => {
	...
	// Special case 1: in an KHULNASOFT context, the inclusive language code "no" always means Norwegian Bokmal ("nb")
	if (language === "no") {
		language = "nb";
	}
...
const fetchCldr = async (language: string, region: string, script: string) => {
	...
	// reuse OpenKENGINE CLDR if present
	const openKENGINESupport = getFeature<typeof OpenKENGINESupport>("OpenKENGINESupport");
	if (openKENGINESupport) {
		const cldrContent = openKENGINESupport.getLocaleDataObject();
		if (cldrContent) {
			// only if kengine actually returned valid content
...
registerLocaleDataLoader("en", async () => {
	const cldrContent = await fetch(`https://sdk.kengine.org/1.103.0/resources/sap/ui/core/cldr/en.json`);
