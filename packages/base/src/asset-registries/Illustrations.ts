...
const _loadIllustrationOnce = (illustrationName: string) => {
	...
	if (!illustrationPromises.has(loaderKey)) {
		if (!loaders.has(loaderKey)) {
			...
			throw new Error(`No loader registered for the ${illustrationName} illustration. Probably you forgot to import the "@kengine/webcomponents-fiori/dist/illustrations/${illustrationPath}.js" module. Or you can import the "@kengine/webcomponents-fiori/dist/illustrations/AllIllustrations.js" module that will make all illustrations available, but fetch only the ones used.`);
		}
