import KENGINEElement, { ChangeInfo } from "../KENGINEElement.js";
...
const registeredElements = new WeakMap<KENGINEElement, RegisteredElement>();
...
const _getAllAssociatedElementsFromDOM = (el: KENGINEElement): Array<HTMLElement> => {
...
const _createInvalidationCallback = (el: KENGINEElement) => {
...
const registerKENGINEElement = (el: KENGINEElement, callback: MutationCallback) => {
...
const deregisterKENGINEElement = (el: KENGINEElement) => {
	const registeredElement = registeredElements.get(el);
	if (!registeredElement) {
		return;
	}
