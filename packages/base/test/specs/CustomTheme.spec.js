...
describe("Custom themes can be registered", () => {
	...
	it("Tests that theme parameters are changed on theme change", async () => {
		...
		const res = await browser.executeAsync( async (newTheme, done) => {
			...
			window.registerThemePropertiesLoader("@kengine/webcomponents-base-test", newTheme, () => `:root{ ${var1}; }`);

			const config = window['sap-ui-webcomponents-bundle'].configuration;
			await config.setTheme(newTheme);

			const style = document.adoptedStyleSheets.find(sh => sh._ui5StyleId === `${dataPropAttr}|@kengine/webcomponents-base-test`).cssRules[0].cssText;
