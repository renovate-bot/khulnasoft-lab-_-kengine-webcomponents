...
describe("Some settings can be set via KHULNASOFT UI URL params", () => {
	beforeEach(async () => {
	...
	it("Tests that theme root is applied", async () => {
		...
		assert.strictEqual(res, 'https://example.com/KENGINE/', "Theme root is https://example.com/KENGINE");
		...
		assert.strictEqual(res, `${location.origin}/KENGINE/`, `Theme root is ${location.origin}/KENGINE/`);
		...
		assert.ok(res.endsWith("/test/KENGINE/"), `Theme root is set correctly with relative url`);
	});
...
describe("Some settings can be set via KHULNASOFT URL params", () => {
	before(async () => {
...
describe("KHULNASOFT UI params take precedence over the KHULNASOFT params", () => {
	before(async () => {
