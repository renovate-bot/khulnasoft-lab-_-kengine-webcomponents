...
import assets from "@kengine/webcomponents-tools/assets-meta.js";

const allLocales = assets.locales.all;

const imports = allLocales.map(locale => `import ${locale} from "../assets/cldr/${locale}.json";`).join("\n");
const caseImports = allLocales.map(locale => `\t\tcase "${locale}": return (await import(/* webpackChunkName: "kengine-webcomponents-cldr-${locale}" */ "../assets/cldr/${locale}.json")).default;`).join("\n");
const localesKeys = allLocales.join(",");
const localesKeysStrArray = allLocales.map(_ => `"${_}"`).join(",");

const contentStatic = `import { registerLocaleDataLoader } from "@kengine/webcomponents-base/dist/asset-registries/LocaleData.js";
...
const contentDynamic = `import { registerLocaleDataLoader } from "@kengine/webcomponents-base/dist/asset-registries/LocaleData.js";
