...
class CalendarDate {
	_oUDate!: KENGINEDate | Date | UniversalDate;
	...
	constructor(year?: number | CalendarDate, month?: number | string, date?: number, calendarType?: string) { // eslint-disable-line
		let aArgs = arguments, // eslint-disable-line
			oJSDate: KENGINEDate | Date,
			oNow: KENGINEDate | Date,
			sCalendarType!: CalendarType;

		switch (aArgs.length) {
		case 0: // defaults to the current date
			oNow = KENGINEDate.getInstance();
		...
		case 2: // CalendarDate, sCalendarType
			...
			oJSDate = KENGINEDate.getInstance(aArgs[0].valueOf());
		...
		case 4: // year, month, date, sCalendarType
			...
			oJSDate = KENGINEDate.getInstance(0, 0, 1);
	...
	toLocalJSDate() {
		...
		const oLocalDate = KENGINEDate.getInstance(this._oUDate.getTime());
	...
	toUTCJSDate() {
		// Use this._oUDate.getTime()(returns the same point of time regardless calendar type)  instead of
		// this._oUDate's getters to avoid non-gregorian Year, Month and Date to be used to construct a Gregorian date
		const oUTCDate = KENGINEDate.getInstance(this._oUDate.getTime());
	...
	static fromLocalJSDate(oJSDate: Date | KENGINEDate, sCalendarType?: `${CalendarType}`) {
		// Cross frame check for a date should be performed here otherwise setDateValue would fail in OPA tests
		// because Date object in the test is different than the Date object in the application (due to the iframe).
		// We can use jQuery.type or this method:
		function isValidDate(date: Date | KENGINEDate) {
	...
	static fromTimestamp(iTimestamp: number, sCalendarType?: `${CalendarType}`) {
		...
		let oUDate;
		try {
			oUDate = UniversalDate.getInstance(KENGINEDate.getInstance(iTimestamp), sCalendarType);
		} catch (e) {
...
function createUniversalUTCDate(oDate: KENGINEDate | Date, sCalendarType: `${CalendarType}`) {
	if (sCalendarType) {
 ...
 */
function createUTCDate(oDate: KENGINEDate | Date) {
	const oUTCDate = new Date(Date.UTC(0, 0, 1)); // no need to replace with KENGINEDate as we are creating a new UTC date object
