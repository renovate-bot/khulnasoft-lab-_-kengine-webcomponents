 ...
 * @tagname kengine-monthpicker
 * @public
 */
@customElement({
	tag: "kengine-monthpicker",
...
class MonthPicker extends CalendarPart implements ICalendarPicker {
	...
	static async onDefine() {
		MonthPicker.i18nBundle = await getI18nBundle("@kengine/webcomponents");
	}
	...
	_buildMonths() {
		...
		for (let i = 0; i < 12; i++) {
			...
			const month: Month = {
				...
				disabled: isDisabled,
				classes: "kengine-mp-item",
			};

			if (isSelected) {
				month.classes += " kengine-mp-item--selected";
			}

			if (isDisabled) {
				month.classes += " kengine-mp-item--disabled";
			}
	...
	_selectMonth(e: Event) {
		...
		if (target.className.indexOf("kengine-mp-item") > -1) {
