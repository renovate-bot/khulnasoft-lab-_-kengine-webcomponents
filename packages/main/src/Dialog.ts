...
import clamp from "@kengine/webcomponents-base/dist/util/clamp.js";
import {
...
import "@kengine/webcomponents-icons/dist/resize-corner.js";
 ...
 * The <code>kengine-dialog</code> component is used to temporarily display some information in a
 * size-limited window in front of the regular app screen.
 * It is used to prompt the user for an action or a confirmation.
 * The <code>kengine-dialog</code> interrupts the current app processing as it is the only focused UI element and
 * the main screen is dimmed/blocked.
 * The dialog combines concepts known from other technologies where the windows have
 * names such as dialog box, dialog window, pop-up, pop-up window, alert box, or message box.
 * <br><br>
 * The <code>kengine-dialog</code> is modal, which means that an user action is required before it is possible to return to the parent window.
 * To open multiple dialogs, each dialog element should be separate in the markup. This will ensure the correct modal behavior. Avoid nesting dialogs within each other.
 * The content of the <code>kengine-dialog</code> is fully customizable.
 *
 * <h3>Structure</h3>
 * A <code>kengine-dialog</code> consists of a header, content, and a footer for action buttons.
 * The <code>kengine-dialog</code> is usually displayed at the center of the screen.
 ...
 * <code>kengine-dialog</code> on full screen.
 *
 ...
 * <br>
 * The <code>kengine-dialog</code> exposes the following CSS Shadow Parts:
 * <ul>
 * <li>header - Used to style the header of the component</li>
 * <li>content - Used to style the content of the component</li>
 * <li>footer - Used to style the footer of the component</li>
 * </ul>
 * <b>Note:</b> When a <code>kengine-bar</code> is used in the header or in the footer, you should remove the default dialog's paddings.
 * <br>
 ...
 *
 * <code>import "@kengine/webcomponents/dist/Dialog";</code>
 *
 * <b>Note:</b> We don't recommend nesting popup-like components (<code>kengine-dialog</code>, <code>kengine-popover</code>) inside <code>kengine-dialog</code>.
 ...
 *
 * <b>Note:</b> We don't recommend nesting popup-like components (<code>kengine-dialog</code>, <code>kengine-popover</code>) inside other components containing z-index.
...
class Dialog extends Popup {
	 ...
	 * <br><br>
	 * <b>Note:</b> When a <code>kengine-bar</code> is used in the header, you should remove the default dialog's paddings.
	 * <br><br>
	 ...
	 * <br><br>
	 * <b>Note:</b> When a <code>kengine-bar</code> is used in the footer, you should remove the default dialog's paddings.
	 *
	 * @type {HTMLElement[]}
	...
	static async onDefine() {
		Dialog.i18nBundle = await getI18nBundle("@kengine/webcomponents");
	}

	static _isHeader(element: HTMLElement) {
		return element.classList.contains("kengine-popup-header-root") || element.getAttribute("slot") === "header";
	}
	...
	get _ariaLabelledBy() {
		...
		if (this.headerText !== "" && !this._ariaLabel) {
			ariaLabelledById = "kengine-popup-header-text";
		}
	...
	get _minHeight() {
		...
		const header = this._root.querySelector<HTMLElement>(".kengine-popup-header-root");
		if (header) {
			minHeight += header.offsetHeight;
		}

		const footer = this._root.querySelector<HTMLElement>(".kengine-popup-footer-root");
		if (footer) {
	...
	_resizeWithEvent(e: KeyboardEvent) {
		this._draggedOrResized = true;
		this.addEventListener("kengine-before-close", this._revertSize, { once: true });
