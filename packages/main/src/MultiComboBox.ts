...
import type { ClassMap, Timeout } from "@kengine/webcomponents-base/dist/types.js";
...
import "@kengine/webcomponents-icons/dist/multiselect-all.js";
...
interface IMultiComboBoxItem extends KENGINEElement {
	text: string,
	selected: boolean,
	isGroupItem?: boolean,
	stableDomRef: string,
 ...
 *
 * The <code>kengine-multi-combobox</code> component consists of a list box with items and a text field allowing the user to either type a value directly into the text field, or choose from the list of existing items.
 *
 * The drop-down list is used for selecting and filtering values, it enables users to select one or more options from a predefined list. The control provides an editable input field to filter the list, and a dropdown arrow to expand/collapse the list of available options.
 * The options in the list have checkboxes that permit multi-selection. Entered values are displayed as tokens.
 * <h3>Structure</h3>
 * The <code>kengine-multi-combobox</code> consists of the following elements:
 * <ul>
 ...
 *
 * The <code>kengine-multi-combobox</code> provides advanced keyboard handling.
 *
 * <h4>Picker</h4>
 * If the <code>kengine-multi-combobox</code> is focused,
 ...
 * <br>
 * The <code>kengine-multi-combobox</code> exposes the following CSS Shadow Parts:
 * <ul>
 ...
 *
 * <code>import "@kengine/webcomponents/dist/MultiComboBox";</code>
 *
 *
 * @constructor
...
class MultiComboBox extends KENGINEElement {
	/**
	...
	*/
	...
	get _inputDom(): HTMLInputElement {
		return this.shadowRoot!.querySelector("#kengine-multi-combobox-input")!;
	}
	...
	_tokenDelete(e: CustomEvent<TokenizerTokenDeleteEventDetail>) {
		...
		const deletingItem = this.items.find(item => item._id === token.getAttribute("data-kengine-id"))!;
	...
	_tokenizerFocusOut(e: FocusEvent) {
		...
		if (!relatedTarget?.hasAttribute("kengine-token") && !isFocusingPopover && !isFocusingInput && !isFocusingMorePopover) {
	...
	syncItems(listItems: Array<ListItemBase>) {
		listItems.forEach(item => {
			this.items.forEach(mcbItem => {
				if (mcbItem._id === item.getAttribute("data-kengine-token-id")) {
	...
	async _getRespPopover() {
		...
		this.allItemsPopover = staticAreaItem!.querySelector(`.kengine-multi-combobox-all-items-responsive-popover`)!;
	}

	async _getList(): Promise<List> {
		const staticAreaItem = await this.getStaticAreaItemDomRef();
		this.list = staticAreaItem!.querySelector(".kengine-multi-combobox-all-items-list")!;
		return this.list;
	}
	...
	onBeforeRendering() {
		...
		this.items.forEach(item => {
			item._getRealDomRef = () => this.allItemsPopover!.querySelector(`*[data-kengine-stable=${item.stableDomRef}]`)!;
		});
	...
	async _setValueStateHeader() {
		...
		this.valueStateHeader = responsivePopover.querySelector("div.kengine-responsive-popover-header.kengine-valuestatemessage-root")!;
	}

	get _tokenizer() {
		return this.shadowRoot!.querySelector<Tokenizer>("[kengine-tokenizer]")!;
	}
	...
	inputFocusIn(e: FocusEvent) {
		...
		if (!isPhone() && (((e.relatedTarget as HTMLElement)?.tagName !== "KENGINE-STATIC-AREA-ITEM") || !e.relatedTarget)) {
	...
	get valueStateTextId() {
		return this.hasValueState ? `kengine-multi-combobox-valueStateDesc` : undefined;
	}
	...
	get _tokensCountTextId() {
		return "kengine-multi-combobox-hiddenText-nMore";
	}
	...
	get classes(): ClassMap {
		return {
			popover: {
				"kengine-multi-combobox-all-items-responsive-popover": true,
				"kengine-suggestions-popover": true,
				"kengine-suggestions-popover-with-value-state-header": this.hasValueStateMessage,
			},
			popoverValueState: {
	...
	static async onDefine() {
		MultiComboBox.i18nBundle = await getI18nBundle("@kengine/webcomponents");
	}
}
