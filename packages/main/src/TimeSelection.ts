 ...
 * @tagname kengine-time-selection
 * @private
 * @since 1.0.0-rc.12
 */
@customElement({
	tag: "kengine-time-selection",
	languageAware: true,
	renderer: litRender,
...
class TimeSelection extends KENGINEElement {
	/**
	...
	selectSlider(e: MouseEvent | FocusEvent) {
		...
		this._setCurrentSlider(target.closest<WheelSlider>("[kengine-wheelslider]")!.getAttribute("data-sap-slider")!);
	}
	...
	_onkeydown(e: KeyboardEvent) {
		...
		const target = e.target as HTMLElement;
		const activeSlider = target.closest<WheelSlider>("[kengine-wheelslider]")!.getAttribute("data-sap-slider")!;
	...
	get classes() {
		return {
			root: {
				"kengine-time-selection-root": true,
