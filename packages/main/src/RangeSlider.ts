...
import Float from "@kengine/webcomponents-base/dist/types/Float.js";
 ...
 * <br>
 * The <code>kengine-range-slider</code> exposes the following CSS Shadow Parts:
 * <ul>
 * <li>progress-container - Used to style the progress container(the horizontal bar which visually represents the range between the minimum and maximum value) of the <code>kengine-range-slider</code>.</li>
 * <li>progress-bar - Used to style the progress bar, which shows the progress of the <code>kengine-range-slider</code>.</li>
 * <li>handle - Used to style the handles of the <code>kengine-range-slider</code>.</li>
 * </ul>
 *
 ...
 *
 * <code>import "@kengine/webcomponents/dist/RangeSlider";</code>
 *
 *
 * @constructor
 ...
 * @tagname kengine-range-slider
 * @since 1.0.0-rc.11
 * @public
 */
@customElement({
	tag: "kengine-range-slider",
	languageAware: true,
...
class RangeSlider extends SliderBase {
	...
	*/
	...
	_saveInteractionStartData(e: TouchEvent | MouseEvent, newValue: number) {
		const progressBarDom = this.shadowRoot!.querySelector(".kengine-slider-progress")!.getBoundingClientRect();
	...
	_pressTargetAndAffectedValue(clientX: number, value: number) {
		const startHandle = this.shadowRoot!.querySelector(".kengine-slider-handle--start")!;
		const endHandle = this.shadowRoot!.querySelector(".kengine-slider-handle--end")!;
	...
	get _startHandle() {
		return this.shadowRoot!.querySelector<HTMLElement>(".kengine-slider-handle--start")!;
	}

	get _endHandle() {
		return this.shadowRoot!.querySelector<HTMLElement>(".kengine-slider-handle--end")!;
	}

	get _progressBar() {
		return this.shadowRoot!.querySelector<HTMLElement>(".kengine-slider-progress")!;
	}
	...
	static async onDefine() {
		RangeSlider.i18nBundle = await getI18nBundle("@kengine/webcomponents");
	}
}
