...
const OVERFLOW_BTN_CLASS = "kengine-avatar-group-overflow-btn";
 ...
 * <li>[SPACE],[ENTER],[RETURN] - Trigger <code>kengine-click</code> event</li>
 * </ul>
 * <br>
 ...
 * @tagname kengine-avatar-group
 * @since 1.0.0-rc.11
 * @public
 */

@customElement({
	tag: "kengine-avatar-group",
	renderer: litRender,
...
/**
* Fired when the count of visible <code>kengine-avatar</code> elements in the
...
class AvatarGroup extends KENGINEElement {
	/**
	...
	/**
	 * Defines the items of the component. Use the <code>kengine-avatar</code> component as an item.
	 * <br><br>
	 ...
	 * <b>Note:</b> We recommend using the <code>kengine-button</code> component.
	 * <br><br>
	...
	static async onDefine() {
		AvatarGroup.i18nBundle = await getI18nBundle("@kengine/webcomponents");
	}

	/**
	 * Returns an array containing the <code>kengine-avatar</code> instances that are currently not displayed due to lack of space.
	 * @readonly
	 * @type {HTMLElement[]}
	 * @defaultValue []
	...
	get classes() {
		return {
			overflowButton: {
				"kengine-avatar-group-overflow-btn": true,
				"kengine-avatar-group-overflow-btn-xs": this.firstAvatarSize === "XS",
				"kengine-avatar-group-overflow-btn-s": this.firstAvatarSize === "S",
				"kengine-avatar-group-overflow-btn-m": this.firstAvatarSize === "M",
				"kengine-avatar-group-overflow-btn-l": this.firstAvatarSize === "L",
				"kengine-avatar-group-overflow-btn-xl": this.firstAvatarSize === "XL",
			},
		};
	}
	...
	_onkeydown(e: KeyboardEvent) {
		// when type is "Individual" the kengine-avatar and kengine-button both
		// fire "click" event when SPACE or ENTER are pressed and
		// AvatarGroup "click" is fired in their handlers (_onClick, _onKENGINEClick).
		if (this._isGroup) {
			if (isEnter(e)) {
	...
	_onClick(e: MouseEvent) {
		...
		// no matter the value of noConflict, the kengine-button and the group container (div) always fire a native click event
		const isButton = target.hasAttribute("kengine-button");
		e.stopPropagation();
	...
	_onKENGINEClick(e: MouseEvent) {
		const target = e.target as HTMLElement;
		// when noConflict=true only kengine-avatar will fire kengine-click event
		const isAvatar = target.hasAttribute("kengine-avatar");
		e.stopPropagation();
	...
	_prepareAvatars() {
		...
		this.items.forEach((avatar, index) => {
			...
			if (!avatar.getAttribute("_color-scheme")) {
				// AvatarGroup respects colors set to kengine-avatar
