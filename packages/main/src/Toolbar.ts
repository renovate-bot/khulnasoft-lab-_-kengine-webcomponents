import KENGINEElement, { ChangeInfo } from "@kengine/webcomponents-base/dist/KENGINEElement.js";
...
import "@kengine/webcomponents-icons/dist/overflow.js";
 ...
 *
 * The <code>kengine-toolbar</code> component is used to create a horizontal layout with items.
 * The items can be overflowing in a popover, when the space is not enough to show all of them.
 *
 * <h3>Keyboard Handling</h3>
 * The <code>kengine-toolbar</code> provides advanced keyboard handling.
 * <br>
 * <ul>
...
class Toolbar extends KENGINEElement {
     ...
     *
     * <b>Note:</b> Currently only <code>kengine-toolbar-button</code>, <code>kengine-toolbar-select</code>, <code>kengine-toolbar-separator</code> and <code>kengine-toolbar-spacer</code> are allowed here.
	 *
	...
	static async onDefine() {
		Toolbar.i18nBundle = await getI18nBundle("@kengine/webcomponents");
	}
	...
	get padding(): number {
		...
		return calculateCSSREMValue(toolbarComputedStyle, getScopedVarName("--_kengine-toolbar-padding-left"))
			+ calculateCSSREMValue(toolbarComputedStyle, getScopedVarName("--_kengine-toolbar-padding-right"));
	}
	...
	get classes() {
		return {
			items: {
				"kengine-tb-items": true,
				"kengine-tb-items-full-width": this.hasFlexibleSpacers,
			},
			overflow: {
				"kengine-overflow-list--alignleft": this.hasItemWithText,
			},
			overflowButton: {
				"kengine-tb-item": true,
				"kengine-tb-overflow-btn": true,
				"kengine-tb-overflow-btn-hidden": this.hideOverflowButton,
			},
		};
	}
	...
	get overflowButtonDOM(): Button | null {
		return this.shadowRoot!.querySelector(".kengine-tb-overflow-btn");
	}

	get itemsDOM() {
		return this.shadowRoot!.querySelector(".kengine-tb-items");
	}
	...
	async getOverflowPopover(): Promise<Popover | null> {
		...
		return staticAreaItem!.querySelector<Popover>(".kengine-overflow-popover");
	}
	...
	onInteract(e: CustomEvent) {
		...
		const item = target.closest<ToolbarItem>(".kengine-tb-item") || target.closest<ToolbarItem>(".kengine-tb-popover-item");
		...
		const refItemId = target.getAttribute("data-kengine-external-action-item-id");
	...
	getRegisteredToolbarItemByID(id: string): HTMLElement | null {
		return this.itemsDOM!.querySelector(`[data-kengine-external-action-item-id="${id}"]`);
	}
}
