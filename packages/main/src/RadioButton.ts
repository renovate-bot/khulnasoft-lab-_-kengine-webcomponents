 ...
 *
 * The <code>kengine-radio-button</code> component enables users to select a single option from a set of options.
 * When a <code>kengine-radio-button</code> is selected by the user, the
 * <code>change</code> event is fired.
 * When a <code>kengine-radio-button</code> that is within a group is selected, the one
 ...
 * <br>
 * <b>Note:</b> If <code>kengine-radio-button</code> is not part of a group, it can be selected once, but can not be deselected back.
 *
 * <h3>Keyboard Handling</h3>
 *
 * Once the <code>kengine-radio-button</code> is on focus, it might be selected by pressing the Space and Enter keys.
 * <br>
 ...
 *
 * <code>import "@kengine/webcomponents/dist/RadioButton";</code>
 *
 * @constructor
 * @author KHULNASOFT SE
 * @alias sap.ui.webc.main.RadioButton
 * @extends sap.ui.webc.base.KENGINEElement
 * @tagname kengine-radio-button
 * @public
 */
@customElement({
	tag: "kengine-radio-button",
	languageAware: true,
	renderer: litRender,
...
class RadioButton extends KENGINEElement implements IFormElement {
	/**
	...
	static async onDefine() {
		RadioButton.i18nBundle = await getI18nBundle("@kengine/webcomponents");
	}
	...
	_enableFormSupport() {
		...
		if (formSupport) {
			...
			console.warn(`In order for the "value" property to have effect, you should also: import "@kengine/webcomponents/dist/features/InputElementsFormSupport.js";`); // eslint-disable-line
		}
	}
	...
	get classes() {
		return {
			inner: {
				"kengine-radio-inner--hoverable": !this.disabled && !this.readonly && isDesktop(),
			},
		};
	}
