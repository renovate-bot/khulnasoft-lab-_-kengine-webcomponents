...
import getEffectiveContentDensity from "@kengine/webcomponents-base/dist/util/getEffectiveContentDensity.js";
import "@kengine/webcomponents-icons/dist/navigation-up-arrow.js";
import "@kengine/webcomponents-icons/dist/navigation-down-arrow.js";
 ...
 *
 * <code>import "@kengine/webcomponents/dist/WheelSlider.js";</code>
 *
 * @constructor
 * @author KHULNASOFT SE
 * @alias sap.ui.webc.main.WheelSlider
 * @extends sap.ui.webc.base.KENGINEElement
 * @tagname kengine-wheelslider
 * @public
 * @since 1.0.0-rc.6
 */
@customElement({
	tag: "kengine-wheelslider",
	renderer: litRender,
 ...
 */
@event("collapse")
class WheelSlider extends KENGINEElement {
	/**
	...
	onAfterRendering() {
		...
		if (this.expanded) {
			const elements = this.shadowRoot!.querySelectorAll(".kengine-wheelslider-item");
	...
	get classes() {
		return {
			root: {
				"kengine-wheelslider-root": true,
				"kengine-phone": isPhone(),
			},
		};
	}
	...
	get _itemCellHeight() {
		...
		if (this.shadowRoot!.querySelectorAll(".kengine-wheelslider-item").length) {
			const itemComputedStyle = getComputedStyle(this.shadowRoot!.querySelector(".kengine-wheelslider-item")!);
			const itemHeightValue = itemComputedStyle.getPropertyValue(getScopedVarName("--_kengine_wheelslider_item_height"));
	...
	_onclick(e: MouseEvent) {
		...
		if (!target.classList.contains("kengine-wheelslider-item")) {
			return;
		}
