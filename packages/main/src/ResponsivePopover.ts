...
import { getNextZIndex } from "@kengine/webcomponents-base/dist/util/PopupUtils.js";
 ...
 * The <code>kengine-responsive-popover</code> acts as a Popover on desktop and tablet, while on phone it acts as a Dialog.
 ...
 * <br>
 * The <code>kengine-responsive-popover</code> exposes the following CSS Shadow Parts:
 * <ul>
 ...
 * @tagname kengine-responsive-popover
 * @since 1.0.0-rc.6
 * @public
 */
@customElement({
	tag: "kengine-responsive-popover",
...
class ResponsivePopover extends Popover {
	...
	get classes() {
		...
		allClasses.header = {
			...
			"kengine-responsive-popover-header-no-title": !this.headerText,
		};
	...
	get _dialog() {
		return this.shadowRoot!.querySelector<Dialog>("[kengine-dialog]")!;
	}
	...
	_propagateDialogEvent(e: CustomEvent) {
		const type = e.type.replace("kengine-", "");
	...
	static async onDefine() {
		ResponsivePopover.i18nBundle = await getI18nBundle("@kengine/webcomponents");
	}
}
