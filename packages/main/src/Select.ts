...
import connectToComponent from "@kengine/webcomponents-base/dist/connectToComponent.js";
import {
	isSpace,
	isUp,
	isDown,
...
import DOMReference from "@kengine/webcomponents-base/dist/types/DOMReference.js";
import announce from "@kengine/webcomponents-base/dist/util/InvisibleMessage.js";
...
import "@kengine/webcomponents-icons/dist/slim-arrow-down.js";
...
import type { Timeout } from "@kengine/webcomponents-base/dist/types.js";
import Integer from "@kengine/webcomponents-base/dist/types/Integer.js";
import InvisibleMessageMode from "@kengine/webcomponents-base/dist/types/InvisibleMessageMode.js";
...
interface IOption extends KENGINEElement {
	id: string,
	selected: boolean,
	_focused?: boolean,
	focused?: boolean,
 ...
 *
 * The <code>kengine-select</code> component is used to create a drop-down list.
 *
 * <h3>Usage</h3>
 *
 * There are two main usages of the <code>kengine-select></code>.
 *
 * 1. With Option (<code>kengine-option</code>) web component:
 * <br>
 ...
 * <br><br>
 *
 * 2. With SelectMenu (<code>kengine-select-menu</code>) and SelectMenuOption (<code>kengine-select-menu-option</code>) web components:
 * <br>
 ...
 * The <code>kengine-select</code> provides advanced keyboard handling.
 * <br>
 * <ul>
 ...
 * <code>import "@kengine/webcomponents/dist/Select";</code>
 * <br>
 * <code>import "@kengine/webcomponents/dist/Option";</code> (comes with <code>kengine-select</code>)
 * @constructor
 * @author KHULNASOFT SE
 * @alias sap.ui.webc.main.Select
 * @extends sap.ui.webc.base.KENGINEElement
 * @tagname kengine-select
 ...
 */
@event("close")
class Select extends KENGINEElement implements IFormElement {
	 ...
	 * <br><br>
	 * <b>Note:</b> Usage of <code>kengine-select-menu</code> is recommended.
	 *
	 ...
	 * The value of the component will be the value of the currently selected <code>kengine-option</code>.
	 *
	 * <br><br>
	 ...
	 * will be created inside the <code>kengine-select</code> so that it can be submitted as
	 ...
	 *
	 * <br><br>
	 * <b>Note:</b> Use the <code>kengine-option</code> component to define the desired options.
	...
	*/
	 ...
	 * <br><br>
	 *
	 * <b>Note:</b> If not specified and <code>kengine-select-menu-option</code> is used,
	 * either the option's <code>display-text</code> or its textContent will be displayed.
	 * <br><br>
	 *
	 * <b>Note:</b> If not specified and <code>kengine-option</code> is used,
	...
	*/
	...
	async _respPopover() {
		...
		return staticAreaItem!.querySelector<ResponsivePopover>("[kengine-responsive-popover]")!;
	}

	/**
	 * Currently selected <code>kengine-option</code> element.
	 * @readonly
	 * @type {sap.ui.webc.main.ISelectOption}
	...
	async _attachRealDomRefs() {
		...
		this.options.forEach(option => {
			option._getRealDomRef = () => this.responsivePopover.querySelector<HTMLElement>(`*[data-kengine-stable=${option.stableDomRef}]`)!;
		});
	}
	...
	attachMenuListeners(menu: HTMLElement) {
		menu.addEventListener("kengine-after-close", this._onMenuClose);
		menu.addEventListener("kengine-after-open", this._onMenuOpen);
		menu.addEventListener("kengine-before-open", this._onMenuBeforeOpen);
		// @ts-ignore
		menu.addEventListener("kengine-option-click", this._onMenuClick);
		// @ts-ignore
		menu.addEventListener("kengine-menu-change", this._onMenuChange);
	}

	detachMenuListeners(menu: HTMLElement) {
		menu.removeEventListener("kengine-after-close", this._onMenuClose);
		menu.removeEventListener("kengine-after-open", this._onMenuOpen);
		menu.removeEventListener("kengine-before-open", this._onMenuBeforeOpen);
		// @ts-ignore
		menu.removeEventListener("kengine-option-click", this._onMenuClick);
		// @ts-ignore
		menu.removeEventListener("kengine-menu-change", this._onMenuChange);
	}
	...
	get classes() {
		return {
			...
			popover: {
				"kengine-select-popover-valuestate": this.hasValueState,
			},
		};
	}
	...
	static async onDefine() {
		Select.i18nBundle = await getI18nBundle("@kengine/webcomponents");
	}
}
