...
import { isPhone, isAndroid, isSafari } from "@kengine/webcomponents-base/dist/Device.js";
...
interface IComboBoxItem extends KENGINEElement {
	text: string,
	focused: boolean,
	isGroupItem: boolean,
	selected?: boolean,
 ...
 *
 * The <code>kengine-combobox</code> component represents a drop-down menu with a list of the available options and a text input field to narrow down the options.
 *
 * It is commonly used to enable users to select an option from a predefined list.
 *
 * <h3>Structure</h3>
 * The <code>kengine-combobox</code> consists of the following elements:
 *
 * <ul>
 ...
 *
 * The <code>kengine-combobox</code> provides advanced keyboard handling.
 * <br>
 *
 * <ul>
 ...
 *
 * <code>import "@kengine/webcomponents/dist/ComboBox";</code>
 *
 *
 * @constructor
...
class ComboBox extends KENGINEElement {
	/**
	 ...
	 * when the <code>kengine-combobox</code> is in <code>Information</code>, <code>Warning</code> or <code>Error</code> value state.
	 * @type {HTMLElement[]}
	...
	onBeforeRendering() {
		...
		this.style.setProperty(getScopedVarName("--_kengine-input-icons-count"), `${slottedIconsCount + arrowDownIconsCount}`);
	}
	...
	_focusout(e: FocusEvent) {
		...
		const focusedOutToValueStateMessage = toBeFocused?.shadowRoot?.querySelector(".kengine-valuestatemessage-root");
	...
	async _getValueStatePopover() {
		...
		const popover: Popover = staticAreaItem!.querySelector<Popover>(".kengine-valuestatemessage-popover")!;
	...
	_closeRespPopover(e?: Event) {
		...
		if (e && (e.target as HTMLElement).classList.contains("kengine-responsive-popover-close-btn") && this._selectedItemText) {
			this.value = this._selectedItemText;
			this.filterValue = this._selectedItemText;
		}

		if (e && (e.target as HTMLElement).classList.contains("kengine-responsive-popover-close-btn")) {
	...
	get inner(): HTMLInputElement {
		return isPhone() ? this.responsivePopover!.querySelector(".kengine-input-inner-phone")! : this.shadowRoot!.querySelector("[inner-input]")!;
	}

	async _getPicker() {
		const staticAreaItem = await this.getStaticAreaItemDomRef();
		const picker = staticAreaItem!.querySelector<ResponsivePopover>("[kengine-responsive-popover]")!;
	...
	static async onDefine() {
		ComboBox.i18nBundle = await getI18nBundle("@kengine/webcomponents");
	}
	...
	get classes() {
		return {
			popover: {
				"kengine-suggestions-popover": !this._isPhone,
				"kengine-suggestions-popover-with-value-state-header": !this._isPhone && this.hasValueStateText,
			},
			popoverValueState: {
