...
import { getLastTabbableElement } from "@kengine/webcomponents-base/dist/util/TabbableElements.js";
import { getEventMark } from "@kengine/webcomponents-base/dist/MarkedEvents.js";
 ...
 *
 * The <code>kengine-table-row</code> component represents a row in the <code>kengine-table</code>.
 *
 ...
 * <br>
 * The <code>kengine-table-row</code> exposes the following CSS Shadow Parts:
 * <ul>
 ...
 * @tagname kengine-table-row
 * @implements sap.ui.webc.main.ITableRow
 * @public
 */
@customElement({
	tag: "kengine-table-row",
 ...
 */
@event("f7-pressed")
class TableRow extends KENGINEElement implements ITableRow, ITabbable {
	/**
	 ...
	 * <br><br>
	 * <b>Note:</b> Use <code>kengine-table-cell</code> for the intended design.
	 *
	...
	_onkeydown(e: KeyboardEvent) {
		...
		const isRowFocused = this._activeElementHasAttribute("kengine-table-row");
		const target = e.target as HTMLElement;
		const checkboxPressed = target.classList.contains("kengine-multi-select-checkbox");
	...
	_onfocusin(e: FocusEvent, forceSelfFocus = false) {
		if (forceSelfFocus || this._activeElementHasAttribute("kengine-table-cell")) {
			this.root.focus();
			this.activate();
		}
	...
	_onrowclick(e: MouseEvent) {
		const checkboxPressed = (e.target as HTMLElement).classList.contains("kengine-multi-select-checkbox");
		// If the user tab over a button on IOS device, the document.activeElement
		// is the kengine-table-row. The check below ensure that, if a button within the row is pressed,
		...
		if (!this.contains(activeElement)) {
			// If the user clickes on non-focusable element within the kengine-table-cell,
		...
		if (this._activeElementHasAttribute("kengine-table-row")) {
		...
		this._columnsInfo.forEach((info, index) => {
			...
			if (info.visible) {
				...
				this.popinCells.push({
					...
					classes: `kengine-table-popin-row ${allColumnsPoppedInClass} ${popinHeaderClass}`,
	...
	get root() {
		return this.shadowRoot!.querySelector<HTMLElement>(".kengine-table-row-root")!;
	}
	...
	static async onDefine() {
		TableRow.i18nBundle = await getI18nBundle("@kengine/webcomponents");
	}
}
