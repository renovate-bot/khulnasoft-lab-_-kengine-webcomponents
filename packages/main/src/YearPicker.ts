...
import { getMaxCalendarDate } from "@kengine/webcomponents-localization/dist/dates/ExtremeDates.js";
 ...
 * @tagname kengine-yearpicker
 * @public
 */
@customElement({
	tag: "kengine-yearpicker",
...
class YearPicker extends CalendarPart implements ICalendarPicker {
	...
	static async onDefine() {
		YearPicker.i18nBundle = await getI18nBundle("@kengine/webcomponents");
	}
	...
	_buildYears() {
		...
		for (let i = 0; i < pageSize; i++) {
			...
			const year: Year = {
				...
				disabled: isDisabled,
				classes: "kengine-yp-item",
			};

			if (isSelected) {
				year.classes += " kengine-yp-item--selected";
			}

			if (isDisabled) {
				year.classes += " kengine-yp-item--disabled";
			}

			if (this.hasSecondaryCalendarType) {
				year.classes += " kengine-yp-item-secondary-type";
			}
	...
	_selectYear(e: Event) {
		...
		if (target.className.indexOf("kengine-yp-item") > -1) {
