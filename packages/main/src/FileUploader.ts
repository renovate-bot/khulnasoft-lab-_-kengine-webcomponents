 ...
 *
 * The <code>kengine-file-uploader</code> opens a file explorer dialog and enables users to upload files.
 ...
 *
 * For the <code>kengine-file-uploader</code>
 * <h3>ES6 Module Import</h3>
 *
 * <code>import "@kengine/webcomponents/dist/FileUploader.js";</code>
 *
 * @constructor
 * @since 1.0.0-rc.6
 ...
 * @tagname kengine-file-uploader
 * @public
 */
@customElement({
	tag: "kengine-file-uploader",
	languageAware: true,
	renderer: litRender,
...
class FileUploader extends KENGINEElement implements IFormElement {
	/**
	...
	_onmouseover() {
		this.content.forEach(item => {
			item.classList.add("kengine_hovered");
		});
	}

	_onmouseout() {
		this.content.forEach(item => {
			item.classList.remove("kengine_hovered");
		});
	}
	...
	async _getPopover(): Promise<Popover> {
		...
		return staticAreaItem!.querySelector<Popover>(".kengine-valuestatemessage-popover")!;
	}
	...
	get _keepInputInShadowDOM(): boolean {
		// only put input in the light dom when kengine-file-uploader is placed inside form and there is no support for form elements
		return this._canUseNativeFormSupport || !this.name;
	}

	get _input(): HTMLInputElement {
		return (this.shadowRoot!.querySelector<HTMLInputElement>("input[type=file]") || this.querySelector<HTMLInputElement>("input[type=file][data-kengine-form-support]"))!;
	}
	...
	get styles() {
		return {
			popoverHeader: {
				"width": `${this.kengineInput ? this.kengineInput.offsetWidth : 0}px`,
			},
		};
	}

	get kengineInput() {
		return this.shadowRoot!.querySelector<Input>(".kengine-file-uploader-input");
	}

	static async onDefine() {
		FileUploader.i18nBundle = await getI18nBundle("@kengine/webcomponents");
	}
}
