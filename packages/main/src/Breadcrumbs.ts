 ...
 * The <code>kengine-breadcrumbs</code> provides advanced keyboard handling.
 * <br>
 * <ul>
 ...
 * @tagname kengine-breadcrumbs
 * @appenddocs sap.ui.webc.main.BreadcrumbsItem
 * @public
 * @since 1.0.0-rc.15
 */
@customElement({
	tag: "kengine-breadcrumbs",
	languageAware: true,
	renderer: litRender,
...
class Breadcrumbs extends KENGINEElement {
	/**
	...
	*/
	 ...
	 *
	 * <br><br>
	 * <b>Note:</b> Use the <code>kengine-breadcrumbs-item</code> component to define the desired items.
	...
	_cacheWidths() {
		...
		if (!this._isOverflowEmpty) {
			const arrow = this.shadowRoot!.querySelector<HTMLElement>(".kengine-breadcrumbs-dropdown-arrow-link-wrapper")!;
	...
	_updateOverflow() {
		const items = this._getItems(),
			availableWidth = this.shadowRoot!.querySelector<HTMLElement>(".kengine-breadcrumbs-root")!.offsetWidth;
	...
	_preprocessItems() {
		this.items.forEach(item => {
			item._getRealDomRef = () => this.getDomRef()!.querySelector(`[data-kengine-stable*=${item.stableDomRef}]`)!;
		});
	}
	...
	getCurrentLocationLabelWrapper() {
		return this.shadowRoot!.querySelector<HTMLElement>(".kengine-breadcrumbs-current-location > span");
	}
	...
	get _currentLocationLabel() {
		return this.shadowRoot!.querySelector<Label>(".kengine-breadcrumbs-current-location [kengine-label]");
	}
	...
	get _dropdownArrowLink() {
		return this.shadowRoot!.querySelector<Link>(".kengine-breadcrumbs-dropdown-arrow-link-wrapper [kengine-link]")!;
	}
	 ...
	 */
	get _links() {
		return Array.from(this.shadowRoot!.querySelectorAll<Link>(".kengine-breadcrumbs-link-wrapper [kengine-link]"));
	}
	...
	static async onDefine() {
		Breadcrumbs.i18nBundle = await getI18nBundle("@kengine/webcomponents");
	}
}
