...
import type { StyleData } from "@kengine/webcomponents-base/dist/types.js";
...
import "@kengine/webcomponents-icons/dist/slim-arrow-up.js";
...
interface ITab extends KENGINEElement {
	isSeparator: boolean;
 ...
 *
 * The <code>kengine-tabcontainer</code> represents a collection of tabs with associated content.
 ...
 * <h3>Structure</h3>
 *
 * The <code>kengine-tabcontainer</code> can hold two types of entities:
 * <ul>
 * <li><code>kengine-tab</code> - contains all the information on an item (text and icon)</li>
 * <li><code>kengine-tab-separator</code> - used to separate tabs with a line</li>
 * </ul>
 *
 * <h3>Hierarchies</h3>
 ...
 * <br>
 * The <code>kengine-tabcontainer</code> exposes the following CSS Shadow Parts:
 * <ul>
 ...
 *
 * <code>import "@kengine/webcomponents/dist/TabContainer";</code>
 * <br>
 * <code>import "@kengine/webcomponents/dist/Tab";</code> (for <code>kengine-tab</code>)
 * <br>
 * <code>import "@kengine/webcomponents/dist/TabSeparator";</code> (for <code>kengine-tab-separator</code>)
 *
 * @constructor
 ...
 * @tagname kengine-tabcontainer
 * @public
 */
@customElement({
	tag: "kengine-tabcontainer",
	languageAware: true,
	fastNavigation: true,
...
class TabContainer extends KENGINEElement {
	/**
	...
	async _onTabStripClick(e: Event) {
		...
		if ((e.target as HTMLElement).hasAttribute("kengine-button")) {
	...
	async _onTabExpandButtonClick(e: Event) {
		...
		if (e.type === "keydown" && !(e.target as Tab)._realTab.isSingleClickArea) {
			button = (e.target as Tab).querySelectorAll<HTMLElement>(".kengine-tab-expand-button")[0];
	...
	_getFirstFocusableItemInOverflow() {
		return <TabContainerTabInOverflow>(<List> this.responsivePopover!.content[0]).items.find(item => item.classList.contains("kengine-tab-overflow-item"));
	}
	 ...
	 * <pre><code>
	 * 	&lt;kengine-tabcontainer&gt;
	 * 		&lt;kengine-tab id="First" text="First"&gt;
	 * 			...
	 * 			&lt;kengine-tab slot="subTabs" id="Nested" text="Nested"&gt;...&lt;/kengine-tab&gt;
	 * 		&lt;/kengine-tab&gt;
	 * 		&lt;kengine-tab id="Second" text="Second"&gt;...&lt;/kengine-tab&gt;
	 * 		&lt;kengine-tab-separator id="sep"&gt;&lt;/kengine-tab-separator&gt;
	 * 		&lt;kengine-tab id="Third" text="Third"&gt;...&lt;/kengine-tab&gt;
	 * 	&lt;/kengine-tabcontainer&gt;
	 * </code></pre>
	 * Calling <code>allItems</code> on this TabContainer will return the instances in the following order:
	 * <code>[ kengine-tab#First, kengine-tab#Nested, kengine-tab#Second, kengine-tab-separator#sep, kengine-tab#Third ]</code>
	 * @public
	 * @readonly
	...
	_getAllSubItems(items: Array<ITab>, result: Array<ITab> = [], level = 1) {
		items.forEach(item => {
			if (item.hasAttribute("kengine-tab") || item.hasAttribute("kengine-tab-separator")) {
				item._level = level;
				result.push(item);
	...
	async toggleAnimated(selectedTab: Tab, previousTab: Tab) {
		const content = this.shadowRoot!.querySelector<HTMLElement>(".kengine-tc__content")!;
	...
	async _onOverflowClick(e: Event) {
		if ((e.target as HTMLElement).classList.contains("kengine-tc__overflow")) {
			// the empty area in the overflow was clicked
			return;
		}

		const overflow = e.currentTarget as HTMLElement;
		const isEndOverflow = overflow.classList.contains("kengine-tc__overflow--end");
	...
	_addStyleIndent(tabs: Array<ITab>) {
		...
		walk(tabs, tab => {
			...
			tab._style = {
				[getScopedVarName("--_kengine-tab-indentation-level")]: level,
				[getScopedVarName("--_kengine-tab-extra-indent")]: extraIndent ? 1 : null,
			};
		});
	}
	...
	async _onOverflowKeyDown(e: KeyboardEvent) {
		const overflow = e.currentTarget as HTMLElement;
		const isEndOverflow = overflow.classList.contains("kengine-tc__overflow--end");
		const isStartOverflow = overflow.classList.contains("kengine-tc__overflow--start");
	...
	_getRootTab(tab: Tab) {
		while (tab.hasAttribute("kengine-tab")) {
			if (tab.parentElement!.hasAttribute("kengine-tabcontainer")) {
				break;
			}
	...
	_getStartOverflow() {
		return this.shadowRoot!.querySelector<HTMLElement>(".kengine-tc__overflow--start")!;
	}

	_getEndOverflow() {
		return this.shadowRoot!.querySelector<HTMLElement>(".kengine-tc__overflow--end")!;
	}

	_getStartOverflowBtnDOM() {
		return this._getStartOverflow().querySelector<Button>("[kengine-button]");
	}

	_getEndOverflowBtnDOM() {
		return this._getEndOverflow().querySelector<Button>("[kengine-button]");
	}
	...
	get classes() {
		return {
			root: {
				"kengine-tc-root": true,
				"kengine-tc--textOnly": this.textOnly,
				"kengine-tc--withAdditionalText": this.withAdditionalText,
				"kengine-tc--standardTabLayout": this.standardTabLayout,
			},
			header: {
				"kengine-tc__header": true,
			},
			tabStrip: {
				"kengine-tc__tabStrip": true,
			},
			separator: {
				"kengine-tc__separator": true,
			},
			content: {
				"kengine-tc__content": true,
				"kengine-tc__content--collapsed": this._contentCollapsed,
			},
		};
	}
	...
	static async onDefine() {
		TabContainer.i18nBundle = await getI18nBundle("@kengine/webcomponents");
	}
}
