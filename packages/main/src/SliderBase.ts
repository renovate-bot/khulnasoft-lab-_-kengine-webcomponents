...
import { isPhone, supportsTouch } from "@kengine/webcomponents-base/dist/Device.js";
...
import "@kengine/webcomponents-icons/dist/direction-arrows.js";
import {
 ...
 */
@event("input")
abstract class SliderBase extends KENGINEElement {
	/**
	...
	get classes() {
		return {
			root: {
				"kengine-slider-root-phone": isPhone(),
			},
			labelContainer: {
				"kengine-slider-hidden-labels": this._labelsOverlapping,
			},
		};
	}
	...
	_handleResize() {
		...
		const labelItems = this.shadowRoot!.querySelectorAll(".kengine-slider-labels li");
