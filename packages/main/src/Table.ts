...
import NavigationMode from "@kengine/webcomponents-base/dist/types/NavigationMode.js";
import {
	isTabNext,
	isTabPrevious,
	isSpace,
...
import getNormalizedTarget from "@kengine/webcomponents-base/dist/util/getNormalizedTarget.js";
import getActiveElement from "@kengine/webcomponents-base/dist/util/getActiveElement.js";
import { getLastTabbableElement, getTabbableElements } from "@kengine/webcomponents-base/dist/util/TabbableElements.js";
...
import debounce from "@kengine/webcomponents-base/dist/util/debounce.js";
import isElementInView from "@kengine/webcomponents-base/dist/util/isElementInView.js";
...
interface ITableRow extends KENGINEElement {
	mode: `${TableMode}`,
	selected: boolean,
	_busy: boolean,
	_tabIndex: string,
 ...
 *
 * The <code>kengine-table</code> component provides a set of sophisticated and convenient functions for responsive table design.
 ...
 *
 * <h3>Selection</h3>
 * To benefit from the selection mechanism of <code>kengine-table</code> component, you can use the available selection modes:
 * <code>SingleSelect</code> and <code>MultiSelect</code>.
 * <br>
 * In additition to the used mode, you can also specify the <code>kengine-table-row</code> type choosing between
 ...
 * <br><br>
 * Furthermore, you can interact with <code>kengine-table</code> via the following keys.
 * <br>
 *
 * <ul>
 ...
 *
 * <code>import "@kengine/webcomponents/dist/Table.js";</code>
 * <br>
 * <code>import "@kengine/webcomponents/dist/TableColumn.js";</code> (for <code>kengine-table-column</code>)
 * <br>
 * <code>import "@kengine/webcomponents/dist/TableRow.js";</code> (for <code>kengine-table-row</code>)
 * <br>
 * <code>import "@kengine/webcomponents/dist/TableCell.js";</code> (for <code>kengine-table-cell</code>)
 *
 * @constructor
 * @author KHULNASOFT SE
 * @alias sap.ui.webc.main.Table
 * @extends sap.ui.webc.base.KENGINEElement
 * @tagname kengine-table
...
/**
* Fired when <code>kengine-table-column</code> is shown as a pop-in instead of hiding it.
*
...
class Table extends KENGINEElement {
	/**
	 ...
	 * <br><br>
	 * <b>Note:</b> Use <code>kengine-table-row</code> for the intended design.
	 *
	 ...
	 * <br><br>
	 * <b>Note:</b> Use <code>kengine-table-column</code> for the intended design.
	 *
	...
	static async onDefine() {
		Table.i18nBundle = await getI18nBundle("@kengine/webcomponents");
	}
	...
	constructor() {
		...
		// The ItemNavigation requires each item to 1) have a "_tabIndex" property and 2) be either a KENGINEElement, or have an id property (to find it in the component's shadow DOM by)
		this._columnHeader = {
	...
	onBeforeRendering() {
		...
		this.rows.forEach((row, index) => {
			...
			row._busy = this.busy;
			row.removeEventListener("kengine-_focused", this.fnOnRowFocused as EventListener);
			row.addEventListener("kengine-_focused", this.fnOnRowFocused as EventListener);
			row.removeEventListener("kengine-f7-pressed", this.fnHandleF7 as EventListener);
			row.addEventListener("kengine-f7-pressed", this.fnHandleF7 as EventListener);
			row.mode = this.mode;
		});
	...
	getFocusedElementType(element: HTMLElement): TableFocusTargetElement | undefined {
		...
		if (this.rows.includes(element as ITableRow)) {
			const isGroupRow = element.hasAttribute("kengine-table-group-row");
	 ...
	 * @private
	 * @param { CustomEvent } e "kengine-f7-pressed"
	 */
	...
	getRowParent(child: HTMLElement): ITableRow | undefined {
		if (child.hasAttribute("kengine-table-row")) {
		...
		if (parent.hasAttribute("kengine-table-row")) {
	...
	get tableEndDOM(): Element {
		return this.shadowRoot!.querySelector(".kengine-table-end-marker")!;
	}
