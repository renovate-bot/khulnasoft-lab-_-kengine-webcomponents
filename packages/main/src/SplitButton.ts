 ...
 *
 * <code>kengine-split-button</code> enables users to trigger actions. It is constructed of two separate actions -
 ...
 * <h3>Usage</h3>
 *
 * <code>kengine-split-button</code> consists two separate buttons:
 * <ul>
 ...
 * </ul>
 * You can choose a <code>design</code> from a set of predefined types (the same as for kengine-button) that offer
 * different styling to correspond to the triggered action. Both text and arrow actions have the same design.
 * <br><br>
 * You can set the <code>kengine-split-button</code> as enabled or disabled. Both parts of an enabled
 * <code>kengine-split-button</code> can be pressed by clicking or tapping it, or by certain keys, which changes
 * the style to provide visual feedback to the user that it is pressed or hovered over with
 * the mouse cursor. A disabled <code>kengine-split-button</code> appears inactive and any of the two buttons
 * cannot be pressed.
 *
 ...
 * There are separate events that are fired on activating of <code>kengine-split-button</code> parts:
 * <ul>
 ...
 *
 * <code>import "@kengine/webcomponents/dist/SplitButton.js";</code>
 *
 * @constructor
 * @author KHULNASOFT SE
 * @alias sap.ui.webc.main.SplitButton
 * @extends sap.ui.webc.base.KENGINEElement
 * @tagname kengine-split-button
 * @public
 * @since 1.1.0
 */
@customElement({
	tag: "kengine-split-button",
	renderer: litRender,
 ...
 */
@event("arrow-click")
class SplitButton extends KENGINEElement {
	/**
	...
	static async onDefine() {
		SplitButton.i18nBundle = await getI18nBundle("@kengine/webcomponents");
	}
	...
	get textButton() {
		return this.getDomRef()?.querySelector<Button>(".kengine-split-text-button");
	}

	get arrowButton() {
		return this.getDomRef()?.querySelector<Button>(".kengine-split-arrow-button");
	}
