...
import ItemNavigationBehavior from "@kengine/webcomponents-base/dist/types/ItemNavigationBehavior.js";
 ...
 * The <code>kengine-color-palette</code> provides the users with a range of predefined colors. The colors are fixed and do not change with the theme.
 *
 * <h3>Usage</h3>
 *
 * The <code>kengine-color-palette</code> is meant for users that need to select a color from a predefined set.
 * To define the colors, use the <code>kengine-color-palette-item</code> component inside the default slot of the <code>kengine-color-palette</code>.
 *
 * <h3>ES6 Module Import</h3>
 *
 * <code>import "@kengine/webcomponents/dist/ColorPalette.js";</code>
 *
 * @constructor
 * @author KHULNASOFT SE
 * @alias sap.ui.webc.main.ColorPalette
 * @extends sap.ui.webc.base.KENGINEElement
 * @tagname kengine-color-palette
 * @since 1.0.0-rc.12
 * @appenddocs sap.ui.webc.main.ColorPaletteItem
 * @public
 */
@customElement({
	tag: "kengine-color-palette",
	renderer: litRender,
	...
	get dependencies() {
		...
		return ([ColorPaletteItem, Button] as Array<typeof KENGINEElement>).concat(colorPaletteMoreColors ? colorPaletteMoreColors.dependencies : []);
	},
})
...
class ColorPalette extends KENGINEElement {
	/**
	...
	/**
	 * Defines the <code>kengine-color-palette-item</code> elements.
	...
	onBeforeRendering() {
		...
		if (this.showMoreColors) {
			...
			if (ColorPaletteMoreColorsClass) {
				...
				throw new Error(`You have to import "@kengine/webcomponents/dist/features/ColorPaletteMoreColors.js" module to use the more-colors functionality.`);
			}
		}
	}
	...
	_onclick(e: MouseEvent) {
		...
		if (target.hasAttribute("kengine-color-palette-item")) {
	...
	_onkeyup(e: KeyboardEvent) {
		const target = e.target as ColorPaletteItem;
		if (isSpace(e) && target.hasAttribute("kengine-color-palette-item")) {
			e.preventDefault();
			this.selectColor(target);
		}
	}
	...
	_onkeydown(e: KeyboardEvent) {
		const target = e.target as ColorPaletteItem;
		if (isEnter(e) && target.hasAttribute("kengine-color-palette-item")) {
			this.selectColor(target);
		}
	}
	...
	get recentColorsElements(): Array<ColorPaletteItem> {
		...
		if (domRef) {
			return Array.from(domRef.querySelectorAll<ColorPaletteItem>(".kengine-cp-recent-colors-wrapper [kengine-color-palette-item]")).filter(x => x.value !== "");
		}
	...
	get colorPaletteNavigationElements() {
		...
		const rootElement = this.shadowRoot!.querySelector(".kengine-cp-root")!;

		if (this.showDefaultColor) {
			navigationElements.push(rootElement.querySelector<Button>(".kengine-cp-default-color-button")!);
		}

		navigationElements.push(this.displayedColors[0]);

		if (this.showMoreColors) {
			navigationElements.push(rootElement.querySelector<Button>(".kengine-cp-more-colors")!);
		}
	...
	get classes() {
		return {
			colorPaletteRoot: {
				"kengine-cp-root": true,
				"kengine-cp-root-phone": isPhone(),
			},
		};
	}
	...
	async _getDialog() {
		...
		return staticAreaItem!.querySelector<Dialog>("[kengine-dialog]")!;
	}

	async getColorPicker() {
		const dialog = await this._getDialog();
		return dialog.content[0].querySelector<ColorPicker>("[kengine-color-picker]")!;
	}
}
