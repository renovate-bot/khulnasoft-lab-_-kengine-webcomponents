...
import type LocaleData from "@kengine/webcomponents-localization/dist/LocaleData.js";
...
import calculateWeekNumber from "@kengine/webcomponents-localization/dist/dates/calculateWeekNumber.js";
 ...
 * @class
 *
 * Represents the days inside a single month view of the <code>kengine-calendar</code> component.
 *
 * @constructor
 * @author KHULNASOFT SE
 * @alias sap.ui.webc.main.DayPicker
 * @extends sap.ui.webc.main.CalendarPart
 * @tagname kengine-daypicker
 * @public
 */
@customElement({
	tag: "kengine-daypicker",
	styles: dayPickerCSS,
...
class DayPicker extends CalendarPart implements ICalendarPicker {
	...
	_buildWeeks(localeData: LocaleData) {
		...
		for (let i = 0; i < DAYS_IN_WEEK * 6; i++) { // always show 6 weeks total, 42 days to avoid jumping
			...
			const day: Day = {
				...
				classes: `kengine-dp-item kengine-dp-wday${dayOfTheWeek}`,
				ariaLabel,
			...
			if (isFirstDayOfWeek) {
				day.classes += " kengine-dp-firstday";
			}

			if (isSelected) {
				day.classes += " kengine-dp-item--selected";
			}

			if (isSelectedBetween) {
				day.classes += " kengine-dp-item--selected-between";
			}

			if (isToday) {
				day.classes += " kengine-dp-item--now";
			}

			if (isOtherMonth) {
				day.classes += " kengine-dp-item--othermonth";
			}

			if (isWeekend) {
				day.classes += " kengine-dp-item--weeekend";
			}

			if (isDisabled) {
				day.classes += " kengine-dp-item--disabled";
			}

			if (this.hasSecondaryCalendarType) {
				day.classes += " kengine-dp-item--withsecondtype";
			}
	...
	_buildDayNames(localeData: LocaleData) {
		...
		this._dayNames[1].classes += " kengine-dp-firstday";
	...
	_onmouseover(e: MouseEvent) {
		const target = e.target as HTMLElement;
		const hoveredItem = target.closest(".kengine-dp-item") as HTMLElement;
	...
	get classes() {
		return {
			root: {
				"kengine-dp-root": true,
				"kengine-dp-twocalendartypes": this.hasSecondaryCalendarType,
			},
		};
	}
	...
	_isDayPressed(target: HTMLElement): boolean {
		...
		return (target.className.indexOf("kengine-dp-item") > -1) || (targetParent && targetParent.classList && targetParent.classList.contains("kengine-dp-item"));
	}
