...
import { isTabNext } from "@kengine/webcomponents-base/dist/Keys.js";
 ...
 *
 * The <code>kengine-busy-indicator</code> signals that some operation is going on and that the
 ...
 *
 * <h3>Usage</h3>
 * For the <code>kengine-busy-indicator</code> you can define the size, the text and whether it is shown or hidden.
 * In order to hide it, use the "active" property.
 * <br><br>
 * In order to show busy state over an HTML element, simply nest the HTML element in a <code>kengine-busy-indicator</code> instance.
 * <br>
 * <b>Note:</b> Since <code>kengine-busy-indicator</code> has <code>display: inline-block;</code> by default and no width of its own,
 ...
 *
 * <code>import "@kengine/webcomponents/dist/BusyIndicator";</code>
 *
 * @constructor
 * @author KHULNASOFT SE
 * @alias sap.ui.webc.main.BusyIndicator
 * @extends sap.ui.webc.base.KENGINEElement
 * @tagname kengine-busy-indicator
 * @public
 * @since 0.12.0
 */
@customElement({
	tag: "kengine-busy-indicator",
	languageAware: true,
...
class BusyIndicator extends KENGINEElement {
	/**
	...
	static async onDefine() {
		BusyIndicator.i18nBundle = await getI18nBundle("@kengine/webcomponents");
	}
	...
	get classes() {
		return {
			root: {
				"kengine-busy-indicator-root": true,
			},
		};
	}
	...
	_handleKeydown(e: KeyboardEvent) {
		...
		if (isTabNext(e)) {
			...
			this.shadowRoot!.querySelector<HTMLElement>("[data-kengine-focus-redirect]")!.focus();
	...
	_redirectFocus(e: FocusEvent) {
		...
		e.preventDefault();
		this.shadowRoot!.querySelector<HTMLElement>(".kengine-busy-indicator-busy-area")!.focus();
	}
}
