import type KENGINEElement from "@kengine/webcomponents-base/dist/KENGINEElement.js";
import { registerFeature } from "@kengine/webcomponents-base/dist/FeaturesRegistry.js";
...
// @ts-ignore
import encodeXML from "@kengine/webcomponents-base/dist/sap/base/security/encodeXML.js";
import generateHighlightedMarkup from "@kengine/webcomponents-base/dist/util/generateHighlightedMarkup.js";
import type ValueState from "@kengine/webcomponents-base/dist/types/ValueState.js";
...
interface SuggestionComponent extends KENGINEElement {
...
class Suggestions {
	...
	onItemSelected(selectedItem: SuggestionListItem | null, keyboardUsed: boolean) {
		...
		// to have consistency with the way "Inactive" items behave in the kengine-list
	...
	onItemPress(e: CustomEvent<ListItemClickEventDetail | ListSelectionChangeEventDetail>) {
		...
		const isPressEvent = e.type === "kengine-item-click";

		// Only use the press e if the item is already selected, in all other cases we are listening for 'kengine-selection-change' from the list
	...
	async _attachItemsListeners() {
		...
		list.removeEventListener("kengine-item-click", this.fnOnSuggestionItemPress as EventListener);
		list.addEventListener("kengine-item-click", this.fnOnSuggestionItemPress as EventListener);
		list.removeEventListener("kengine-selection-change", this.fnOnSuggestionItemPress as EventListener);
		list.addEventListener("kengine-selection-change", this.fnOnSuggestionItemPress as EventListener);
	...
	_attachPopupListeners() {
		...
		if (!this.attachedAfterOpened) {
			this.responsivePopover!.addEventListener("kengine-after-open", this._onOpen.bind(this));
			this.attachedAfterOpened = true;
		}

		if (!this.attachedAfterClose) {
			this.responsivePopover!.addEventListener("kengine-after-close", this._onClose.bind(this));
	...
	async _getScrollContainer() {
		if (!this._scrollContainer) {
			...
			this._scrollContainer = this.responsivePopover!.shadowRoot!.querySelector(".kengine-popup-content")!;
		}
	...
	_getItems(): Array<SuggestionListItem> {
		return this.responsivePopover ? [...this.responsivePopover.querySelector<List>("[kengine-list]")!.children] as Array<SuggestionListItem> : [];
	}
	...
	async _getList() {
		this.responsivePopover = await this._getSuggestionPopover();
		return this.responsivePopover.querySelector<List>("[kengine-list]")!;
	}
	...
	async _getSuggestionPopover() {
		...
		this.responsivePopover = staticAreaItem!.querySelector<ResponsivePopover>("[kengine-responsive-popover]")!;
		return this.responsivePopover;
	}
	...
	static async init() {
		Suggestions.i18nBundle = await getI18nBundle("@kengine/webcomponents");
	}
}
