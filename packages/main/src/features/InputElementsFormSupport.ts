...
interface IFormElement extends KENGINEElement {
	value?: string | number,
	name?: string,
	disabled?: boolean,
	required?: boolean,
...
class FormSupport {
	...
	static syncNativeHiddenInput(element: IFormElement, nativeInputUpdateCallback?: NativeInputUpdateCallback) {
		...
		const nativeInput = element.querySelector<NativeFormElement>("input[data-kengine-form-support]");
	...
	static syncNativeHiddenTextArea(element: IFormElement, nativeInputUpdateCallback?: NativeInputUpdateCallback) {
		...
		const nativeTextarea = element.querySelector<NativeFormElement>("textarea[data-kengine-form-support]");
	...
	static syncNativeElement(element: IFormElement, nativeElement: NativeFormElement | null, nativeInputUpdateCallback?: NativeInputUpdateCallback, nativeElementTagName = "input") {
		if (!nativeElement) {
			...
			nativeElement.setAttribute("data-kengine-form-support", "");
	...
	/**
	 * Syncs the native file input element, rendered into the <code>kengine-file-uploader</code> component's light DOM,
	 * with the <code>kengine-file-uploader</code> component's state.
	...
	static syncNativeFileInput(element: IFormElement, nativeInputUpdateCallback: NativeInputUpdateCallback, nativeInputChangeCallback: NativeInputChangeCallback) {
		...
		let nativeInput = element.querySelector<HTMLInputElement>(`input[type="file"][data-kengine-form-support]`);

		if (needsNativeInput && !nativeInput) {
			nativeInput = document.createElement("input");
			nativeInput.type = "file";
			nativeInput.setAttribute("data-kengine-form-support", "");
