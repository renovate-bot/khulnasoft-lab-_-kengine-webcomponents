...
import {
	isPhone,
	isAndroid,
} from "@kengine/webcomponents-base/dist/Device.js";
...
import {
	...
	registerKENGINEElement,
	deregisterKENGINEElement,
} from "@kengine/webcomponents-base/dist/util/AriaLabelHelper.js";
import { getCaretPosition, setCaretPosition } from "@kengine/webcomponents-base/dist/util/Caret.js";
...
import "@kengine/webcomponents-icons/dist/not-editable.js";
...
import "@kengine/webcomponents-icons/dist/information.js";
 ...
 *
 * The <code>kengine-input</code> component allows the user to enter and edit text or numeric values in one line.
 * <br>
 ...
 * <br><br>
 * <b>Note:</b> If you are using the <code>kengine-input</code> as a single npm module,
 * don't forget to import the <code>InputSuggestions</code> module from
 * "@kengine/webcomponents/dist/features/InputSuggestions.js"
 * to enable the suggestions functionality.
 *
 * <h3>Keyboard Handling</h3>
 * The <code>kengine-input</code> provides the following keyboard shortcuts:
 * <br>
 *
 * <ul>
 ...
 *
 * <code>import "@kengine/webcomponents/dist/Input.js";</code>
 * <br>
 * <code>import "@kengine/webcomponents/dist/features/InputSuggestions.js";</code> (optional - for input suggestions support)
 *
 * @constructor
 * @author KHULNASOFT SE
 * @alias sap.ui.webc.main.Input
 * @extends sap.ui.webc.base.KENGINEElement
 * @tagname kengine-input
...
@customElement({
	...
	get dependencies() {
		...
		return ([Popover, Icon] as Array<typeof KENGINEElement>).concat(Suggestions ? Suggestions.dependencies : []);
	},
})
...
class Input extends KENGINEElement implements SuggestionComponent, IFormElement {
	/**
	 ...
	 * from <code>"@kengine/webcomponents/dist/features/InputSuggestions.js"</code> to enable this functionality.
	 * @type {boolean}
	 ...
	 * <br><br>
	 * <b>Note:</b> This property is not compatible with the kengine-input type InputType.Number. If the kengine-input type is set to Number, the maxlength value is ignored.
	 ...
	 * Example:
	 * <br><br>
	 * &lt;kengine-input show-suggestions><br>
	 * &nbsp;&nbsp;&nbsp;&nbsp;&lt;kengine-suggestion-item text="Item #1">&lt;/kengine-suggestion-item><br>
	 * &nbsp;&nbsp;&nbsp;&nbsp;&lt;kengine-suggestion-item text="Item #2">&lt;/kengine-suggestion-item><br>
	 * &lt;/kengine-input>
	 * <br>
	 * <kengine-input show-suggestions>
	 * <kengine-suggestion-group-item text="Group #1"></kengine-suggestion-group-item>
	 * <kengine-suggestion-item text="Item #1"></kengine-suggestion-item>
	 * <kengine-suggestion-item text="Item #2"></kengine-suggestion-item>
	 * <kengine-suggestion-group-item text="Group #2"></kengine-suggestion-group-item>
	 * <kengine-suggestion-item text="Item #3"></kengine-suggestion-item>
	 * <kengine-suggestion-item text="Item #4"></kengine-suggestion-item>
	 * </kengine-input>
	 * <br><br>
	 * <b>Note:</b> The suggestions would be displayed only if the <code>showSuggestions</code>
	 * property is set to <code>true</code>.
	 * <br><br>
	 * <b>Note:</b> The <code>&lt;kengine-suggestion-item&gt;</code> and <code>&lt;kengine-suggestion-group-item&gt;</code> are recommended to be used as suggestion items.
	 * <br><br>
	 * <b>Note:</b> Importing the Input Suggestions Support feature:
	 * <br>
	 * <code>import "@kengine/webcomponents/dist/features/InputSuggestions.js";</code>
	 * <br>
	 * automatically imports the <code>&lt;kengine-suggestion-item></code> and <code>&lt;kengine-suggestion-group-item></code> for your convenience.
	 *
	...
	onEnterDOM() {
		...
		registerKENGINEElement(this, this._updateAssociatedLabelsTexts.bind(this));
	}

	onExitDOM() {
		ResizeHandler.deregister(this, this._handleResizeBound);
		deregisterKENGINEElement(this);
	}
	...
	onBeforeRendering() {
		...
		this.style.setProperty(getScopedVarName("--_kengine-input-icons-count"), `${this.iconsCount}`);
	...
	async _onfocusin(e: FocusEvent) {
		...
		this._inputIconFocused = !!e.target && e.target === this.querySelector<Icon>("[kengine-icon]");
		this._focusedAfterClear = false;
	}
	...
	_onfocusout(e: FocusEvent) {
		...
		const focusedOutToValueStateMessage = toBeFocused && toBeFocused.shadowRoot && toBeFocused.shadowRoot.querySelector(".kengine-valuestatemessage-root");
	...
	async _getPopover() {
		...
		return staticAreaItem!.querySelector<Popover>("[kengine-popover]")!;
	}
	...
	enableSuggestions() {
		...
		if (Suggestions) {
			...
			throw new Error(`You have to import "@kengine/webcomponents/dist/features/InputSuggestions.js" module to use kengine-input suggestions`);
		}
	}
	...
	async getInputDOMRef() {
		if (isPhone() && this.Suggestions) {
			...
			return this.Suggestions.responsivePopover!.querySelector<Input>(".kengine-input-inner-phone")!;
		}
	...
	getInputDOMRefSync() {
		if (isPhone() && this.Suggestions && this.Suggestions.responsivePopover) {
			return this.Suggestions.responsivePopover.querySelector(".kengine-input-inner-phone")!.shadowRoot!.querySelector<HTMLInputElement>("input")!;
		}
	...
	getSuggestionByListItem(item: SuggestionListItem): SuggestionItem {
		const key = parseInt(item.getAttribute("data-kengine-key")!);
	...
	get classes(): ClassMap {
		return {
			popover: {
				"kengine-suggestions-popover": !this._isPhone && this.showSuggestions,
				"kengine-suggestions-popover-with-value-state-header": !this._isPhone && this.showSuggestions && this.hasValueStateMessage,
			},
			popoverValueState: {
				"kengine-valuestatemessage-root": true,
				"kengine-valuestatemessage-header": true,
				"kengine-valuestatemessage--success": this.valueState === ValueState.Success,
				"kengine-valuestatemessage--error": this.valueState === ValueState.Error,
				"kengine-valuestatemessage--warning": this.valueState === ValueState.Warning,
				"kengine-valuestatemessage--information": this.valueState === ValueState.Information,
			},
		};
	}
	...
	static async onDefine() {
		...
		[Input.i18nBundle] = await Promise.all([
			getI18nBundle("@kengine/webcomponents"),
