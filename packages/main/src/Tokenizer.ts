 ...
 * @tagname kengine-tokenizer
 * @usestextcontent
 * @private
 */
@customElement({
	tag: "kengine-tokenizer",
	languageAware: true,
	renderer: litRender,
...
class Tokenizer extends KENGINEElement {
	...
	_onmousedown(e: MouseEvent) {
		if ((e.target as HTMLElement).hasAttribute("kengine-token")) {
	 ...
	 * This method should only be used by kengine-multi-combobox and kengine-multi-input
	 *
	 * @protected
	...
	async itemDelete(e: CustomEvent) {
		...
		if (this._getTokens().length === 1 && this._getTokens()[0].isTruncatable) {
			...
			morePopover.addEventListener("kengine-after-close", () => {
				this.fireEvent<TokenizerTokenDeleteEventDetail>("token-delete", { ref: token });
			}, {
				once: true,
			});
	...
	_handleTokenSelection(e: KeyboardEvent | MouseEvent, deselectAll = true) {
		const target = e.target as Token;
		if (target.hasAttribute("kengine-token")) {
	...
	get contentDom() {
		return this.shadowRoot!.querySelector<HTMLElement>(".kengine-tokenizer--content")!;
	}

	get expandedContentDom() {
		return this.shadowRoot!.querySelector<HTMLElement>(".kengine-tokenizer-expanded--content");
	}

	get narrowContentDom() {
		return this.shadowRoot!.querySelector<HTMLElement>(".kengine-tokenizer-nmore--content");
	}
	...
	get classes(): ClassMap {
		return {
			wrapper: {
				"kengine-tokenizer-root": true,
				"kengine-tokenizer-nmore--wrapper": this.showMore,
				"kengine-tokenizer-no-padding": !this._getTokens().length,
			},
			content: {
				"kengine-tokenizer--content": true,
				"kengine-tokenizer-expanded--content": !this.showNMore,
				"kengine-tokenizer-nmore--content": this.showNMore,
			},
			popoverValueState: {
				"kengine-valuestatemessage-root": true,
				"kengine-responsive-popover-header": true,
	...
	static async onDefine() {
		Tokenizer.i18nBundle = await getI18nBundle("@kengine/webcomponents");
	}
