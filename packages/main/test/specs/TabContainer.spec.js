...
describe("TabContainer general interaction", () => {
	...
	it("tests initially selected tab", async () => {
		...
		const selectedFilter = await tabContainer.shadow$(".kengine-tab-strip-item:nth-child(4)");
		const SELECTION_CSS_CLASS = "kengine-tab-strip-item--selected";
		...
		assert.strictEqual(selectedFilter.id, selectedTab.id, "The IDs of the kengine-tab and the rendered tab matches.");
	});
	...
	it("tests tabSelect event", async () => {
		const item = await browser.$("#tabContainer1").shadow$(".kengine-tab-strip-item:nth-child(3)");
	...
	it("tests preventing tabSelect", async() => {
		...
		const selectedTab = await browser.$("#tabContainer1").shadow$(".kengine-tab-strip-item--selected");
		const newTab = await browser.$("#tabContainer1").shadow$(".kengine-tab-strip-item:nth-child(1)");
		...
		// Assert
		assert.ok(await selectedTab.hasClass("kengine-tab-strip-item--selected"), "previously selected tab is still selected");
		assert.notOk(await newTab.hasClass("kengine-tab-strip-item--selected"), "clicked tab is not selected");
	...
	it("tests aria attrs", async () => {
		const tabContainer = await browser.$("#tabContainer1");
		const tab4 = await tabContainer.shadow$(".kengine-tab-strip-item:nth-child(4)");

		assert.notOk(await tabContainer.shadow$(".kengine-tc__tabStrip").getAttribute("aria-describedby"),"aria-describedby is not set");
		assert.notOk(await tabContainer.shadow$(".kengine-hidden-text").isExisting(),"hidden text is not created");
	...
	it("tests aria attrs when sub tabs", async () => {
		...
		const tab1 = await tabContainer.shadow$(".kengine-tab-strip-item:nth-child(1)");
		const tab2 = await tabContainer.shadow$(".kengine-tab-strip-item:nth-child(3)");
		const tab3 = await tabContainer.shadow$(".kengine-tab-strip-item:nth-child(4)");

		const tablistAriaDescribedBy = await tabContainer.shadow$(".kengine-tc__tabStrip").getAttribute("aria-describedby");
		const hiddenTextId = await tabContainer.shadow$(".kengine-hidden-text").getAttribute("id");
	...
	it("tests start and end overflow behavior", async () => {
		...
		const startOverflow = await tabcontainer.shadow$(".kengine-tc__overflow--start");
		...
		const item = await (await popover.$("kengine-list").$$("kengine-li-custom"))[0];
	...
	it("tests end overflow behavior", async () => {
		...
		const endOverflow = await tabcontainer.shadow$(".kengine-tc__overflow--end");
		...
		const popover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover");
		await (await popover.$("kengine-list").$$("kengine-li-custom"))[0].click();
	...
	it("tests removing of responsive paddings for the content", async () => {
		...
		const actualContentPadding = await tabContainer.shadow$(".kengine-tc__content").getCSSProperty("padding-left");
	...
	it("tests nested tabs", async () => {
		const tabContainer = await browser.$("#tabContainerNestedTabs");
		const expandButton = await tabContainer.shadow$(".kengine-tab-expand-button [kengine-button]");
		...
		await expandButton.click();
		await (await popover.$("kengine-list").$$("kengine-li-custom"))[1].click();
		...
		await expandButton.click();
		await (await popover.$("kengine-list").$$("kengine-li-custom"))[4].click();
		newlySelectedItem = await tabContainer.$("[selected]");
		assert.notOk(await browser.$("#button21").isDisplayed(), "Content for tab 2.1 is not displayed");
		assert.notOk(await browser.$("#button211").isDisplayed(), "Content for tab 2.1.1 is not displayed");
	...
	it("selecting a tab programatically will update the tab strip", async () => {
		...
		assert.ok(await lastTabInStrip.hasClass("kengine-tab-strip-item--selected"), "last tab is selected");
		...
		assert.notOk(await lastTabInStrip.hasClass("kengine-tab-strip-item--selected"), "last tab is selected");
		assert.ok(await firstTabInStrip.isDisplayed(), "last tab in strip is visible");
		assert.ok(await firstTabInStrip.hasClass("kengine-tab-strip-item--selected"), "last tab is selected");
		...
		assert.notOk(await firstTabInStrip.hasClass("kengine-tab-strip-item--selected"), "last tab is selected");
		assert.ok(await nestedTabParentInTabStrip.isDisplayed(), "last tab in strip is visible");
		assert.ok(await nestedTabParentInTabStrip.hasClass("kengine-tab-strip-item--selected"), "last tab is selected");
	});

	it("tests effective selected tab", async () => {
		const tabContainer = await browser.$("#tabContainerAddTabsProgrammatically");
		const allInitialTabs = await tabContainer.$$("kengine-tab");
		...
		const allTabs = await tabContainer.$$("kengine-tab");
	...
	it("tests tabs dom ref", async () => {
		...
		const productsTabDomRefInStripExpected = await browser.$(() => document.getElementById("tabContainer1").getDomRef().querySelector(".kengine-tab-strip-item:first-child"));
