...
describe("ResponsivePopover general interaction", () => {
	...
	it("header and footer are displayed by default", async () => {
		...
		const header = await popover.shadow$(".kengine-popup-header-root");
...
describe("Acc", () => {
	...
	it("tests accessible-role", async () => {
		...
		assert.strictEqual(await respPopover.shadow$(".kengine-popup-root").getAttribute("role"), "dialog","The default role is applied.");
		assert.strictEqual(await respPopover.shadow$(".kengine-popup-root").getAttribute("aria-modal"), "true", "aria-modal=true is applied.");

		const respPopoverAlertRole = await browser.$("#rPAlertRole");

		assert.strictEqual(await respPopoverAlertRole.shadow$(".kengine-popup-root").getAttribute("role"), "alertdialog", "role='alertdialog' is applied.");
		assert.strictEqual(await respPopoverAlertRole.shadow$(".kengine-popup-root").getAttribute("aria-modal"), "true", "aria-modal=true is applied.");

		const respPopoverNoneRole = await browser.$("#rPNoneRole");

		assert.notOk(await respPopoverNoneRole.shadow$(".kengine-popup-root").getAttribute("role"), "role is not set.");
		assert.notOk(await respPopoverNoneRole.shadow$(".kengine-popup-root").getAttribute("aria-modal"), "aria-modal not set.");
	});
});