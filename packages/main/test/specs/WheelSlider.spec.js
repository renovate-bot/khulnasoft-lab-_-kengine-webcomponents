...
describe("Wheel Slider general interaction", () => {
	...
	it("tests slider's label rendering", async () => {
		const textValue = await (await browser.$("#wheelslider").shadow$$(".kengine-wheelslider-label"))[0].getText();
	...
	it("Arrow down button is working", async () => {
		...
		const button = (await slider.shadow$$(".kengine-wheelslider-arrow"))[1];
		await button.click();
	...
	it("Arrow up button is working", async () => {
		...
		const button = await slider.shadow$$(".kengine-wheelslider-arrow")[0];
	...
	it("Click on element selects element", async () => {
		const slider = await browser.$("#wheelslider");
		const sliderElements = await slider.shadow$$(".kengine-wheelslider-item");
