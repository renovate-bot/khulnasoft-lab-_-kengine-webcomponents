...
describe("MultiInput general interaction", () => {
	...
	it("tests expanding of tokenizer", async () => {
		...
		const basicTokenizer = await basic.shadow$("kengine-tokenizer");
	...
	it ("tests opening of tokenizer Popover", async () => {
		const tokenizer = await browser.$("#basic-overflow").shadow$("kengine-tokenizer");
	...
	it ("fires value-help-trigger on icon press", async () => {
		...
		const icon = await browser.$("#basic-overflow-and-icon").shadow$("kengine-icon");
	...
	it ("fires value-help-trigger with F4 and Alt/Option + ArrowUp/Down", async () => {
		...
		const multiInputInner = await browser.$("#multi-with-value-help-icon").shadow$(".kengine-input-inner");
	...
	it ("adds a token to multi input", async () => {
		...
		assert.strictEqual((await mi.$$("kengine-token")).length, 1, "should have 1 token");
		await browser.$("#suggestion-token").scrollIntoView();

		let allTokens = await mi.$$("kengine-token");
		assert.notOk(await allTokens[0].getProperty("overflows"), "Token should not overflow");

		await btn.click();

		allTokens = await mi.$$("kengine-token");
	...
	it ("adds an overflowing token to multi input", async () => {
		...
		assert.strictEqual(await mi.$$("kengine-token").length, 5, "should have 5 token");
	...
	it ("adds a token after selection change", async () => {
		...
		await popover.$("kengine-li-suggestion-item").click();
	...
	it ("Placeholder", async () => {
		const mi1 = await browser.$("#empty-mi").shadow$(".kengine-input-inner");
		const mi2 = await browser.$("#mi-with-tokens-customicon").shadow$(".kengine-input-inner");
	...
	it("tests if tokenizer is scrolled to the end when expanded and to start when narrowed", async () => {
		...
		let tokenizerScrollContainerScrollLeft = await browser.execute(() => document.querySelector("#basic-overflow").shadowRoot.querySelector("kengine-tokenizer").shadowRoot.querySelector(".kengine-tokenizer--content").scrollLeft);
		let tokenizerScrollContainerScrollWidth = await browser.execute(() => document.querySelector("#basic-overflow").shadowRoot.querySelector("kengine-tokenizer").shadowRoot.querySelector(".kengine-tokenizer--content").scrollWidth);
		let tokenizerScrollContainerClientWidth = await browser.execute(() => document.querySelector("#basic-overflow").shadowRoot.querySelector("kengine-tokenizer").shadowRoot.querySelector(".kengine-tokenizer--content").getBoundingClientRect().width);
		...
		tokenizerScrollContainerScrollLeft = await browser.execute(() => document.querySelector("#basic-overflow").shadowRoot.querySelector("kengine-tokenizer").shadowRoot.querySelector(".kengine-tokenizer--content").scrollLeft);
	...
	it("should NOT fire token-delete when MI is readonly", async () => {
		...
		const deleteIcon = input.$$("kengine-token")[0].shadow$("kengine-icon");
		...
		const tokens = await input.$$("kengine-token");
	...
	it("should empty the field when value is cleared in the change handler", async () => {
		...
		const valueHelpIcon = mi.shadow$("kengine-icon");
		...
		const listItem = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$("kengine-li-suggestion-item");
	...
	it("Should apply correct text to the tokens overflow indicator", async () => {
		...
		const tokenizerNItems = await miNItems.shadow$("kengine-tokenizer");
		const tokenizerNMore = await miNMore.shadow$("kengine-tokenizer");
...
describe("MultiInput Truncated Token", () => {
	...
	it("should truncate token when single token is in the multinput and open popover on click", async () => {
		...
		const token = await mi.$("kengine-token");
	...
	it("should truncate token when a long token is added", async () => {
		...
		const deleteIcon = await token.shadow$("kengine-icon");

		await deleteIcon.click();

		const tokensCount = (await mi.$$("kengine-token")).length;
...
describe("ARIA attributes", () => {
	it ("aria-describedby value according to the tokens count", async () => {
		...
		const invisibleText = await mi.shadow$(".kengine-hidden-text");
...
describe("Keyboard handling", () => {
	...
	it("left/right arrow navigation", async () => {
		...
		const innerInput =  await input.shadow$("input");
		const firstToken = await browser.$("#basic-overflow kengine-token:first-child");
		const lastToken = await browser.$("#basic-overflow kengine-token:last-child");
	...
	it("should select tokens with key modifiers (Shift + [Ctrl])", async () => {
		const input = await browser.$("#basic-overflow");
		const innerInput = await input.shadow$("input");
		const firstToken = await browser.$("#basic-overflow kengine-token:last-child");
		const secondToken = await browser.$("#basic-overflow kengine-token:nth-child(10)");
		const thirdToken = await browser.$("#basic-overflow kengine-token:nth-child(9)");
	...
	it("should move caret to start of input, when a value is present and home is pressed", async () => {
		...
		const innerInput = await input.shadow$("input");
		const firstToken = await browser.$("#two-tokens kengine-token:first-child");
		let caretPosition;
	...
	it ("Clicking delete icon should delete token and place the focus on the previous one", async () => {
		const input = await browser.$("#two-tokens");
		const innerInput = await input.shadow$("input");
		const secondToken = await browser.$("#two-tokens kengine-token#secondToken");
		const deleteIcon = input.$$("kengine-token")[0].shadow$("kengine-icon");
		let tokens;
		...
		tokens = await input.$$("kengine-token");
	...
	it ("Should focus the input when all tokens are deleted", async () => {
		...
		let tokens = await input.$$("kengine-token");
	...
	it("should delete token on backspace", async () => {
		const input = await browser.$("#two-tokens");
		const innerInput = await input.shadow$("input");
		const firstToken = await browser.$("#two-tokens kengine-token#firstToken");
		const lastToken = await browser.$("#two-tokens kengine-token#secondToken");
		let tokens;
	...
	it("tests if tokenizer is scrolled on keyboard navigation through the tokens", async () => {
		...
		let scrollLeftFirstToken = await browser.execute(() => document.querySelector("#basic-overflow").shadowRoot.querySelector("kengine-tokenizer").shadowRoot.querySelector(".kengine-tokenizer--content").scrollLeft);

		await input.keys('ArrowLeft');
		await input.keys('ArrowLeft');
		await input.keys('ArrowLeft');

		let scrollLeftForthToken = await browser.execute(() => document.querySelector("#basic-overflow").shadowRoot.querySelector("kengine-tokenizer").shadowRoot.querySelector(".kengine-tokenizer--content").scrollLeft);
		...
		let newScrollLeft =  await browser.execute(() => document.querySelector("#basic-overflow").shadowRoot.querySelector("kengine-tokenizer").shadowRoot.querySelector(".kengine-tokenizer--content").scrollLeft);
	...
	it("should open popover with all tokens on keyboard combination ctrl + i", async () => {
		...
		await mi.keys(["Control", "i"]);
		assert.ok(await rpo.getProperty("opened"), "Focused MI - n-more popover should be opened");
		const listItems = await rpo.$("kengine-list").$$("kengine-li");
