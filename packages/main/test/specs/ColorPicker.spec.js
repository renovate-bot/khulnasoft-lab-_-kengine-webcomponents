...
describe("Color Picker general interaction", () => {
	...
	it("tests color picker rendering", async () => {
		const circle = await browser.$("#cp1").shadow$(".kengine-color-picker-circle");
	...
	it("tests setting hex input", async () => {
		...
		const redInput = await colorPicker.shadow$("#red");
		const hexInput = await colorPicker.shadow$(".kengine-color-picker-hex-input");
	...
	it("Alpha value change via the slider", async () => {
		const colorPicker = await browser.$("#cp1");
		const alphaSliderHandle = await colorPicker.shadow$(".kengine-color-picker-alpha-slider").shadow$(".kengine-slider-handle");
	...
	it("Hue value change via the slider", async () => {
		const colorPicker = await browser.$("#cp1");
		const hueSliderHandle = await colorPicker.shadow$(".kengine-color-picker-hue-slider").shadow$(".kengine-slider-handle");
		const stepInput = await browser.$("#changeEventCounter");
	...
	it("Hue value remains unchanged when user presses over the main color section", async () => {
		const colorPicker = await browser.$("#change-event");
		const hexInput = await colorPicker.shadow$(".kengine-color-picker-hex-input");
		const mainColorSection = await colorPicker.shadow$(".kengine-color-picker-main-color");
	...
	it("Inner input components accessibility references", async () => {
		const colorPicker = await browser.$("#cp1");
		const hueSlider = await colorPicker.shadow$(".kengine-color-picker-hue-slider");
		const alphaSlider = await colorPicker.shadow$(".kengine-color-picker-alpha-slider");
		const hexInput = await colorPicker.shadow$(".kengine-color-picker-hex-input");
		const redInput = await colorPicker.shadow$("#red");
