...
async function getItems(selector) {
    const listItems = await browser.$$(`${selector} [kengine-tree-item]`);
...
describe("Tree general interaction", () => {
	...
	it("Tree is rendered", async () => {
		const treeRoot = await browser.$("#tree").shadow$("kengine-tree-list");
	...
	it("Tree items can be collapsed", async () => {
		...
		const toggleButton = await browser.$(">>>#tree kengine-tree-item[expanded] kengine-icon.kengine-li-tree-toggle-icon");
	...
	it("Tree items can be expanded", async () => {
		const listItemsBefore = await getItemsCount("#tree");
		const toggleButton = await browser.$(">>>#tree kengine-tree-item kengine-icon.kengine-li-tree-toggle-icon");
...
describe("Tree proxies properties to list", () => {
	...
	it("Mouseover/mouseout events", async () => {
		const treeItems = await browser.$$(">>>#tree kengine-tree-item .kengine-li-root-tree");
	...
	it("Mode works", async () => {
		...
		const list = await tree.shadow$("kengine-tree-list");
	...
	it("Tests the prevention of the kengine-itemClick event", async () => {
		const treeItems = await browser.$$("#preventable-click-event kengine-tree-item");
	...
	it("selectionChange event provides targetItem parameter", async () => {
		...
		const listItems = await browser.$$("#treeIndeterminate kengine-tree-item");
...
describe("Tree has screen reader support", () => {
	...
	it("List item acc attributes correct", async () => {
		const listItems = await browser.$$("#tree kengine-tree-item");
	...
	it ("Tree list item receives aria-labelledby from the accessibleName property", async () => {
		const listTreeItem = await browser.$("#tree kengine-tree-item");
