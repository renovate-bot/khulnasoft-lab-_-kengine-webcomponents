...
describe("List Tests", () => {
	...
	it("List is rendered", async () => {
		const list = await browser.$("#infiniteScrollEx").shadow$(".kengine-list-root");
		const busyInd = await browser.$("#infiniteScrollEx").shadow$(".kengine-list-busy-row");
	...
	it("itemClick and selectionChange events are fired in Single selection", async () => {
		...
		const secondItemRadio = await browser.$("#listEvents #country2").shadow$("kengine-radio-button");
	...
	it("No data text is shown", async () => {
		const noDataText = await browser.$("#no-data-list").shadow$(".kengine-list-nodata-text");
	...
	it("Tests header text", async () => {
		...
		assert.ok(await header.hasClass("kengine-list-header"), "header has the right classes");
	...
	it("Test default slot", async () => {
		const listItemsLength = await browser.executeAsync(done => {
			...
			const result = slots.filter(slot => {
				return slot.tagName === "KENGINE-LI";
			});
	...
	it("mode: delete. items have X buttons which delete them", async () => {
		...
		const itemDeleteButton = await firstItem.shadow$('kengine-button');
	...
	it("item size and classed, when an item has both text and description", async () => {
		...
		const firstItem = await browser.$("#listWithDesc kengine-li:first-child");
	...
	it("keyboard handling on SHIFT + TAB", async () => {
		...
		const firstItem = await browser.$("kengine-li.firstItem");
		...
		// act: Shift + Tab from the growing button - the focus should leave the kengine-list
	...
	it("keyboard handling on TAB", async () => {
		...
		const afterBtn = await browser.$("#afterBtn");
		const beforeBtn = await browser.$("#beforeBtn");
		const item = await browser.$("kengine-li-custom.item");
		const itemBtn = await browser.$("kengine-button.itemBtn");
		const itemLink = await browser.$("kengine-link.itemLink");
		const itemRadioBtn = await browser.$("kengine-radio-button.itemRadio");
		...
		// act: TAB from the "Click me" button - the the focus should go to "KENGINE Link" anchor
		await itemBtn.keys("Tab");
		assert.ok(await itemLink.isFocused(), "the 2nd tabbable element (link) is focused");

		// act: TAB from the "KENGINE Link" anchor - the the focus should skip the "Disabled" button
		...
		// act: TAB from the growing button - the focus should leave the kengine-list
	...
	it("keyboard handling on TAB when 2 level nested KENGINEElement is focused", async () => {
		const list = await browser.$("#focusAfterList");
		const breadcrumbsItem = await list.$(".breadcrumbsItem");
		const breadcrumb = await list.$("kengine-breadcrumbs");
	...
	it("detailPress event is fired", async () => {
		...
		const detailButton = await firstItem.shadow$(".kengine-li-detailbtn");
	...
	it("tests aria-labelledby", async () => {
		...
		const ulInternalHeader = await listWithInternalHeader.shadow$(".kengine-list-ul");
		const ulCustomHeader = await listWithCustomHeader.shadow$(".kengine-list-ul");
	...
	it("tests events for kengine-li-custom", async () => {
	...
	it("tests the prevention of the kengine-itemClick event", async () => {
	...
	it('focusable list-items are correctly disabled', async () => {
		const item2 = await browser.$('#basicList kengine-li:nth-child(2)');

		// focus the second item
		await item2.click();

		// disable the second item
		await browser.$("#basicList kengine-li:nth-child(2)").setProperty("disabled", true);

		assert.strictEqual(await item2.shadow$('li').getProperty("tabIndex"), -1, "disabled item is no longer focusable");
		assert.strictEqual(await item2.shadow$('li').getAttribute("class"), "kengine-li-root", "disabled item no longer styled as focusable");
	});

	it('disabled list-items are skipped on navigation', async () => {
		const item1 = await browser.$('#basicList kengine-li:nth-child(1)'),
			item3 = await browser.$('#basicList kengine-li:nth-child(3)');
	...
	it('group headers should not be with role options', async () => {
		const groupHeader = await browser.$("#listSelectedItem #group-header").shadow$(".kengine-ghli-root");
	...
	it('anchor tabs should be accessible within list items', async () => {
		...
		assert.strictEqual(url, "https://sap.github.io/kengine-webcomponents/playground/components", "Link target is accessible");
