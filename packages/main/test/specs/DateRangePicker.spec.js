...
describe("DateRangePicker general interaction", () => {
	...
	it("Custom Validation Error", async () => {
		...
		assert.strictEqual(await daterangepicker.shadow$("kengine-input").getProperty("valueState"), "Error", "The value state is on error");
	});
	...
	it("Custom Validation None", async () => {
		...
		assert.strictEqual(await daterangepicker.shadow$("kengine-input").getProperty("valueState"), "None", "The value state is on none");
	});
	...
	it("Selected dates are updated after value update in the input field", async () => {
		...
		const dayPicker = await browser.$(`.${staticAreaItemClassName}`).shadow$(`kengine-calendar`).shadow$(`kengine-daypicker`);
	...
	it("Change event fired once", async () => {
		...
		const dayPicker = await browser.$(`.${staticAreaItemClassName}`).shadow$(`kengine-calendar`).shadow$(`kengine-daypicker`);
		const dayOne = await dayPicker.shadow$(`.kengine-dp-root`).$(".kengine-dp-content").$$("div > .kengine-dp-item" )[5];
		const dayTwo = await dayPicker.shadow$(`.kengine-dp-root`).$(".kengine-dp-content").$$("div > .kengine-dp-item" )[15];
	...
	it("Month is not changed in multiselect mode", async () => {
		...
		const staticAreaItemClassName = await browser.getStaticAreaItemClassName("#daterange-picker1");
		const daterangepicker = await browser.$("#daterange-picker1");
		const calendarHeader = await browser.$(`.${staticAreaItemClassName}`).shadow$(`kengine-calendar`).shadow$(`kengine-calendar-header`);
		const dayPicker = await browser.$(`.${staticAreaItemClassName}`).shadow$(`kengine-calendar`).shadow$(`kengine-daypicker`);
		const dayOne = await dayPicker.shadow$(`.kengine-dp-root`).$(".kengine-dp-content").$$("div > .kengine-dp-item" )[15];
	...
	it("Min and max dates are set without format-pattern by using ISO (YYYY-MM-dd) format", async () => {
		...
		const dateRangePickerInput = await daterangepicker.shadow$("kengine-input");
