...
const getPicker = async id => {
	...
	return browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover");
};
...
const getTimeSlidersCount = async id => {
	...
	return await browser.executeAsync( (picker, done) => {
		done(picker.querySelector("kengine-time-selection").shadowRoot.querySelectorAll("kengine-wheelslider").length);
	}, picker);
};
...
describe("DateTimePicker general interaction", () => {
	...
	it("tests selection of new date/time", async () => {
		...
		// assert
		const currentValue = await dtPicker.shadow$("kengine-input").getValue();
		...
		const selectedDay = await picker.$("kengine-calendar").shadow$("kengine-daypicker").shadow$(".kengine-dp-item--selected");
		await selectedDay.click();
		await browser.keys("ArrowRight");
		await browser.keys("Space");

		// select new time
		await picker.$("kengine-time-selection").shadow$(`kengine-wheelslider[data-sap-slider="hours"]`).shadow$(`div[tabindex="0"]`).click();
		await browser.keys("PageDown"); // select 01

		await picker.$("kengine-time-selection").shadow$(`kengine-wheelslider[data-sap-slider="minutes"]`).shadow$(`div[tabindex="0"]`).click();
		await browser.keys("PageDown"); // select 0
		await browser.keys("ArrowDown"); await browser.keys("ArrowDown"); // select 02

		await picker.$("kengine-time-selection").shadow$(`kengine-wheelslider[data-sap-slider="seconds"]`).shadow$(`div[tabindex="0"]`).click();
		await browser.keys("PageDown"); // select 0
		...
		// assert
		const newValue = await dtPicker.shadow$("kengine-input").getValue();
	...
	it("tests selection of new date without changing the time section", async () => {
		...
		const picker = await getPicker("dtSeconds");
		// select the next day (the right from the selected)
		const selectedDay = await picker.$("kengine-calendar").shadow$("kengine-daypicker").shadow$(".kengine-dp-item--now");
	...
	it("tests change event is fired on submit", async () => {
		...
		// act
		await picker.$("kengine-calendar").shadow$("kengine-daypicker").shadow$("[data-sap-focus-ref]").click(); // select a date to enable the OK button
	...
	it("tests hours slider is expanded", async () => {
		...
		const expanded = await picker.$("kengine-time-selection").shadow$(`kengine-wheelslider[data-sap-slider="hours"]`).getProperty("expanded");
	...
	it("tests selection of 12:00:00 AM", async () => {
		...
		await picker.$("kengine-time-selection").shadow$(`kengine-wheelslider[data-sap-slider="periods"]`).shadow$(`div[tabindex="0"]`).click();
	...
	it("Min and max dates are set, with no format pattern provided, using valid ISO format", async () => {
		...
		const prevButton = await picker.$("kengine-calendar").shadow$("kengine-calendar-header").shadow$("div[data-kengine-cal-header-btn-prev]");
		const nextButton = await picker.$("kengine-calendar").shadow$("kengine-calendar-header").shadow$("div[data-kengine-cal-header-btn-next]");

		// assert
		assert.strictEqual(await prevButton.hasClass("kengine-calheader-arrowbtn-disabled"), true, "The previous button is disabled.");
		assert.strictEqual(await nextButton.hasClass("kengine-calheader-arrowbtn-disabled"), true, "The next button is disabled.");
	});
