...
describe("Icon general interaction", () => {
	...
	it("Tests icon rendering", async () => {
		const iconRoot = await browser.$("#interactive-icon").shadow$("kengine-icon-root");
	...
	it("Tests events 'click' and 'kengine-click' events", async () => {
		...
		// - noConflict: true - the icon fires "click" and "kengine-click" (custom events)

		// Interactive icon
		const interactiveIcon = await browser.$("#interactive-icon");
		const inpClickRes = await browser.$("#click-event");
		const inpKENGINEClickRes = await browser.$("#kengine-click-event");

		await interactiveIcon.click();
		assert.strictEqual(await inpClickRes.getAttribute("value"), "1", "The 'click' event is fired.");
		assert.strictEqual(await inpKENGINEClickRes.getAttribute("value"), "0", "The 'kengine-click' event is not fired on mouse click.");

		await interactiveIcon.keys("Enter");
		assert.strictEqual(await inpClickRes.getAttribute("value"), "2", "Enter fires 'click'");
		assert.strictEqual(await inpKENGINEClickRes.getAttribute("value"), "1", "Enter fires 'kengine-click'");

		await interactiveIcon.keys("Space");
		assert.strictEqual(await inpClickRes.getAttribute("value"), "3", "Space fires 'click'");
		assert.strictEqual(await inpKENGINEClickRes.getAttribute("value"), "2", "Space fires 'kengine-click'");

		// Non-interactive icon
		const nonInteractiveIcon = await browser.$("#non-interactive-icon");
		const inpClickRes2 = await browser.$("#click-event-2");
		const inpKENGINEClickRes2 = await browser.$("#kengine-click-event-2");

		await nonInteractiveIcon.click();
		assert.strictEqual(await inpClickRes2.getAttribute("value"), "1", "The 'click' event is fired.");
		assert.strictEqual(await inpKENGINEClickRes2.getAttribute("value"), "0", "The 'kengine-click' event is not fired on mouse click..");
	});

	it("Tests the accessibility attributes", async () => {
		const iconRoot = await browser.$("#myIcon").shadow$(".kengine-icon-root");
		const accRoleIconRoot = await browser.$("#accRoleIcon").shadow$(".kengine-icon-root");
		const ariaHiddenIconRoot = await browser.$("#araHiddenIcon").shadow$(".kengine-icon-root");
	...
	it("Tests switch to sap_horizon", async () => {
		...
		const iconPath = await browser.$("#myIcon").shadow$(".kengine-icon-root path");
		...
		const iconPathAfter = await browser.$("#myIcon").shadow$(".kengine-icon-root path");
	...
	it("Tests icon modules' exported values", async () => {
		const expectedExportedValues = "accept|KHULNASOFT-icons-v4/accept|KHULNASOFT-icons-v5/accept|tnt/actor|tnt-v2/actor|tnt-v3/actor|business-suite/3d|business-suite-v1/3d|business-suite-v2/3d";
	...
	it("Icon svg aria-label cleaned after name change", async () => {
		...
		let iconSVG = await browser.$("#iconError").shadow$(".kengine-icon-root");
		...
		iconSVG = await browser.$("#iconError").shadow$(".kengine-icon-root");
