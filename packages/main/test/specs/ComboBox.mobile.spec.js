...
describe("Basic mobile picker rendering and interaction", () => {
	...
	it("Should render properly the mobile picker", async () => {
		...
		const dialogInput = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$(".kengine-input-inner-phone");
...
describe("Eventing", () => {
	...
	it("Should fire change event with correct parameters on item press", async () => {
		...
		const suggestionItem = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$$("kengine-li")[1];
	...
	it("Should fire input event with correct parameters when typing in internal input", async () => {
		...
		await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$(".kengine-responsive-popover-close-btn").click();
	});
	...
	it("Should not fire change event when pressing the picker's Close button", async () => {
		...
		const dialogInput = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$(".kengine-input-inner-phone");
		const closeButton = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$(".kengine-responsive-popover-close-btn");
	...
	it ("When select an item, then open the dialog again and delete the text, then press OK button, the value should be deleted.", async ()=> {
		...
		const resPopover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover");
		const dialogInput = await resPopover.$(".kengine-input-inner-phone");
		const okBtn = await resPopover.$(".kengine-responsive-popover-footer").$("kengine-button");
...
describe("Picker filtering", () => {
	...
	it("Should filter items", async () => {
		...
		const dialogInput = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$(".kengine-input-inner-phone");
		const dialogList = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$('kengine-list')

		assert.strictEqual(await dialogList.$$('kengine-li').length, 8, "All of the items are shown (8)");
		await dialogInput.keys("B");
		assert.strictEqual(await dialogList.$$('kengine-li').length, 3, "There are 3 filtered items");
	});
	...
	it("Should filter group header list items", async () => {
		...
		assert.strictEqual(await dialogList.$$('kengine-li-groupheader').length, 3, "All of the group header list items are shown (3)");
		await dialogInput.keys("B");
		assert.strictEqual(await dialogList.$$('kengine-li-groupheader').length, 1, "There is only 1 visible group header");
	});
});
