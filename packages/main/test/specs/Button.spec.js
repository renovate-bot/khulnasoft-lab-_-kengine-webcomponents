...
describe("Button general interaction", () => {
	...
	it("tests button's text rendering", async () => {
		const slotsLength = (await browser.$("#button1").shadow$$(".kengine-button-text>bdi>slot")).length;
	...
	it("tests button's icon rendering", async () => {
		...
		assert.ok(await button.shadow$("kengine-icon").isExisting(), "icon is present");

		await button.setAttribute("icon", "");
		assert.notOk(await button.shadow$("kengine-icon").isExisting(),"icon is not present");
	});
	...
	it("tests clicking on disabled button whith Icon", async () => {
		...
		const buttonIcon = await button.shadow$("[kengine-icon]");
	...
	it("tests button with text icon role", async () => {
		...
		const icon = await button.shadow$("kengine-icon");
