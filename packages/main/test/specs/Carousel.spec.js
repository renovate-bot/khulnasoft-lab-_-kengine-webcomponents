...
describe("Carousel general interaction", () => {
	...
	it("rendering", async () => {
		const carouselRoot = await browser.$("#carousel1").shadow$(".kengine-carousel-root");
		const carousel = await browser.$("#carousel1");
		const carouselItem1 = await carousel.shadow$(".kengine-carousel-item:nth-child(1)");
		const carouselItem2 = await carousel.shadow$(".kengine-carousel-item:nth-child(2)");

		assert.ok(await carouselRoot.isExisting(), "Carousel is rendered.");

		assert.notOk(await carouselItem1.hasClass("kengine-carousel-item--hidden"), "kengine-carousel-item--hidden class is not set");
		assert.ok(await carouselItem2.hasClass("kengine-carousel-item--hidden"), "kengine-carousel-item--hidden class is set");
	});
	...
	it("Carousel navigates left", async () => {
		...
		const carouselRightButton = await carousel.shadow$$(".kengine-carousel-navigation-button")[0];
	...
	it("Carousel navigates right", async () => {
		...
		await carousel.scrollIntoView();
		await carousel.moveTo();
		const carouselLeftButton = await carousel.shadow$$(".kengine-carousel-navigation-button")[1];
	...
	it("Navigation is rendered for carousel with less than 9 elements", async () => {
		...
		const navigation = await carousel.shadow$(".kengine-carousel-navigation > div");
	...
	it("Navigation is rendered for carousel with more than 9 elements", async () => {
		...
		await carousel.scrollIntoView();
		await carousel.moveTo();
		const navigation = await carousel.shadow$(".kengine-carousel-navigation > .kengine-carousel-navigation-text");
	...
	it("Buttons are rendered in the content only when hovering (arrows-placement)", async () => {
		...
		const carouselNextButton = await carousel.shadow$(".kengine-carousel-navigation-button[arrow-forward]");
		await carouselNextButton.click();
		await carousel.moveTo();

		const buttons = await carousel.shadow$$(".kengine-carousel-navigation-arrows .kengine-carousel-navigation-button:not(.kengine-carousel-navigation-button--hidden)");
	...
	it("Buttons are rendered in the navigation without hovering (arrows-placement)", async () => {
		...
		await carousel.scrollIntoView();
		const buttons = await carousel.shadow$$(".kengine-carousel-navigation-wrapper .kengine-carousel-navigation-button:not(.kengine-carousel-navigation-button--hidden)");
	...
	it("Aria attributes are set", async () => {
		...
		await carousel.scrollIntoView();
		await carousel.moveTo();
		const pageIndicatorDot1 = await browser.$('#carousel5').shadow$(".kengine-carousel-navigation-dot:first-child");
		const pageIndicatorDot2 = await carousel.shadow$(".kengine-carousel-navigation-dot:nth-child(2)");
		...
		const carouselItem3 = await carousel.shadow$(".kengine-carousel-item:nth-child(3)");
		const carouselItem4 = await carousel.shadow$(".kengine-carousel-item:nth-child(4)");
		...
		const carouselRoot = await carousel.shadow$(".kengine-carousel-root");
		...
		await carousel.shadow$(".kengine-carousel-navigation-button:nth-child(2)").click();
	...
	it("all visible elements in the current page have correct tabindex values", async () => {
		...
		const visibleItems = [
			await carousel.shadow$(".kengine-carousel-item:nth-child(1) slot"),
			await carousel.shadow$(".kengine-carousel-item:nth-child(2) slot"),
			await carousel.shadow$(".kengine-carousel-item:nth-child(3) slot"),
		];
	...
	it("Arrows and Dots not displayed in case of single page", async () => {
		...
		const pages = await carousel.getProperty("pagesCount");
		const pageIndicator = await carousel.shadow$(".kengine-carousel-navigation-wrapper");
		const navigationArrows = await carousel.shadow$(".kengine-carousel-navigation-arrows");
	...
	it("Event navigate fired when pressing navigation arrows", async () => {
		...
		const navigationArrowForward = await carousel.shadow$("kengine-button[arrow-forward]");
		const navigationArrowsBack = await carousel.shadow$("kengine-button[arrow-back]");
	...
	it("page-indicator-style property", async () => {
		...
		assert.strictEqual(await carousel.shadow$(".kengine-carousel-navigation .kengine-carousel-navigation-text").getText(), "1 of 2", "carousel is showing numeric page indicator");
	});
	...
	it("hide-page-indicator property", async () => {
		...
		assert.strictEqual(await carousel.shadow$$(".kengine-carousel-navigation > *").length, 0, "carousel has not rendered a page indicator");
	});
	...
	it("F7 keyboard navigation", async () => {
		...
		assert.ok(await browser.$(innerFocusedElement).hasClass("kengine-carousel-root"), "Carousel is focused");

		await browser.keys("F7");

		innerFocusedElement = innerFocusedElement = await browser.custom$("activeElement", "#carouselF7Button");

		assert.ok(await browser.$(innerFocusedElement).hasClass("kengine-button-root"), "Button is focused");
		...
		assert.ok(await browser.$(innerFocusedElement).hasClass("kengine-input-inner"), "Input is focused");
