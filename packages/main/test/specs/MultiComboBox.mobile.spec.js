...
describe("Basic interaction", () => {
	...
	it("Should render properly the mobile picker", async () => {
		...
		const dialogInput = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$("input");
		assert.ok(await dialogInput.isDisplayed(), "Input is displayed");

		const toggleSelectedButton =  await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$("kengine-toggle-button");
		assert.ok(await toggleSelectedButton.isDisplayed(), "Toggle selected items button is displayed");

		const dialogCloseButton = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$(".kengine-responsive-popover-close-btn");
		assert.ok(await dialogCloseButton.isDisplayed(), "Close icon is displayed");

		const dialogOkButton = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$(".kengine-responsive-popover-footer").$("kengine-button");
	...
	it("Should open the picker with preselected items only when n-more is clicked", async () => {
		...
		await multiCombo.scrollIntoView();
		await multiCombo.shadow$('kengine-tokenizer').shadow$(".kengine-tokenizer-more-text").click();

		const toggleSelectedButton =  await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$("kengine-toggle-button");
		assert.strictEqual(await toggleSelectedButton.getAttribute("pressed"), "", "Toggle selected items button is pressed");

		const itemsCount = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$("kengine-list").$$("kengine-li")
...
describe("Typeahead", () => {
	...
	it("Should autocomplete the first matched suggestion item", async () => {
		...
		const mcbInput = await mcb.shadow$("#kengine-multi-combobox-input");
		const sExpected = "Cosy";
		const staticAreaItemClassName = await browser.getStaticAreaItemClassName("#mcb")

		await mcbInput.click();

		const dialogInput = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$(".input-root-phone input");
...
describe("Items selection", () => {
	...
	it("Should close the picker and create token when item is pressed in the picker", async () => {
		const multiCombo = await browser.$("#mcb");
		const mcbInput = await multiCombo.shadow$("#kengine-multi-combobox-input");
		const staticAreaItemClassName = await browser.getStaticAreaItemClassName("#mcb");

		await multiCombo.scrollIntoView();
		await mcbInput.click();

		const listItem = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$$("kengine-li")[0];
		await listItem.click();

		const token = await multiCombo.shadow$("kengine-tokenizer").$("kengine-token");
	...
	it("Should create token when item is selected in the picker and ok button is pressed", async () => {
		...
		const listItemCheckbox = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$$("kengine-li")[1].shadow$("kengine-checkbox");
		await listItemCheckbox.click();

		const dialogOkButton = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$(".kengine-responsive-popover-footer").$("kengine-button");
		await dialogOkButton.click();

		const tokens = await multiCombo.shadow$("kengine-tokenizer").$$("kengine-token");
	...
	it("Should not create token when item is selected in the picker and the 'Close' button is pressed", async () => {
		...
		const listItemCheckbox = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$$("kengine-li")[2].shadow$("kengine-checkbox");
		await listItemCheckbox.click();
	...
	it("Should not allow deselection when readonly", async () => {
		...
		const listItemCheckbox = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$$("kengine-li")[0].shadow$("kengine-checkbox");
		await listItemCheckbox.click();

		const listItem = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$$("kengine-li")[1];
		await listItem.click();
...
describe("Value state header", () => {
	...
	it("Should show value state header inside mobile dialog", async () => {
		...
		await multiCombo.scrollIntoView();
		await multiCombo.shadow$("kengine-icon").click();

		const dialogStateHeader = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$(".kengine-valuestatemessage-header");
...
describe("Eventing", () => {
	...
	it("Should prevent selection-change when item is selected in the picker and ok button is pressed", async () => {
		...
		let tokens = await multiCombo.shadow$("kengine-tokenizer").$$("kengine-token");
		...
		tokens = await multiCombo.shadow$("kengine-tokenizer").$$("kengine-token");
		assert.strictEqual(tokens.length, 1, "There should be only one token.");
	});
});
