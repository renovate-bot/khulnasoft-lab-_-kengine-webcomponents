...
describe("Attributes propagation", () => {
	...
	it("Popover arrow", async () => {
		...
		assert.ok(await popover.shadow$(".kengine-popover-arrow").isDisplayedInViewport(), "Initially popover has arrow.");
		...
		assert.notOk(await popover.shadow$(".kengine-popover-arrow").isDisplayedInViewport(), "The arrow was hidden.");
	});
...
describe("Popover general interaction", () => {
	...
	it("tests popover toggling", async () => {
		...
		const popover = await browser.$("kengine-popover");
	...
	it("tests if overflown content can be reached by scrolling 1", async () => {
		...
		const items = await staticAreaItem.shadow$$("kengine-li");
	...
	it("tests if overflown content can be reached by scrolling (with header and arrow) 1", async () => {
		...
		const items = await bigPopover.$$("kengine-li");
	...
	it("tests modal popover with no block layer", async () => {
		...
		const blockLayerIsCreated = await browser.executeAsync((popoverId, done) => {
			const staticAreaItems = document.querySelectorAll("kengine-static-area-item");
			let result = false;

			staticAreaItems.forEach(item => {
				if (item.shadowRoot.querySelector(".kengine-block-layer") && item.classList.contains(popoverId)) {
					result = true;
				}
			});
...
describe("Acc", () => {
	...
	it("tests aria-labelledby and aria-label", async () => {
		...
		assert.ok(await popover.shadow$(".kengine-popup-root").getAttribute("aria-labelledby"), "Popover has aria-labelledby.");
		assert.notOk(await popover.shadow$(".kengine-popup-root").getAttribute("aria-label"), "Popover does not have aria-label.");

		await popover.setAttribute("accessible-name", "text");
		assert.notOk(await popover.shadow$(".kengine-popup-root").getAttribute("aria-labelledby"), "Popover does not have aria-labelledby.");
		assert.ok(await popover.shadow$(".kengine-popup-root").getAttribute("aria-label"), "Popover has aria-label.");
	});
	...
	it("tests accessible-name-ref", async () => {
		...
		assert.strictEqual(await popover.shadow$(".kengine-popup-root").getAttribute("aria-label"), expectedText, "aria-label should be the text of the label.");
	});
	...
	it("tests that aria-labelledby is not set when there is no header and no accessible-name-ref", async () => {
		...
		assert.isNull(await popoverWithoutHeader.shadow$(".kengine-popup-root").getAttribute("aria-labelledby"), "Popover should NOT have aria-labelledby set.");
	});
});
...
describe("Horizontal Alignment", () => {
	...
	it("Center", async () => {
		await browser.$("[kengine-radio-button][name='horizontalAlign'][text='Center']").click();
	...
	it("Left", async () => {
		await browser.$("[kengine-radio-button][name='horizontalAlign'][text='Left']").click();
		await browser.$("#horizontalAlignBtn").click();
		const popover = await browser.$("#popoverHorizontalAlign");
		const opener = await browser.$("#targetOpener");
	...
	it("Right", async () => {
		await browser.$("[kengine-radio-button][name='horizontalAlign'][text='Right']").click();
		await browser.$("#horizontalAlignBtn").click();
		const popover = await browser.$("#popoverHorizontalAlign");
		const opener = await browser.$("#targetOpener");
