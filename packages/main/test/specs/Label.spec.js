...
describe("General API", () => {
	...
	it("tests initial rendering", async () => {
		const labelRoot = await browser.$("#basic-label").shadow$(".kengine-label-root");

		assert.ok(labelRoot, "Label is rendered");
	});

	it("changes text of kengine-label", async () => {
	...
	it("should show required star", async () => {
		const requiredLabelContent = await browser.execute(`
			return window.getComputedStyle(document.querySelector('#required-label').shadowRoot.querySelector(".kengine-label-required-colon"), ':after').content;
		`);
	...
	it("tests show-colon does not force truncation", async () => {
		const labelWithNoColon = await browser.$("#showColon-false").shadow$(".kengine-label-text-wrapper");
		const labelWithShowColon = await browser.$("#showColon-true").shadow$(".kengine-label-text-wrapper");

		const labelWithNoColonSize = await labelWithNoColon.getSize();
		const labelWithShowColonSize = await labelWithShowColon.getSize();

		// Comparing kengine-label(s) "Basic Label" and "Basic Label:", but just the "Basic Label" part,
	...
	it("colon symbol should be taken from the i18n bundle", async () => {
		...
		actualSymbol = await browser.executeAsync(done => {
			...
			done(window.getComputedStyle(label.shadowRoot.querySelector(".kengine-label-required-colon"), "::before").content);
		});
	...
	describe("linked element with 'for' property", async () => {
		it("should focus kengine-input on click", async () => {
			const label = await browser.$("#label-for-kengine-input");
			const field = await browser.$("#form-kengine-input");

			assert.strictEqual(await label.getText(), await field.shadow$("input").getAttribute("aria-label"), "input aria-label is correct");

			await label.click();

			assert.ok(await field.isFocused(), "kengine-input should be focussed");
		});
		...
		it("should focus kengine-textarea on click", async () => {
			const label = await browser.$("#label-for-kengine-textarea");
			const field = await browser.$("#kengine-textarea");

			await label.click();

			assert.ok(await field.isFocused(), "kengine-textarea should be focussed");
		});
		...
		it("should focus kengine-date-picker on click", async () => {
			const label = await browser.$("#label-for-kengine-datepicker");
			const field = await browser.$("#kengine-datepicker");

			await label.click();

			assert.ok(await field.isFocused(), "kengine-datepicker should be focussed");
		});

		it("should focus within a shadow root", async () => {
			const customComponent = await browser.$("#custom-element-with-label");
			const label = await customComponent.shadow$('kengine-label[for="input"]');
