...
describe("Clock API", () => {
	...
	it("'disabled' property", async () => {
		...
		const enabled = await hours12.shadow$(".kengine-tp-clock-cover");
		const disabled = await hours12Disabled.shadow$(".kengine-tp-clock-cover");
	...
	it("'active' property", async () => {
		...
		const activeCSS = await minutesActive.shadow$(".kengine-tp-clock").getCSSProperty("display");
		const inactiveCSS = await minutesInactive.shadow$(".kengine-tp-clock").getCSSProperty("display");
	...
	it("'showInnerCircle' property", async () => {
		...
		const numbersInHours12 = await hours12.shadow$$(".kengine-tp-clock-item:not([marker]) .kengine-tp-clock-number");
		const numbersInHours24 = await hours24.shadow$$(".kengine-tp-clock-item:not([marker]) .kengine-tp-clock-number");
	...
	it("'displayStep' and 'valueStep' properties", async () => {
		...
		const minutes10 = await browser.$("#myMinutes10");
		const numbersInHours12 = await hours12.shadow$$(".kengine-tp-clock-item:not([marker]) .kengine-tp-clock-number");
		const numbersInMinutes = await minutes.shadow$$(".kengine-tp-clock-item:not([marker]) .kengine-tp-clock-number");
		const numbersInMinutes10 = await minutes10.shadow$$(".kengine-tp-clock-item:not([marker]) .kengine-tp-clock-number");
...
describe("Clock item selection", () => {
	it("select clock item and 'change' event", async () => {
		const hours12 = await browser.$("#myHours12");
		const hours24 = await browser.$("#myHours24");
		const hours12Cover = await hours12.shadow$(".kengine-tp-clock-cover");
		const hours24Cover = await hours24.shadow$(".kengine-tp-clock-cover");
