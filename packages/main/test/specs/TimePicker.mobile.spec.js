...
describe("TimePicker on phone - general interactions", () => {
	...
	it("opening of popover with numeric inputs", async () => {
		...
		const timePickerPopover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-popover");
		const timeSelectionInputs = await timePickerPopover.$('div.popover-content').$('kengine-time-selection-inputs');
		const components = await timeSelectionInputs.shadow$$('kengine-input');
		...
		await timePicker.shadow$("kengine-input").click();
	...
	it("value change with numeric inputs on OK and Cancel button press", async () => {
		...
		const timePickerPopover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-popover");
		const timePickerPopoverButtons = await timePickerPopover.$('div.kengine-time-picker-footer').$$("kengine-button");
		const timeSelectionInputs = await timePickerPopover.$('div.popover-content').$('kengine-time-selection-inputs');
		const components = await timeSelectionInputs.shadow$$('kengine-input');
		const hoursInnerInput = await components[0].shadow$("input");
		const minutesInnerInput = await components[1].shadow$("input");
		const secondsInnerInput = await components[2].shadow$("input");
		const amPmButton = await timeSelectionInputs.shadow$('kengine-segmented-button');
		const amPmButtonItems = await amPmButton.$$("kengine-segmented-button-item");
