...
describe("Ignore Custom Elements", () => {
	...
	it("Tests ignore custom elements", async () => {
		const result = await browser.executeAsync(done => {
			...
			res.ignoreKENGINE = bundle.shouldIgnoreCustomElement("kengine-card-header"); // false
			done(res);
		});
		...
		assert.notOk(result.ignoreKENGINE, "The kengine-card-header tag is not ignored");
	});
});