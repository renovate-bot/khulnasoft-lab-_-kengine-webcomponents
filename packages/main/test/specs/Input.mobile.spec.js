...
describe("Eventing", () => {
	...
	it("Should fire events with correct parameters on item press", async () => {
		...
		const suggestionItem = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$$("kengine-li-suggestion-item")[2];
...
describe("Picker filtering", () => {
	...
	it("Should filter items", async () => {
		...
		assert.strictEqual(await dialogList.$$('kengine-li-suggestion-item').length, 0, "There are no filtered items initially");
		await dialogInput.keys("B");
		assert.strictEqual(await dialogList.$$('kengine-li-suggestion-item').length, 4, "There are 4 filtered items");
	});
	...
	it("Should filter group header list items", async () => {
		...
		assert.strictEqual(await dialogList.$$('kengine-li-groupheader').length, 1, "There is 1 filtered group header");
	});
});
