...
describe("Toolbar general interaction", () => {
	...
	it("Should render the button with the correct text", async () => {
		...
		const button = await toolbar.shadow$("kengine-button")
	...
	it ("Should render the button with the correct accessible name", async () => {
		const toolbar = await browser.$("kengine-toolbar");
		const buttonAccName = await toolbar.shadow$("kengine-button[accessible-name]");
	...
	it("Should render the button with the correct accessibilityAttributes", async () => {
		const toolbar = await browser.$("kengine-toolbar");
		const buttonAccAttributes = await toolbar.shadow$("kengine-button[accessible-name]").getProperty("accessibilityAttributes");
	...
	it("Should render the button with the correct text inside the popover", async () => {
		...
		const buttonText = await popover.$("kengine-button").getText();
		const buttonDesign = await popover.$("kengine-button").getAttribute("design");
		const buttonDisabled = await popover.$("kengine-button").getAttribute("disabled");
		const buttonIcon = await popover.$("kengine-button").getAttribute("icon");
		const buttonIconEnd = await popover.$("kengine-button").getAttribute("icon-end");
		const buttonTooltip = await popover.$("kengine-button").getAttribute("tooltip");
	...
	it ("Should render the button with the correct accessible name inside the popover", async () => {
		...
		const staticAreaItemClassName = await browser.getStaticAreaItemClassName("#otb_d");
		const popover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-popover");
		const buttonAccName = await popover.$("kengine-button[accessible-name]");
		const buttonAccessibleName = await buttonAccName.getAttribute("accessible-name");
		const buttonAccessibleNameRef = await buttonAccName.getAttribute("accessible-name-ref");
	...
	it("Should render the button with the correct accessibilityAttributes inside the popover", async () => {
		...
		const staticAreaItemClassName = await browser.getStaticAreaItemClassName("#otb_d");
		const popover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-popover");
		const buttonAccAttributes = await popover.$("kengine-button[accessible-name]").getProperty("accessibilityAttributes");
