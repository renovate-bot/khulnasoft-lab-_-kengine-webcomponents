...
describe("Calendar general interaction", () => {
	...
	it("Calendar is rendered", async () => {
		const calendar = await browser.$("#calendar1").shadow$(".kengine-cal-root");

		assert.ok(calendar, "Calendar is rendered");
	});

	it("Year is set in the header", async () => {
		const calendarHeader = await browser.$("#calendar1").shadow$("kengine-calendar-header");
		const yearButton = await calendarHeader.shadow$(`[data-kengine-cal-header-btn-year]`);
	...
	it("Month is set in the header", async () => {
		...
		const calendarHeader = await browser.$("#calendar1").shadow$("kengine-calendar-header");
		const monthButton = await calendarHeader.shadow$(`[data-kengine-cal-header-btn-month]`);
	...
	it("Focus goes into the current day item of the day picker", async () => {
		...
		const dayPicker = await calendar.shadow$("kengine-daypicker");
		const header = await calendar.shadow$("kengine-calendar-header");
		const currentDayItem = await dayPicker.shadow$(`div[data-sap-timestamp="974851200"]`);
		const monthButton = await header.shadow$(`[data-kengine-cal-header-btn-month]`);
		const yearButton = await header.shadow$(`[data-kengine-cal-header-btn-year]`);
	...
	it("Calendar focuses the selected year when yearpicker is opened", async () => {
		await browser.url(`test/pages/Calendar.html`);
		const calendar = await browser.$("#calendar1");
		const yearPicker = await calendar.shadow$("kengine-yearpicker");
		const YEAR = 1997;
		await calendar.setAttribute("timestamp", Date.UTC(YEAR) / 1000);
		await calendar.shadow$("kengine-calendar-header").shadow$(`div[data-kengine-cal-header-btn-year]`).click();
	...
	it("Calendar focuses the selected month when monthpicker is opened with space", async () => {
		...
		const calendar = await browser.$("#calendar1");
		const dayPicker = await calendar.shadow$("kengine-daypicker");
		const monthPicker = await calendar.shadow$("kengine-monthpicker");
		const currentDayItem = await dayPicker.shadow$(`div[data-sap-timestamp="974851200"]`);
		await calendar.setAttribute("timestamp", new Date(Date.UTC(2000, 10, 22, 0, 0, 0)).valueOf() / 1000);
	...
	it("Calendar doesn't mark year as selected when there are no selected dates", async () => {
		...
		await calendar.shadow$("kengine-calendar-header").shadow$(`div[data-kengine-cal-header-btn-year]`).click();
		const focusedItem = await calendar.shadow$("kengine-yearpicker").shadow$(`[data-sap-timestamp="973036800"]`);

		assert.ok(await focusedItem.isFocusedDeep(), "Current year element is the active element");
		assert.notOk(await focusedItem.hasClass("kengine-yp-item--selected"), "Current year is not selected");
	});
	...
	it("Calendar doesn't mark month as selected when there are no selected dates", async () => {
		...
		const calendar = await browser.$("#calendar1");
		await calendar.setAttribute("timestamp", new Date(Date.UTC(2000, 10, 1, 0, 0, 0)).valueOf() / 1000);
		await calendar.shadow$("kengine-calendar-header").shadow$(`div[data-kengine-cal-header-btn-month]`).click();
		const focusedItem = await calendar.shadow$("kengine-monthpicker").shadow$(`[data-sap-timestamp="973036800"]`);

		assert.ok(await focusedItem.isFocusedDeep(), "Current month element is the active element");
		assert.notOk(await focusedItem.hasClass("kengine-mp-item--selected"), "Current month is not selected");
	});
	...
	it("Page up/down increments/decrements the month value", async () => {
		...
		await calendar.shadow$("kengine-daypicker").shadow$(`[tabindex="0"]`).click();
	...
	it("When month picker is shown the month button is hidden", async () => {
		await browser.url(`test/pages/Calendar.html`);
		const calendar = await browser.$("#calendar1");
		const calendarHeader = await calendar.shadow$("kengine-calendar-header");

		await calendar.shadow$("kengine-daypicker").shadow$(`[tabindex="0"]`).click();
		await browser.keys(["F4"]);
		await browser.keys('PageUp');

		assert.ok(await calendarHeader.shadow$("[data-kengine-cal-header-btn-month]").getAttribute("hidden"), "The button for month is hidden");
		await browser.keys("Space");
	});
	...
	it("Calendar with 'Multiple' selection type", async () => {
		...
		const dates = [
			await calendar.shadow$("kengine-daypicker").shadow$(`[data-sap-timestamp="971136000"]`),
			await calendar.shadow$("kengine-daypicker").shadow$(`[data-sap-timestamp="971222400"]`),
			await calendar.shadow$("kengine-daypicker").shadow$(`[data-sap-timestamp="971308800"]`),
		];

		await Promise.all(dates.map(async date => {
			await date.click();
			assert.ok(await date.hasClass("kengine-dp-item--selected"), `${await date.getAttribute("data-sap-timestamp")} is selected`);
		}));
	...
	it("Keyboard navigation works properly, when calendar selection type is set to 'Multiple'", async () => {
		...
		assert.ok(await calendar.shadow$("kengine-daypicker").shadow$(`[data-sap-timestamp="971222400"]`).isFocusedDeep(), "Focus is properly set");
	});
	...
	it("Calendar with 'Range' selection type", async () => {
		...
		const dates = [
			await calendar.shadow$("kengine-daypicker").shadow$(`[data-sap-timestamp="971740800"]`),
			await calendar.shadow$("kengine-daypicker").shadow$(`[data-sap-timestamp="971827200"]`),
			await calendar.shadow$("kengine-daypicker").shadow$(`[data-sap-timestamp="971913600"]`),
		];

		await dates[0].click();
		await dates[2].click();

		assert.ok(await dates[0].hasClass("kengine-dp-item--selected"), `${await dates[0].getAttribute("data-sap-timestamp")} is selected`);
		assert.ok(await dates[1].hasClass("kengine-dp-item--selected-between"), `${await dates[1].getAttribute("data-sap-timestamp")} is selected between`);
		assert.ok(await dates[2].hasClass("kengine-dp-item--selected"), `${await dates[2].getAttribute("data-sap-timestamp")} is selected`);
	...
	it("Previous and next buttons are disabled when necessary", async () => {
		await browser.url(`test/pages/Calendar.html`);
		const calendarHeader = await browser.$("#calendar4").shadow$("kengine-calendar-header");
		const prevButton = await calendarHeader.shadow$(`[data-kengine-cal-header-btn-prev]`);
		const nextButton = await calendarHeader.shadow$(`[data-kengine-cal-header-btn-next]`);

		assert.ok(await prevButton.hasClass("kengine-calheader-arrowbtn-disabled"), "Previous Button is disabled");
		assert.notOk(await nextButton.hasClass("kengine-calheader-arrowbtn-disabled"), "Next Button is enabled");

		await nextButton.click();

		assert.notOk(await prevButton.hasClass("kengine-calheader-arrowbtn-disabled"), "Previous Button is enabled");
		assert.notOk(await nextButton.hasClass("kengine-calheader-arrowbtn-disabled"), "Next Button is enabled");
		...
		assert.notOk(await prevButton.hasClass("kengine-calheader-arrowbtn-disabled"), "Previous Button is enabled");
		assert.ok(await nextButton.hasClass("kengine-calheader-arrowbtn-disabled"), "Next Button is disabled");
	});

	it("Second month and year are rendered in the header", async () => {
		const calendar = await browser.$("#calendar5");
		await calendar.setAttribute("timestamp", new Date(Date.UTC(2000, 9, 10, 0, 0, 0)).valueOf() / 1000);
		const calendarHeader = await browser.$("#calendar5").shadow$("kengine-calendar-header");
		const monthButton = await calendarHeader.shadow$(`[data-kengine-cal-header-btn-month]`).$$('span');
		const yearButton = await calendarHeader.shadow$(`[data-kengine-cal-header-btn-year]`).$$('span');
	...
	it("Calendar render two type for Month when MonthPicker is opened", async () => {
		...
		await calendar.shadow$("kengine-calendar-header").shadow$(`div[data-kengine-cal-header-btn-month]`).click();
		const months = await calendar.shadow$("kengine-monthpicker").shadow$$(`.kengine-mp-item`);
	...
	it("Calendar render two type for Year when YaerPicker is opened", async () => {
		...
		await calendar.setAttribute("timestamp", new Date(Date.UTC(2000, 0, 1, 0, 0, 0)).valueOf() / 1000);
		await calendar.shadow$("kengine-calendar-header").shadow$(`div[data-kengine-cal-header-btn-year]`).click();
		const years = await calendar.shadow$("kengine-yearpicker").shadow$$(`.kengine-yp-item`);
	...
	it("Min and max dates are set without format-pattern by using ISO (YYYY-MM-dd) format", async () => {
		...
		const yearButton = await calendar.shadow$("kengine-calendar-header").shadow$(`div[data-kengine-cal-header-btn-year]`);
		await yearButton.click();

		const year2025 = await calendar.shadow$("kengine-yearpicker").shadow$$(`div[role="gridcell"] span`).find(async span => {
			const text = await span.getText();
			return text === "2025";
		}).parentElement();

		assert.strictEqual(await year2025.hasClass("kengine-yp-item--disabled"), true, "Year 2025 is disabled");
	});
	...
	it("Min and max dates are NOT set without format-pattern, because are not in ISO format (YYYY-MM-dd)", async () => {
		...
		await yearButton.click();
		const year2016 = await calendar.shadow$("kengine-yearpicker").shadow$$(`div[role="gridcell"] span`).find(async span => {
			const text = await span.getText();
			return text === "2016";
		}).parentElement();

		assert.strictEqual(await year2016.hasClass("kengine-yp-item--disabled"), false, "Year 2016 is not disabled");

		const year2024 = await calendar.shadow$("kengine-yearpicker").shadow$$(`div[role="gridcell"] span`).find(async span => {
			const text = await span.getText();
			return text === "2024";
		}).parentElement();

		assert.strictEqual(await year2024.hasClass("kengine-yp-item--disabled"), false, "Year 2024 is not disabled");
	});
	...
	it("Apply secondaryCalendar class when primary and secondary calendar types are different", async () => {
		...
		const calendar = await browser.$("#calendar5");
		const dayPickerRoot = await calendar.shadow$("kengine-daypicker").shadow$(".kengine-dp-root");

		assert.strictEqual(await dayPickerRoot.hasClass("kengine-dp-twocalendartypes"), true, "Secondary Calendar class is applied correctly");
	});
	...
	it("Apply secondaryCalendar class when primary and secondary calendar types are the same", async () => {
		...
		assert.strictEqual(await dayPickerRoot.hasClass("kengine-dp-twocalendartypes"), false, "Secondary Calendar class is applied correctly");
	});
