...
describe("Select Menu general interaction", () => {
	...
	it("first option is selected by default", async () => {
		...
		const selectText = select.shadow$(".kengine-select-label-root");
	...
	it("reverts value on ESC key", async () => {
		...
		const selectText = select.shadow$(".kengine-select-label-root");
		const selectTextHtml = await selectText.getHTML(false);
		const selectedOption = await browser.$("#selectOptionsTest kengine-select-menu-option[selected]");

		await select.click();
		await select.keys("ArrowDown");
		await select.keys("Escape");

		const selectTextAfterEscape = await select.shadow$(".kengine-select-label-root").getHTML(false);
