...
describe("Dialog general interaction", () => {
	...
	it("tests popover in dialog", async () => {
		...
		const popoverZIndex = parseInt((await browser.$(`.${await select.getProperty("_id")}`).shadow$("kengine-responsive-popover").getCSSProperty("z-index")).value);
	...
	it("tests dialog lifecycle", async () => {
		...
		let staticAreaItem = await browser.$("kengine-static-area>kengine-static-area-item");
		...
		staticAreaItem = await browser.$("kengine-static-area>kengine-static-area-item");
	...
	it("draggable and resizable dialog repositions after screen resize", async () => {
		...
		const header = await dialog.shadow$(".kengine-popup-header-root");
	...
	it("draggable - mouse support", async () => {
		...
		const header = await browser.$("#draggable-dialog").shadow$(".kengine-popup-header-root");
	...
	it("resizable - mouse support", async () => {
		...
		const handle = await browser.$("#resizable-dialog").shadow$(".kengine-popup-resize-handle");
...
describe("Acc", () => {
	...
	it("tests aria-labelledby and aria-label", async () => {
		const dialog = await browser.$("kengine-dialog");
		await dialog.removeAttribute("accessible-name");
		assert.ok(await dialog.shadow$(".kengine-popup-root").getAttribute("aria-labelledby"), "dialog has aria-labelledby.");
		assert.notOk(await dialog.shadow$(".kengine-popup-root").getAttribute("aria-label"), "dialog does not have aria-label.");

		await dialog.setAttribute("accessible-name", "text");
		assert.notOk(await dialog.shadow$(".kengine-popup-root").getAttribute("aria-labelledby"), "dialog does not have aria-labelledby.");
		assert.notOk(await dialog.shadow$(".kengine-popup-header-root").getAttribute("aria-describedby"), "dialog hasn't aria-describedby.");
		assert.notOk(await dialog.shadow$(".kengine-popup-header-root").getAttribute("aria-roledescription"), "dialog hasn't aria-roledescription.");
		assert.ok(await dialog.shadow$(".kengine-popup-root").getAttribute("aria-label"), "dialog has aria-label.");
	});
	...
	it("tests aria-labelledby for slot header", async () => {
		...
		const dialog = await browser.$("#draggable-dialog");
		const idOfTheHiddenText = await dialog.shadow$(".kengine-hidden-text").getAttribute("id");
		const valueOfTheHiddenTextHTML = await dialog.shadow$(".kengine-hidden-text").getHTML(false);
		...
		assert.strictEqual(await dialog.shadow$(".kengine-popup-root").getAttribute("aria-label"), accName, "dialog has aria-label.");
		assert.strictEqual(await dialog.shadow$(".kengine-popup-header-root").getAttribute("aria-describedby"), idOfTheHiddenText, "dialog has aria-describedby.");
		assert.strictEqual(await valueOfTheHiddenText, ariaDescribedbyText, "dialog has aria-describedby with correct value.");
		assert.strictEqual(await dialog.shadow$(".kengine-popup-header-root").getAttribute("aria-roledescription"), roleDescriptionHeader, "dialog has aria-roledescription.");
	...
	it("tests accessible-name-ref", async () => {
		...
		assert.strictEqual(await dialog.shadow$(".kengine-popup-root").getAttribute("aria-label"), expectedText, "aria-label should be the text of the label.");
	});

	it("tests role and aria-modal", async () => {
		const dialog1 = await browser.$("#dialog");

		assert.strictEqual(await dialog1.shadow$(".kengine-popup-root").getAttribute("role"), "dialog", "role='dialog' is set");
		assert.strictEqual(await dialog1.shadow$(".kengine-popup-root").getAttribute("aria-modal"), "true", "aria-modal='true' is set");

		const dialog2 = await browser.$("#dialog-error-no-role");

		assert.strictEqual(await dialog2.shadow$(".kengine-popup-root").getAttribute("role"), "alertdialog", "role='alertdialog' is set");
		assert.strictEqual(await dialog2.shadow$(".kengine-popup-root").getAttribute("aria-modal"), "true", "aria-modal='true' is set");

		const dialog3 = await browser.$("#dialog-no-state-alert-role");

		assert.strictEqual(await dialog3.shadow$(".kengine-popup-root").getAttribute("role"), "alertdialog", "role='alertdialog' is set");
		assert.strictEqual(await dialog3.shadow$(".kengine-popup-root").getAttribute("aria-modal"), "true", "aria-modal='true' is set");

		const dialog4 = await browser.$("#dialog-none-role");

		assert.notOk(await dialog4.shadow$(".kengine-popup-root").getAttribute("role"),  "role is not set");
		assert.notOk(await dialog4.shadow$(".kengine-popup-root").getAttribute("aria-modal"), "aria-modal is not set");

		const dialog5 = await browser.$("#dialog-success-state-alert-role");

		assert.strictEqual(await dialog5.shadow$(".kengine-popup-root").getAttribute("role"), "alertdialog", "role='alertdialog' is set");
		assert.strictEqual(await dialog5.shadow$(".kengine-popup-root").getAttribute("aria-modal"), "true", "aria-modal='true' is set");
	});
});
...
describe("Responsive paddings", async () => {
	...
	it("tests responsive paddings", async () => {
		...
		// content
		const actualContentPadding = await dialog.shadow$(".kengine-popup-content").getCSSProperty("padding-left");

		// header
		const actualHeaderPadding = await dialog.shadow$(".kengine-popup-header-root").getCSSProperty("padding-left");

		// footer
		const actualFooterPadding = await dialog.shadow$(".kengine-popup-footer-root").getCSSProperty("padding-left");
...
describe("Dialog States", () => {
	...
	it("tests error state dialog accessibility role", async () => {
		...
		assert.strictEqual(await browser.$("#dialog-error-state").shadow$(".kengine-popup-root").getAttribute("role"), "alertdialog", "error dialog has correct role");
	...
	it("tests information state dialog accessibility role", async () => {
		...
		assert.strictEqual(await browser.$("#dialog-info-state").shadow$(".kengine-popup-root").getAttribute("role"), "dialog", "information dialog has correct role");
	...
	it("tests success state dialog accessibility role", async () => {
		...
		assert.strictEqual(await browser.$("#dialog-success-state").shadow$(".kengine-popup-root").getAttribute("role"), "dialog", "success dialog has correct role");
	...
	it("tests warning state dialog accessibility role", async () => {
		...
		assert.strictEqual(await browser.$("#dialog-warn-state").shadow$(".kengine-popup-root").getAttribute("role"), "alertdialog", "warning dialog has correct role");
...
describe("Block layers", () => {
	...
	it("test dialog overlay when dialog isn't open", async () => {
		const isBlockLayerHidden = await browser.executeAsync(async (done) => {
			...
			done(staticAreaItemDomRef.querySelector(".kengine-block-layer").hasAttribute("hidden"));
		});
