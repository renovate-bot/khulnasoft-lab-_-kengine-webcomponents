...
describe("BusyIndicator general interaction", () => {
	...
	it("tests event propagation", async () => {
		const dynamicItem = await browser.$(">>>#busy-tree [kengine-tree-item] kengine-icon.kengine-li-tree-toggle-icon");
	...
	it("test activation", async () => {
		...
		const busyArea = await busyIndicator.shadow$(".kengine-busy-indicator-busy-area");
	...
	it("tests focus handling", async () => {
		...
		assert.strictEqual(await innerFocusElement.getAttribute("class"), "kengine-busy-indicator-busy-area", "The correct inner element is focused");
	});

	it("tests internal focused element attributes", async () => {
		const busyIndicator = await browser.$("#indicator1");
		await busyIndicator.click();
		const innerFocusElement = await busyIndicator.shadow$(".kengine-busy-indicator-busy-area");
	...
	it("Height of the root element depends on the height of the Busy Indicator - issue 6668", async () => {
		...
		const height = parseInt((await busyIndicator.shadow$(".kengine-busy-indicator-root").getCSSProperty("height")).value);
