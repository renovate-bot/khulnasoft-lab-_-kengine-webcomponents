...
describe("Avatar", () => {
	...
	it("tests rendering of image", async () => {
		...
		const icon = await avatar.shadow$("kengine-avatar-icon");

		// img tag is rendered, kengine-icon - not
	...
	it("tests rendering of icon", async () => {
		...
		// kengine-icon tag is rendered, img - not
	...
	it("tests rendering of image, when all set", async () => {
		...
		const image = await avatar.shadow$('slot:not([name="badge"])');
		const icon = await avatar.shadow$(".kengine-avatar-icon");
		const initials = await avatar.shadow$(".kengine-avatar-initials");
	...
	it("tests rendering of custom fallback icon when initials are overflowing ", async () => {
		...
		const fbIcon = await avatar.shadow$(".kengine-avatar-icon-fallback");
	...
	it("tests clicking on interactive disabled avatar", async () => {
		...
		const avatarRoot = await browser.$("#interactive-disabled").shadow$(".kengine-avatar-root");
	...
	it("Tests noConflict 'kengine-click' event is thrown for interactive avatars", async () => {
		const avatarRoot = await browser.$("#interactive-avatar").shadow$(".kengine-avatar-root");
	  ...
	  it("Tests noConflict 'kengine-click' event is not thrown for non interactive avatars", async () => {
		const avatarRoot = await browser.$("#non-interactive-avatar").shadow$(".kengine-avatar-root");;
		const input = await browser.$("#click-event");
...
describe("ARIA attributes", () => {
	...
	it ("role set correctly", async () => {
		...
		const avatarRoot = await avatar.shadow$(".kengine-avatar-root");
		const nonInteractiveAvatar = await $("#non-interactive-avatar");;
		const nonInteractiveavatarRoot = await nonInteractiveAvatar.shadow$(".kengine-avatar-root");
	...
	it ("aria-label is correctly set", async () => {
		const avatar = await $("#interactive-avatar"),
			  avatarRoot = avatar.shadow$(".kengine-avatar-root");
