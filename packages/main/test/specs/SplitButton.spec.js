...
describe("Split Button general interaction", () => {
	it("tests inner buttons design", async () => {
		...
		const textButton = await sbDefault.shadow$(".kengine-split-text-button");
		const arrowButton = await sbDefault.shadow$(".kengine-split-arrow-button");
	...
	it("tests textContent on 'click'", async () => {
		...
		const textButton1 = await sbEmpty.shadow$(".kengine-split-text-button");
		const sbDefault = await browser.$("#defaultSpBtn");
		const textButton2 = await sbDefault.shadow$(".kengine-split-text-button");
	...
	it("tests text button 'click' event (mouse)", async () => {
		...
		assert.strictEqual(await field.getValue(), "kengine-click", "'click' is fired");
	});
	...
	it("tests arrow button 'arrow-click' event (mouse)", async () => {
		...
		assert.strictEqual(await field.getValue(), "kengine-arrow-click", "'arrow-click' is fired");
	});
