...
describe("Select general interaction", () => {
	...
	it("fires change on selection", async () => {
		...
		const selectText = await browser.$("#mySelect").shadow$(".kengine-select-label-root");
		...
		const firstItem = (await browser.$(`.${staticAreaItemClassName}`).shadow$$("kengine-li"))[0];
	...
	it("prevents change on selection", async () => {
		...
		const selectText = await browser.$("#selectPrevent").shadow$(".kengine-select-label-root");
		...
		const secondItem = (await browser.$(`.${staticAreaItemClassName}`).shadow$$("kengine-li"))[1];
	...
	it("does not fire change, when clicking on selected item", async () => {
		...
		const staticAreaItemClassName = await browser.getStaticAreaItemClassName("#mySelect")
		const firstItem = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-li:last-child");
		await firstItem.click();
	...
	it("fires change on selection with keyboard handling", async () => {
		...
		const select = await browser.$("#errorSelect").shadow$(".kengine-select-root");
		const selectText = await browser.$("#errorSelect").shadow$(".kengine-select-label-root");
	...
	it("changes selection sync with selection announcement", async () => {
		...
		const inputResult = await browser.$("#inputResult").shadow$("input");
		const politeSpan = await browser.$(".kengine-invisiblemessage-polite");
		const select = await browser.$("#errorSelect");
		const selectText = await browser.$("#errorSelect").shadow$(".kengine-select-label-root");
		const EXPECTED_SELECTION_TEXT1 = "Compact";
		const EXPECTED_SELECTION_TEXT2 = "Condensed";
	...
	it("announces the selected value once Select Popover is opened", async () => {
		...
		let politeSpanHtml = await politeSpan.getHTML(false);
		let selectedOptionHTML = await browser.$("#mySelect kengine-option[selected]").getHTML(false);
	...
	it("changes selection on Tab", async () => {
		...
		const selectText = await select.shadow$(".kengine-select-label-root");
	...
	it("tests selection does not cycle with ArrowDown", async () => {
		...
		const selectOptionText = await select.shadow$(".kengine-select-label-root");
	...
	it("adds unselected only items to select", async () => {
		...
		const firstOption = await browser.$("#mySelect kengine-option:first-child");
		const staticAreaItemClassName = await browser.getStaticAreaItemClassName("#mySelect");
		const firstListItem = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-li:first-child");
	...
	it("reverts value before open after clicking on escape", async () => {
		...
		const select = await browser.$("#mySelect");
		const selectText = await browser.$("#mySelect").shadow$(".kengine-select-label-root").getHTML(false);
		const inputResult = await browser.$("#inputResult").shadow$("input");
		...
		const selectedOption = await browser.$("#mySelect kengine-option[selected]");
		const selectTextAfterEscape = await browser.$("#mySelect").shadow$(".kengine-select-label-root").getHTML(false);
	...
	it("fires change event after selecting a previewed item", async () => {
		...
		await select.click();
		const staticAreaItemClassName = await browser.getStaticAreaItemClassName("#mySelect");
		const firstItem = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-li:first-child");
	...
	it("tests ESC on closed picker", async () => {
		...
		const selectText = await browser.$("#mySelectEsc").shadow$(".kengine-select-label-root");
		const EXPECTED_SELECTION_TEXT = "Cozy";
		const EXPECTED_SELECTION_TEXT2 = "Condensed";
		...
		const firstItem = (await browser.$(`.${staticAreaItemClassName}`).shadow$$("kengine-li"))[0];
		const thirdItem = (await browser.$(`.${staticAreaItemClassName}`).shadow$$("kengine-li"))[2];
	...
	it("Tests accessibility", async () => {
		const select1 = await browser.$("#textAreaAriaLabel").shadow$(".kengine-select-label-root");
		const select2 = await browser.$("#textAreaAriaLabelledBy").shadow$(".kengine-select-label-root");
	...
	it("Tests if currently selected option is visible in the viewport when keyboard navigation is used", async () => {
		...
		const selectedOption = await popover.$("kengine-list").$("kengine-li[selected]");
...
describe("Attributes propagation", () => {
	...
	it("propagates additional-text attribute", async () => {
		const EXPECTED_ADDITIONAL_TEXT = "DZ",
			...
			firstOption = await browser.$("#mySelect6 kengine-option:first-child"),
			firstItem = (await browser.$(`.${staticAreaItemClassName}`).shadow$$("kengine-li"))[0];
