...
describe("Attributes propagation", () => {
	...
	it("Disabled attribute is propagated properly", async () => {
		assert.ok(await browser.$("#input-disabled").shadow$(".kengine-input-inner").getAttribute("disabled"), "Disabled property was propagated");
	});

	it("Redonly attribute is propagated properly", async () => {
		assert.ok(await browser.$("#input-readonly").shadow$(".kengine-input-inner").getAttribute("readonly"), "Readonly property was propagated");
	});

	it("Required attribute is propagated properly", async () => {
		assert.strictEqual(await browser.$("#input-required").shadow$(".kengine-input-inner").getAttribute("aria-required"), "true", "Aria-required attribute is set correctly");
		assert.strictEqual(await browser.$("#input-number").shadow$(".kengine-input-inner").getAttribute("aria-required"), "false", "Aria-required attribute is set correctly");
	});

	it("Type attribute is propagated properly", async () => {
		const sExpectedType = "number";
		assert.strictEqual(await browser.$("#input-number").shadow$(".kengine-input-inner").getAttribute("type"), sExpectedType, "Type property was propagated");
		assert.strictEqual(await browser.$("#input-number").shadow$(".kengine-input-inner").getAttribute("step"), "any", "The step attr is set");
	});
	...
	it("Value attribute is propagated properly", async () => {
		...
		assert.strictEqual(await browser.$("#input3").shadow$(".kengine-input-inner").getValue(), sExpectedValue, "Value property was set correctly");
	});
...
describe("Input general interaction", () => {
	...
	it("handles suggestions", async () => {
		...
		// item = await browser.$("#myInput").$$("kengine-li")[0];
	...
	it("Input value should correspond to suggestion item when it is clicked", async () => {
		...
		const staticAreaItemClassName = await browser.getStaticAreaItemClassName("#myInput");
		const respPopover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover");
		...
		// select the item
		let firstSuggestion = await respPopover.$("kengine-list").$("kengine-li-suggestion-item");
	...
	it("should select typeaheaded item on mouse click and remove value text selection", async () => {
		...
		const firstSuggestion = await respPopover.$("kengine-list").$("kengine-li-suggestion-item");
		await firstSuggestion.click();
	...
	it("should select typeaheaded item on mouse click and remove value text selection", async () => {
		...
		const secondSuggestion = await respPopover.$("kengine-list").$$("kengine-li-suggestion-item")[1];
	...
	it("should remove input's focus when group header item is clicked", async () => {
		...
		const staticAreaItemClassName = await browser.getStaticAreaItemClassName("#myInputGrouping");
		const respPopover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover");
		const groupHeader = await respPopover.$("kengine-list").$("kengine-li-groupHeader");
	...
	it("Checks if valueStateMessage is shown", async () => {
		...
		const respPopover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$(".kengine-responsive-popover-header");
	...
	it("Value state type should be added to the screen readers default value states announcement", async () => {
		...
		let ariaHiddenText = await inputError.shadow$(".kengine-hidden-text").getText();
		...
		ariaHiddenText = await inputWarning.shadow$(".kengine-hidden-text").getText();
		...
		ariaHiddenText = await inputInformation.shadow$(".kengine-hidden-text").getText();
		valueStateText = await popover.$("div").getText();
		...
		assert.strictEqual(await inputSuccess.shadow$(".kengine-hidden-text").getText(), "Value State Success", "Hidden screen reader text is correct");
	});
	...
	it("Value state type should be added to the screen readers custom value states announcement", async () => {
		...
		const popoverHeader = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-popover").$(".kengine-valuestatemessage-header");
	...
	it("fires suggestion-item-preview", async () => {
		...
		const inputPopover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover");
	...
	it("Should open suggestions popover when kengine-input is the first focusable element within a dialog", async () => {
		await browser.url(`test/pages/Input.html`);
	...
	it("Displays clear icon when typing and pressing it clears the value", async () => {
		...
		const clearIcon = await input.shadow$(".kengine-input-clear-icon-wrapper");
...
describe("Input arrow navigation", () => {
	...
	it("Should navigate up and down through the suggestions popover with arrow keys", async () => {
		...
		const respPopover = await browser.$(`.${staticAreaClassName}`).shadow$("kengine-responsive-popover");
		const firstListItem = await respPopover.$("kengine-list").$("kengine-li-suggestion-item");
		...
		await suggestionsInput.keys("ArrowDown");
		const secondListItem = await respPopover.$("kengine-list").$$("kengine-li-suggestion-item")[1];
	...
	it("Value state header and group headers should be included in the arrow navigation", async () => {
		...
		const respPopover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover");
		const valueStateHeader = await respPopover.$(".kengine-responsive-popover-header.kengine-valuestatemessage-root");
		const firstListItem = await respPopover.$("kengine-list").$("kengine-li-suggestion-item");
		const groupHeader = await respPopover.$("kengine-list").$("kengine-li-groupHeader");
...
describe("Input END navigation", () => {
	...
	it("Should focus the last item from the suggestions popover with END", async () => {
		...
		const respPopover = await browser.$(`.${staticAreaClassName}`).shadow$("kengine-responsive-popover");
		const firstListItem = await respPopover.$("kengine-list").$("kengine-li-suggestion-item:last-child");
...
describe("Input PAGEUP/PAGEDOWN navigation", () => {
	...
	it("Should focus the tenth item from the suggestions popover with PAGEDOWN", async () => {
		...
		const respPopover = await browser.$(`.${staticAreaClassName}`).shadow$("kengine-responsive-popover");
		const tenthListItem = await respPopover.$("kengine-list").$("kengine-li-suggestion-item:nth-child(11)");
...
describe("XSS tests for suggestions", () => {
	...
	it("tests dangerous items highlighting", async () => {
		...
		const listItems = await $(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover").$$("kengine-li-suggestion-item");
		const expected = [
			"",
			"<b></b>",
			"3412test1234",
...
describe("Lazy loading", () => {
	...
	it("Lazy loading opens the picker once items are populated", async () => {
		...
		const respPopover = await $(`.${staticAreaClassName}`).shadow$("kengine-responsive-popover");
