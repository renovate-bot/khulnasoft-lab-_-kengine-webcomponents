...
describe("Card general interaction", () => {
	...
	it("tests status is rendered, when action is set", async () => {
		const status = await browser.$("#actionCardHeader").shadow$(".kengine-card-header-status");
	...
	it("tests interactive header results in interactive class on the card", async () => {
		...
		const interactiveCardRoot = interactiveCard.shadow$(".kengine-card-root");
		const interactiveCardHeader = interactiveCard.$("kengine-card-header");

		assert.strictEqual(await interactiveCardHeader.getProperty("interactive"), true, "card header has interactive property");
		assert.strictEqual(await interactiveCardRoot.hasClass("kengine-card--interactive"), true, "card root has interactive class");

		const nonInteractiveCard = await browser.$("#card2");
		const nonInteractiveCardRoot = nonInteractiveCard.shadow$(".kengine-card-root");
		const nonInteractiveCardHeader = nonInteractiveCard.$("kengine-card-header");
		...
		assert.strictEqual(await nonInteractiveCardRoot.hasClass("kengine-card--interactive"), false, "card root doesn't have interactive class");
	});

	it("tests header's click event with mouse click, Enter and Space", async () => {
		const cardHeader = await browser.$("#cardHeader").shadow$(".kengine-card-header");
		const cardHeader2 = await browser.$("#cardHeader2").shadow$(".kengine-card-header");
		// TODO: the field is bound to the "kengine-click" event, but it misses out a test case
	...
	it("tests aria-label", async () => {
		const card = await browser.$("#textAreaAriaLabel").shadow$(".kengine-card-root");
	...
	it("tests ARIA attributes of the content", async () => {
		const card = await browser.$("#card");
		const content = await card.shadow$(".kengine-card-root div:nth-child(2)");
	...
	it("tests aria-level property", async () => {
		const cardHeader = await $("#card2").$("kengine-card-header");

		// Default value
		assert.strictEqual(await cardHeader.shadow$(".kengine-card-header .kengine-card-header-title").getAttribute("aria-level"), "3");

		const cardHeaderTitle = await cardHeader.shadow$(".kengine-card-header .kengine-card-header-title");
		await cardHeaderTitle.setAttribute("aria-level", 4);
		assert.strictEqual(await cardHeader.shadow$(".kengine-card-header .kengine-card-header-title").getAttribute("aria-level"), "4");
	});
});
...
describe("CardHeader", () => {
	...
	it("tests header aria-labelledby", async () => {
		const header = await browser.$("#header").shadow$(".kengine-card-header .kengine-card-header-focusable-element");
		const header2 = await browser.$("#header2").shadow$(".kengine-card-header .kengine-card-header-focusable-element");
...
describe("Card Accessibility", () => {
	...
	it("test accessibleName", async () => {
		const card = await browser.$("#textCard").shadow$(".kengine-card-root");
	...
	it("test accessibleNameRef", async () => {
		const card = await browser.$("#textCardRef").shadow$(".kengine-card-root");
