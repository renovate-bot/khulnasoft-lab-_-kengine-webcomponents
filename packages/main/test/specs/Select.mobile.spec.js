...
describe("Select mobile general interaction", () => {
	...
	it("Changes selection in Dialog", async () => {
		...
		const selectedItem = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-li:last-child");
		...
		const selectTextAfter = await select.shadow$(".kengine-select-label-root");
