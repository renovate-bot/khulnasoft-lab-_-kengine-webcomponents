...
describe("Rating Indicator general interaction", () => {
	...
	it("Tests basic rating indicator rendering", async () => {
		...
		assert.strictEqual(await ratingIndicator.shadow$$(".kengine-rating-indicator-item").length, 5, "Basic rating indicator renders 5 stars");
	});

	it("Tests max property", async () => {
		const ratingIndicator = await browser.$("#rating-indicator2");

		assert.strictEqual(await ratingIndicator.shadow$$(".kengine-rating-indicator-item").length, 10, "Basic rating indicator renders 10 stars");
	});

	it("Tests clicking on star", async () => {
		const ratingIndicator = await browser.$("#rating-indicator3");
		const thirdStar = await ratingIndicator.shadow$$(".kengine-rating-indicator-item")[2];
	...
	it("Tests ACC attrs", async () => {
		const ratingIndicator = await browser.$("#rating-indicator1");
		const ratingIndicatorRoot = ratingIndicator.shadow$(".kengine-rating-indicator-root");
		const ratingIndicatorList = ratingIndicator.shadow$(".kengine-rating-indicator-list");
		const thirdStar = await ratingIndicator.shadow$$(".kengine-rating-indicator-item")[2];
		const ratingIndicatorReadOnly = await browser.$("#rating-indicator-readonly").shadow$(".kengine-rating-indicator-root");
	...
	it("Tests ACC attrs - tooltip property", async () => {
		const ratingIndicator = await browser.$("#rating-indicator-title").shadow$(".kengine-rating-indicator-root");
		const TOOLTIP = "Test";
	...
	it("Tests ACC attrs - required property add aria-description", async () => {
		const ratingIndicatorRoot = await browser.$("#rating-indicator-required").shadow$(".kengine-rating-indicator-root");
	...
	it("Tests ACC attrs - accessible-name-ref", async () => {
		...
		assert.strictEqual(await ratingIndicator.shadow$(".kengine-rating-indicator-root").getAttribute("aria-label"), expectedText, "aria-label should be the text of the label.");
	});
});
