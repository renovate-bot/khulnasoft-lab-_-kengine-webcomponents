...
describe("MultiComboBox general interaction", () => {
	...
	describe("toggling", () => {
		it("opens/closes", async () => {
			...
			const popover = await browser.$(`.${staticAreaItemClassName}`).shadow$(".kengine-multi-combobox-all-items-responsive-popover");
		...
		it("Checks focus state", async () => {
			...
			const input = await mcb.shadow$("#kengine-multi-combobox-input");
		...
		it("Opens selected items Popover", async () => {
			...
			const staticAreaItemClassName = await browser.getStaticAreaItemClassName("#multi1");
			const mcb = await browser.$("#multi1");
			const showMore = mcb.shadow$(".kengine-multi-combobox-tokenizer").shadow$(".kengine-tokenizer-more-text");
			const allPopover = await browser.$(`.${staticAreaItemClassName}`).shadow$(".kengine-multi-combobox-all-items-responsive-popover");
		...
		it("Checks if tokenizer is expanded when adding items dynamically", async () => {
			...
			const inlinedTokens = await mcb.shadow$$("kengine-token:not([overflows])");
	...
	describe("selection and filtering", () => {
		...
		it("Opens all items popover, selects and deselects the first item", async () => {
			...
			const popover = await browser.$(`.${staticAreaItemClassName}`).shadow$(".kengine-multi-combobox-all-items-responsive-popover");
			const firstItem = await popover.$("kengine-list > kengine-li");
			const firstItemCheckbox = await firstItem.shadow$("kengine-checkbox");
			const eventInput = await browser.$("#events-input");
		...
		it("When popover is opened via icon and item is selected/deselected, focus should return to the MultiComboBox", async () => {
			...
			const popover = await browser.$(`.${staticAreaItemClassName}`).shadow$(".kengine-multi-combobox-all-items-responsive-popover");
			const firstItem = await popover.$(".kengine-multi-combobox-all-items-list > kengine-li");
		...
		it("Opens all items popover when start typing and filters items", async () => {
			const input = await browser.$("#mcb").shadow$("#kengine-multi-combobox-input");
			const staticAreaItemClassName = await browser.getStaticAreaItemClassName("#mcb")
			const popover = await browser.$(`.${staticAreaItemClassName}`).shadow$(".kengine-multi-combobox-all-items-responsive-popover");
			...
			const list = await popover.$(".kengine-multi-combobox-all-items-list");
		...
		it("tests built in validation by typing a non existing option", async () => {
			...
			const input = await browser.$("#mcb-validation").shadow$("#kengine-multi-combobox-input");
			const innerInput = await browser.$("#mcb-validation").shadow$("#kengine-multi-combobox-input");
		...
		it("When item is clicked, the popover should be closed and the value in the input should be removed", async () => {
			const input = await browser.$("#another-mcb").shadow$("#kengine-multi-combobox-input");
		...
		it("When item's checkbox is clicked, the popover should not be closed and the value in the input should be kept", async () => {
			...
			const staticAreaItemClassName = await browser.getStaticAreaItemClassName("#another-mcb")
			const popover = await browser.$(`.${staticAreaItemClassName}`).shadow$(".kengine-multi-combobox-all-items-responsive-popover");
			const firstItemCheckbox = await popover.$(".kengine-multi-combobox-all-items-list > kengine-li").shadow$("kengine-checkbox");
		...
		it("tests if n more is applied and corresponding popover", async () => {
			...
			const nMoreText = await browser.$("#more-mcb").shadow$("kengine-tokenizer").shadow$(".kengine-tokenizer-more-text");
		...
		it("tests if clicking n more will prefilter items before opening the popover", async () => {
			...
			const nMoreText = await mcb.shadow$("kengine-tokenizer").shadow$(".kengine-tokenizer-more-text");
			...
			const popover = await $(`.${staticAreaItemClassName}`).shadow$(".kengine-multi-combobox-all-items-responsive-popover");
			const list = await popover.$(".kengine-multi-combobox-all-items-list");
		...
		it("tests if tokenizer is scrolled to the end when expanded and to start when narrowed", async () => {
			...
			let tokenizerContentScrollLeft = await browser.execute(() => document.querySelector("#more-mcb").shadowRoot.querySelector("kengine-tokenizer").shadowRoot.querySelector(".kengine-tokenizer--content").scrollLeft);
			assert.notEqual(tokenizerContentScrollLeft, 0, "tokenizer is not scrolled to start");

			await input.keys('Tab');

			tokenizerContentScrollLeft = await browser.execute(() => document.querySelector("#more-mcb").shadowRoot.querySelector("kengine-tokenizer").shadowRoot.querySelector(".kengine-tokenizer--content").scrollLeft);
		...
		it("tests if tokenizer is scrolled on keyboard navigation through the tokens", async () => {
			...
			let scrollLeftFirstToken = await browser.execute(() => document.querySelector("#more-mcb").shadowRoot.querySelector("kengine-tokenizer").shadowRoot.querySelector(".kengine-tokenizer--content").scrollLeft);
			await input.keys('ArrowLeft');
			let scrollLeftSecondToken = await browser.execute(() => document.querySelector("#more-mcb").shadowRoot.querySelector("kengine-tokenizer").shadowRoot.querySelector(".kengine-tokenizer--content").scrollLeft);

			assert.notEqual(scrollLeftFirstToken, scrollLeftSecondToken, "tokenizer is scrolled when navigating through the tokens");

			await input.keys('ArrowRight');
			let newScrollLeft =  await browser.execute(() => document.querySelector("#more-mcb").shadowRoot.querySelector("kengine-tokenizer").shadowRoot.querySelector(".kengine-tokenizer--content").scrollLeft);
		...
		it("tests if tokenizer is not expanded/collapsed when the suggestions are opened from a selected token", async () => {
			...
			const mcb = await $("#more-mcb");
			let tokenizer = await mcb.shadow$("kengine-tokenizer")
			let tokens = await browser.$("#more-mcb").shadow$$(".kengine-multi-combobox-token");
			const input = mcb.shadow$("input");
			...
			tokens = await browser.$("#more-mcb").shadow$$(".kengine-multi-combobox-token");
		...
		it("tests filtering of items when nmore popover is open and user types in the input fueld", async () => {
			...
			const popover = await $(`.${staticAreaItemClassName}`).shadow$(".kengine-multi-combobox-all-items-responsive-popover");
			const list = await popover.$(".kengine-multi-combobox-all-items-list");
			const lastListItem = await list.$("kengine-li:last-child");
		...
		it("Should make a selection on ENTER and discard on ESC", async () => {
			...
			tokens = await mcb.shadow$$(".kengine-multi-combobox-token");
		...
		it ("should reset typeahead on item navigation and restore it on focus input", async () => {
			...
			const staticAreaItemClassName = await browser.getStaticAreaItemClassName("#mcb");
			const popover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover");
			...
			const listItem = await popover.$("kengine-list").$$("kengine-li")[0];
		...
		it("tests if clicking delete icon of a token removes it from the selection", async () => {
			...
			const token = await mcb.shadow$("kengine-tokenizer kengine-token");
			const deleteIcon = await token.shadow$(".kengine-token--icon");

			await deleteIcon.click();

			const tokens = await mcb.shadow$$(".kengine-multi-combobox-token");
		...
		it("prevents selection change event when clicking an item", async () => {
			...
			const input = mcb.shadow$("#kengine-multi-combobox-input");
			const staticAreaItemClassName = await browser.getStaticAreaItemClassName("#mcb-prevent")
			const popover = await $(`.${staticAreaItemClassName}`).shadow$(".kengine-multi-combobox-all-items-responsive-popover");
			const firstItem = await popover.$(".kengine-multi-combobox-all-items-list > kengine-li");
			const mcbTokens = await mcb.shadow$$(".kengine-multi-combobox-token");
		...
		it("prevents selection change event when deleting a token", async () => {
			...
			const mcb = $("#mcb-prevent");
			let tokens = await mcb.shadow$$(".kengine-multi-combobox-token");

			const deleteIcon = await tokens[0].shadow$("kengine-icon");
	...
	describe("keyboard handling", () => {
		...
		it("tests backspace when combobox has an empty value", async () => {
			let tokens = await browser.$("#multi1").shadow$$(".kengine-multi-combobox-token");
			...
			tokens = await browser.$("#multi1").shadow$$(".kengine-multi-combobox-token");
		...
		it ("Value should be reset on ESC key", async () => {
			...
			const input = await mCombo.shadow$("#kengine-multi-combobox-input");
			const input2 = await mCombo2.shadow$("#kengine-multi-combobox-input");
		...
		it ("focuses the value state header and item on arrow down then the value state and the input on arrow up", async () => {
			...
			let valueStateHeader = await browser.execute(staticArea => staticArea.shadowRoot.querySelector(".kengine-responsive-popover-header.kengine-valuestatemessage-root"), staticArea);
			...
			let listItem = await popover.$("kengine-list").$$("kengine-li")[0];
		...
		it ("CTRL + HOME focus the first item, CTRL + END should focus last item", async () => {
			...
			const lastItem = await popover.$("kengine-list").$$("kengine-li")[5];
		...
		it ("should select multiple tokens and move focus with shift+arrow keys", async () => {
			...
			let tokens2 = await mcb2.shadow$$(".kengine-multi-combobox-token");
		...
		it ("should not be able to paste tokenwith CTRL+V in read only multi combo box", async () => {
			...
			const mcb2Tokens = await mcb2.shadow$$(".kengine-multi-combobox-token");
		...
		it ("F4 should focus the selected item or the first one if there is no selected", async () => {
			...
			const listItem = await popover.$("kengine-list").$("kengine-li");
			...
			const listItem2 = await popover.$("kengine-list").$$("kengine-li")[1];
		...
		it ("Alt + Down should focus the corresponding item to the token from which the combination is pressed", async () => {
			...
			let listItem = await popover.$("kengine-list").$$("kengine-li")[3];
		...
		it ("Alt + Down should focus the item corresponding to the text value", async () => {
			...
			let listItem = await popover.$("kengine-list").$$("kengine-li")[1];
		...
		it("should open/close popover on keyboard combination ctrl + i", async () => {
			...
			const rpo = await browser.$(`.${rpoClassName}`).shadow$("kengine-responsive-popover");
	...
	describe("General", () => {
		...
		it ("tests two-column layout", async () => {
			...
			await icon.click();
			assert.strictEqual(await listItem.shadow$(".kengine-li-additional-text").getText(), "DZ", "Additional item text should be displayed");
			await icon.click();
		});

		it ("placeholder tests", async () => {
			const mcb1 = await browser.$("#another-mcb").shadow$("#kengine-multi-combobox-input");
			const mcb2 = await browser.$("#mcb-with-placeholder").shadow$("#kengine-multi-combobox-input");
		...
		it("placeholder tests for programmatically selected items", async () => {
			const innerInputSel = "#kengine-multi-combobox-input";
		...
		it("Should apply correct text to the tokens overflow indicator", async () => {
			...
			const tokenizerNItems = await mcNItems.shadow$("kengine-tokenizer");
			const tokenizerNMore = await mcNMore.shadow$("kengine-tokenizer");
			const nItemsLabel = await tokenizerNItems.shadow$(".kengine-tokenizer-more-text");
			const nMoreLabel = await tokenizerNMore.shadow$(".kengine-tokenizer-more-text");
	...
	describe("MultiComboBox Truncated Token", () => {
		...
		it("should truncate token when single token is in the multicombobox and open popover on click", async () => {
			const mcb = await $("#truncated-token");
			const token = await mcb.shadow$("kengine-token");
			const rpoClassName = await getTokenizerPopoverId("truncated-token");
			const rpo = await browser.$(`.${rpoClassName}`).shadow$("kengine-responsive-popover");
			...
			assert.ok(await rpo.$("kengine-li").getProperty("focused"), "Token's list item is focused");
	...
	describe("ARIA attributes", () => {
		...
		it ("aria-describedby value according to the tokens count and the value state", async () => {
			...
			const invisibleText = await mcb.shadow$(".kengine-hidden-text");
			let tokens = await mcb.shadow$$(".kengine-multi-combobox-token");
			const tokensCountITextId = `kengine-multi-combobox-hiddenText-nMore`;
			const valuestateITextId = `kengine-multi-combobox-valueStateDesc`;
		...
		it ("aria-describedby value according to the tokens count", async () => {
			...
			const innerInput = await mcb.shadow$("input");
			let invisibleText = await mcb.shadow$(".kengine-hidden-text");
			...
			tokens = await mcb.shadow$$(".kengine-multi-combobox-token");
			invisibleText = await mcb.shadow$(".kengine-hidden-text");
		...
		it("Value state type should be added to the screen readers default value states announcement", async () => {
			...
			let popover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-popover");

			await mCbWarning.click();

			let ariaHiddenText = await mCbWarning.shadow$(`#kengine-multi-combobox-valueStateDesc`).getHTML(false);
			...
			popover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-popover");

			ariaHiddenText = await mCbError.shadow$(`#kengine-multi-combobox-valueStateDesc`).getHTML(false);
			...
			ariaHiddenText = await mCbSuccess.shadow$(`#kengine-multi-combobox-valueStateDesc`).getHTML(false);
		...
		it("Value state type should be added to the screen readers custom value states announcement", async () => {
			...
			const popoverHeader = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover .kengine-valuestatemessage-header");
			const valueStateText = await popoverHeader.$("div").getHTML(false);
			const ariaHiddenText = await mCbInformation.shadow$(`#kengine-multi-combobox-valueStateDesc`).getHTML(false);
	...
	describe("Grouping", () => {
		it ("Tests group filtering", async () => {
			...
			let popover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover");
			let groupItems = await popover.$("kengine-list").$$("kengine-li-groupheader");
			let listItems = await popover.$("kengine-list").$$("kengine-li");
			...
			popover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover");
			groupItems = await popover.$("kengine-list").$$("kengine-li-groupheader");
			listItems = await popover.$("kengine-list").$$("kengine-li");
		...
		it ("Tests group item focusability", async () => {
			...
			groupItem = await popover.$("kengine-list").$$("kengine-li-groupheader")[0];
		...
		it ("Should not select group headers", async () => {
			...
			let tokens = await browser.$("#mcb-grouping").shadow$$(".kengine-multi-combobox-token");
