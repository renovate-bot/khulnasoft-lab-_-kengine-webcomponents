...
describe("Switch general interaction", async () => {
	...
	it("The 'required' attribute is propagated properly", async () => {
		assert.strictEqual(await browser.$("#requiredSwitch").shadow$(".kengine-switch-root").getAttribute("aria-required"), "true", "The required attribute is set correctly");
		assert.strictEqual(await browser.$("#switchprevented").shadow$(".kengine-switch-root").getAttribute("aria-required"), "false", "The required attribute is set correctly");
	})
