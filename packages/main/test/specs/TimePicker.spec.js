...
describe("TimePicker general interaction", () => {
	...
	it("input receives value in format pattern depending on the set language", async () => {
		...
		assert.equal(await timepicker.shadow$("kengine-input").getValue(), "3:16:16 ч.");
	});
	...
	it("tests clocks value", async () => {
		...
		const timepickerPopover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover");

		// act
		await timepicker.setProperty("value", "11:12:13");
		await timepicker.shadow$("kengine-input").$(".kengine-time-picker-input-icon-button").click();

		const hoursClockValue = await timepickerPopover.$("kengine-time-selection-clocks").shadow$(`kengine-time-picker-clock[data-sap-clock="hours"]`).getProperty("selectedValue");
		const minutesClockValue = await timepickerPopover.$("kengine-time-selection-clocks").shadow$(`kengine-time-picker-clock[data-sap-clock="minutes"]`).getProperty("selectedValue");
		const secondsClockValue = await timepickerPopover.$("kengine-time-selection-clocks").shadow$(`kengine-time-picker-clock[data-sap-clock="seconds"]`).getProperty("selectedValue");
	...
	it("tests clocks submit value", async () => {
		...
		const picker = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover");
		...
		const textValue = await timepicker.shadow$("kengine-input").getValue();
	...
	it("tests submit wrong value", async () => {
		...
		assert.strictEqual(await timepicker.shadow$("kengine-input").getProperty("valueState"), "Error", "The value state is on error");
	});
	...
	it("tests valueStateMessage slot", async () => {
		...
		const inputId = await timepicker.shadow$("kengine-input").getProperty("_id");
		const inputStaticAreaItem = await browser.$(`.${inputId}`);
		const slot = await inputStaticAreaItem.shadow$("kengine-popover").$("#customValueStateMessage");
	...
	it("tests change event", async () => {
		...
		const input = await timepicker.shadow$("kengine-input");
		const icon = await input.$("kengine-icon");
	...
	it("tests value state", async () => {
		...
		// act
		await timepicker.click();
		while(await timepicker.shadow$("kengine-input").getProperty("value") !== ""){
			await browser.keys("Backspace");
		}
		await button.click();

		// assert
		assert.strictEqual(await timepicker.shadow$("kengine-input").getProperty("valueState"), "None", "The value state is None");
	});
	...
	it("tests input keyboard handling", async () => {
		...
		// assert
		assert.strictEqual(await timepicker.shadow$("kengine-input").getProperty("value"), "02:41:05", "The value of minutes is +1");
		// act
		await timepicker.click();
		...
		// assert
		assert.strictEqual(await timepicker.shadow$("kengine-input").getProperty("value"), "02:40:05", "The value of minutes is -1");
		...
		// assert
		assert.strictEqual(await timepicker.shadow$("kengine-input").getProperty("value"), "03:40:05", "The value of hours is +1");
		// act
		await timepicker.click();
		await browser.keys('PageDown');

		// assert
		assert.strictEqual(await timepicker.shadow$("kengine-input").getProperty("value"), "02:40:05", "The value of hours is -1");
		...
		// assert
		assert.strictEqual(await timepicker.shadow$("kengine-input").getProperty("value"), "02:40:06", "The value of seconds is +1");
		// act
		await timepicker.click();
		...
		// assert
		assert.strictEqual(await timepicker.shadow$("kengine-input").getProperty("value"), "02:40:05", "The value of seconds is -1");
	});
	...
	it("the value 'now' returns the current time, instead of the string 'now'", async () => {
		...
		assert.notStrictEqual(await timepicker.shadow$("kengine-input").getProperty("value"), "now", "the value is not 'now'");
	});
});
