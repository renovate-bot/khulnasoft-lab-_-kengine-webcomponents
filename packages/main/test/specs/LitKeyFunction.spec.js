...
describe("Lit HTML key function for #each", async () => {
	...
	it("LIT HTML does not mess up keys when looping over lists", async () => {
		...
		const firstItem = await popover.$$(".kengine-multi-combobox-all-items-list > kengine-li")[0];
		...
		const newFirstItem = (await popover.$$(".kengine-multi-combobox-all-items-list > kengine-li"))[0];
		...
		const lastItem = (await popover.$$(".kengine-multi-combobox-all-items-list > kengine-li"))[3];
