...
describe("Date Picker Tests", () => {
	...
	it("value state", async () => {
		...
		const contentWrapper = await browser.$("#dp3").shadow$("kengine-input").shadow$(".kengine-input-content");
	...
	it("Value State Message", async () => {
		...
		const popover = await inputStaticAreaItem.shadow$("kengine-popover");
	...
	it("focusout fires change", async () => {
		...
		await browser.$("#dp1").shadow$("kengine-input").shadow$("input").click(); //click elsewhere to focusout
	...
	it("delete input value then open picker keeps the empty value", async () => {
		...
		assert.ok(await calendarDate_6_Jan_2015.hasClass('kengine-dp-item--selected'), "calendar selected date is ok");
		...
		assert.notOk(await calendarDate_6_Jan_2015.hasClass("kengine-dp-item--selected"), "calendar selected dates is ok");
		assert.notOk(await calendarDate_8_Jan_2015.hasClass("kengine-dp-item--selected"), "calendar selected dates is ok");
	...
	it("respect first day of the week - monday", async () => {
		...
		assert.ok(await calendarDate_3_Feb_2019.hasClass("kengine-dp-wday6"), "3 Feb 2019 is displayed as last day of the week");
	});
	...
	it("today value is normalized and correctly rounded to 00:00:00", async () => {
		...
		assert.ok(await calendarDateToday.hasClass('kengine-dp-item--selected'), "calendar selected date is ok");
	});
	...
	it("[F4] shows year picker after date picker is open", async () => {
		...
		const calendar = await datepicker.getCalendar();
		assert.notOk((await calendar.shadow$("kengine-monthpicker"))._hidden, "Month picker is open");
	...
	it("[SHIFT] + [F4] shows year picker after date picker is open", async () => {
		...
		const calendar = await datepicker.getCalendar();
		assert.notOk((await calendar.shadow$("kengine-yearpicker"))._hidden, "Year picker is open");
		await valueHelpIcon.click(); // close the datepicker
	});
	...
	it("[F4] shows month picker after year picker is open", async () => {
		...
		const calendar = await datepicker.getCalendar();
		assert.notOk((await calendar.shadow$("kengine-monthpicker"))._hidden, "Year picker is open");
		await valueHelpIcon.click(); // close the datepicker
	});
	...
	it("DatePicker popover when initially opened displays a day picker", async () => {
		...
		const calendar = await datepicker.getCalendar();
		assert.notOk((await calendar.shadow$("kengine-daypicker"))._hidden, "Day picker is open");
	...
	it("Going under the minimum date changes value state", async () => {
		...
		const contentWrapper = await browser.$("#dp33").shadow$("kengine-input").shadow$(".kengine-input-content");
		assert.ok(await contentWrapper.isDisplayedInViewport(), "content wrapper has error styles");
	});
	...
	it("Years are disabled when out of range", async () => {
		...
		assert.ok(await displayedYear.hasClass("kengine-yp-item--disabled"), "Years out of range are disabled");
	...
	it("Months are disabled when out of range", async () => {
		...
		assert.ok(await displayedMonth.hasClass("kengine-mp-item--disabled"), "Months out of range are disabled");
	...
	it("Days are disabled when out of range", async () => {
		...
		assert.ok(await displayedDay.hasClass("kengine-dp-item--disabled"), "Days out of range are disabled");
	});
	...
	it("Min and Max date are included in the interval", async () => {
		...
		assert.notOk(await displayedDay.hasClass("kengine-dp-item--disabled"), "Min date is included");

		displayedDay = await datepicker.getDisplayedDay(11);
		assert.notOk(await displayedDay.hasClass("kengine-dp-item--disabled"), "Max date is included");
	});
	...
	it("Tests week numbers column visibility", async () => {
		...
		const weekNumbersCol1 = await dayPicker.shadow$(".kengine-dp-weekname-container");
		...
		const weekNumbersCol2 = await dayPicker.shadow$(".kengine-dp-weekname-container");
	...
	it("Calendar root have correct attribute", async () => {
		...
		const monthpickerContent = (await datepicker.getDayPicker()).shadow$(".kengine-dp-content");
	...
	it("focusout fires change but doesn't change the value state if the default behaviour is prevented", async () => {
		...
		await browser.$("#dp5").shadow$("kengine-input").shadow$("input").click(); //click elsewhere to focusout
	...
	it("Min and max dates are set, with no format pattern provided, using valid ISO format", async () => {
		...
		assert.ok(await displayedYear.hasClass("kengine-yp-item--disabled"), "Year 2021 is disabled");
	...
	it("Min and max dates are NOT set because no format pattern is provided & format used is not ISO", async () => {
		...
		let displayedYear = await datepicker.getDisplayedYear(11);
		assert.notOk(await displayedYear.hasClass("kengine-yp-item--disabled"), "Year 2025 is not disabled");
	});
});
