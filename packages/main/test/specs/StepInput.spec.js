...
describe("Attributes propagation", () => {
	...
	it("'placeholder' attribute is propagated properly", async () => {
		...
		assert.strictEqual(await browser.$("#stepInputCozy").shadow$('.kengine-step-input-input').shadow$("input").getProperty("placeholder"), sExpected, "The 'placeholder' was set correctly");
	});
	...
	it("'min' attribute is propagated properly", async () => {
		...
		assert.strictEqual(await browser.$("#stepInputCozy").shadow$('.kengine-step-input-input').shadow$("input").getProperty("min"), sExpected, "The 'min' was set correctly");
	});
	...
	it("'max' attribute is propagated properly", async () => {
		...
		assert.strictEqual(await browser.$("#stepInputCozy").shadow$('.kengine-step-input-input').shadow$("input").getProperty("max"), sExpected, "The 'max' was set correctly");
	});
	...
	it("'step' attribute is propagated properly", async () => {
		...
		assert.strictEqual(await browser.$("#stepInputCozy").shadow$('.kengine-step-input-input').shadow$("input").getProperty("step"), sExpected, "The 'step' was set correctly");
	});

	it("'disabled' attribute is propagated properly", async () => {
		await browser.url(`test/pages/StepInput.html`);
		assert.ok(await browser.$("#stepInputDisabled").shadow$('.kengine-step-input-input').shadow$("input").getAttribute("disabled"), "The 'disabled' property was propagated");
	});

	it("'redonly' attribute is propagated properly", async () => {
		await browser.url(`test/pages/StepInput.html`);
		assert.ok(await browser.$("#stepInputReadOnly").shadow$('.kengine-step-input-input').shadow$("input").getAttribute("readonly"), "The 'readonly' property was propagated");
	});
	...
	it("'value' attribute is propagated properly", async () => {
		...
		assert.strictEqual(await browser.$("#stepInputCozy").shadow$('.kengine-step-input-input').getValue(), sExpectedValue, "Value property was set correctly");
	});
...
describe("Inc/Dec buttons interactions", () => {
	...
	it("'Increase' button increases the value if it is less than 'max'", async () => {
		await browser.url(`test/pages/StepInput.html`);
		const siMinMax = await browser.$("#stepInputMinMax");
		const incButton = await siMinMax.shadow$(".kengine-step-inc");
		const initValue = await siMinMax.getProperty("value");
	...
	it("'Decrease' button decreases the value if it is greater than 'min'", async () => {
		await browser.url(`test/pages/StepInput.html`);
		const siMinMax = await browser.$("#stepInputMinMax");
		const decButton = await siMinMax.shadow$(".kengine-step-dec");
		const maxValue = await siMinMax.getProperty("max");
		const minValue = await siMinMax.getProperty("min");
...
describe("'change' event firing", () => {
	...
	it("'change' event should be fired after changing value programatically and then manual entry of the previous value and focus out", async () => {
		...
		const incButton = await siChange1.shadow$(".kengine-step-inc");
...
describe("Accessibility related parameters", async () => {
	...
	it("'step', 'min', 'max', 'aria-required' and 'aria-label' attributes presence", async () => {
		await browser.url(`test/pages/StepInput.html`);
		const siCozy = await browser.$("#stepInputCozy");
		const siInner = await siCozy.shadow$('.kengine-step-input-input').shadow$("input");
