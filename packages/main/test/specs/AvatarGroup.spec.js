...
describe("avatar-group rendering", () => {
	...
	it("tests if _background-design attribute is automatically set to avatars", async () => {
		const avatars = await browser.$$("#avatar-group-group kengine-avatar");
	...
	it("tests click event when avatar is clicked", async () => {
		...
		assert.strictEqual(await eventTargetRef.getValue(), await avatar.getProperty("tagName"), "Event should be called with kengine-avatar as targetRef");
	...
	it("tests click event when overflow button is clicked", async () => {
		const overflowButton = await browser.$("#avatar-group-individual").shadow$("kengine-button");
		...
		assert.strictEqual(await eventTargetRef.getValue(), await overflowButton.getProperty("tagName"), "Event should be called with kengine-button as targetRef");
	...
	it("tests click event avatar group with type group is clicked", async () => {
		...
		assert.strictEqual(await eventTargetRef.getValue(), "DIV.kengine-avatar-group-items", "Event should be called with kengine-button as targetRef");
		assert.strictEqual(await eventOverflowButtonClicked.getValue(), "false", "Event should be called with overflowButtonClicked equal to false");
	});
	...
	it("tests if click event is firing only once", async () => {
		...
		const overflowButton = await browser.$("#avatar-group-individual-events").shadow$("kengine-button");
