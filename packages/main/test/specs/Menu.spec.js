...
describe("Menu interaction", () => {
	...
	it("Top level menu items appearance", async () => {
		await browser.url(`test/pages/Menu.html`);
		const openButton = await browser.$("#btnOpen");
		const menuItems = await browser.$$("kengine-menu>kengine-menu-item");
		...
		assert.ok(await listItems[3].$(".kengine-menu-item-icon-end"), "The third list item has sub-items and must have arrow right icon after the text");
		assert.ok(await listItems[4].$(".kengine-menu-item-dummy-icon"), "The fourth list item has no icon and has dummy div instead of icon");
	});
	...
	it("Sub-menu creation, opening, closing and destroying", async () => {
		...
		assert.ok(await browser.$(`.${staticAreaItemClassName}`).shadow$(".kengine-menu-submenus").$("kengine-menu"),
								"The second level sub-menu is being created"); // new kengine-menu element is created for the sub-menu

		await browser.keys("ArrowLeft"); // back to main menu
		await browser.keys("ArrowDown"); // go to the next menu item (close sub-menu)

		assert.strictEqual(await browser.$(`.${staticAreaItemClassName}`).shadow$(".kengine-menu-submenus").$$("kengine-menu").length, 0,
								"The second level sub-menu is being destroyed"); // sub-menu kengine-menu element is destroyed
	});
	...
	it("Menu and Menu items busy indication", async () => {
			...
			const menuPopover = await browser.$("kengine-static-area-item:last-of-type").shadow$("kengine-responsive-popover");
			const visualOpenItem = await menuPopover.$("kengine-li[accessible-name='Open']");
			const visualCloseItem = await menuPopover.$("kengine-li[accessible-name='Close']");

			visualOpenItem.click();
			await browser.pause(350);
			const openSubmenuPopover = await browser.$("kengine-static-area-item:last-of-type").shadow$("kengine-responsive-popover");
			const openMenuList = await openSubmenuPopover.$("kengine-list");

			// assert.ok(await openMenuList.getProperty("busy"), "Busy property is properly propagated to the kengine-list component.");
			await browser.pause(650);
			assert.strictEqual(await openMenuList.$$("kengine-li").length, 4, "Two additional nodes have been added.");

			visualCloseItem.click();
			await browser.pause(350);

			const closeSubmenuPopover = await browser.$("kengine-static-area-item:last-of-type").shadow$("kengine-responsive-popover");
			const busyIndicator = await closeSubmenuPopover.$("kengine-busy-indicator");
	 ...
	 */
		...
		it("Prevent menu closing on item press", async () => {
			...
			const menuPopover = await browser.$("kengine-static-area-item:last-of-type").shadow$("kengine-responsive-popover");
			const newFileItem = await menuPopover.$("kengine-li[accessible-name='New File']");
			newFileItem.click();
			await browser.pause(100);
...
describe("Menu Accessibility", () => {
	it("Menu and Menu items accessibility attributes", async () => {
		...
		const staticAreaItemClassName = await browser.getStaticAreaItemClassName("#menu");
		const popover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover");
		const list = await popover.$("kengine-list");
		const listItems = await popover.$("kengine-list").$$("kengine-li");
