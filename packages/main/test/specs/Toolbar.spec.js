...
describe("Toolbar general interaction", () => {
	...
	it("Should move button with alwaysOverflow priority to overflow popover", async () => {
		...
		const overflowButton = await otb.shadow$(".kengine-tb-overflow-btn");
		...
		const employee_button = await popover.$(`[data-kengine-stable="tb-button-employee-d"]`);
		assert.ok(await employee_button.hasClass("kengine-tb-popover-item"), "'Employee' button is moved in overflow popover since it has 'Always' priority");
	});
	...
	it("Should render kengine-button by toolbar template, when slotting kengine-toolbar-button elements", async () => {
		const otb = await browser.$("#otb_c");
		const declineButton = await otb.shadow$(`[data-kengine-stable="tb-button-decline"]`);

		assert.strictEqual(await declineButton.getTagName(), "kengine-button", false, "Toolbar button is properly rendered");
	});
