...
describe("ColorPalette interactions", () => {
	...
	it("Test if selecting element works", async () => {
		...
		const colorPaletteEntries = await colorPalette.$$("[kengine-color-palette-item]");
	...
	it("Tests show-more-colors functionality", async () => {
		...
		const colorPaletteMoreColorsButton = await colorPalette.shadow$(".kengine-cp-more-colors");

		await colorPaletteMoreColorsButton.click();

		const staticAreaItemClassName = await browser.getStaticAreaItemClassName("#cp3");
		const colorPicker = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-color-picker");
	...
	it("Tests show-recent-colors functionality", async () => {
		...
		const colorPaletteRecentColorsWrapper = await colorPalette.shadow$(".kengine-cp-recent-colors-wrapper");
		const colorPaletteRecentColorsWrapperEntries = await colorPaletteRecentColorsWrapper.$$("[kengine-color-palette-item]");
