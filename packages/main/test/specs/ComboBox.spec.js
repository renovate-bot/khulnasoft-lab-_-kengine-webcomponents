...
describe("General interaction", () => {
	...
	it ("Items filtration", async () => {
		...
		const combo = await browser.$("#combo");
		const arrow = await combo.shadow$("[input-icon]");
		const input = await combo.shadow$("#kengine-combobox-input");
		const staticAreaItemClassName = await browser.getStaticAreaItemClassName("#combo");
		const popover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover");
	...
	it ("Should open the popover when typing a value", async () => {
		...
		const listItems = await popover.$("kengine-list").$$("kengine-li");
	...
	it ("Should filter items based on input with filter='None' and lazy loading", async () => {
		...
		setTimeout(async () => {
			listItems = await popover.$("kengine-list").$$("kengine-li");
			const firstListItemText = await listItems[0].shadow$(".kengine-li-title").getText();
	...
	it ("Tests change event", async () => {
		...
		const popover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover");
		await (await popover.$("kengine-list").$$("kengine-li")[0]).click();
		...
		await (await popover.$("kengine-list").$$("kengine-li"))[1].click();
	...
	it ("Tests change event with value state and links", async () => {
		...
		await browser.executeAsync(done => {
			document.querySelector("[value-state='Error']").addEventListener("kengine-change", function(event) {
		...
		const popover = await browser.$(`.${staticAreaItemClassName}`).shadow$("kengine-responsive-popover");
		const link = await popover.$(".kengine-responsive-popover-header.kengine-valuestatemessage-root a");
	...
	it ("Tests selection-change event and its parameters", async () => {
		...
		const listItemText = await listItem.shadow$(".kengine-li-title").getText();
	...
	it ("Tests selection-change event when type text after selection", async () => {
		...
		assert.strictEqual(await label.getText(), await fisrtListItem.shadow$(".kengine-li-title").getText(), "event is fired correctly");
	});
...
describe("Grouping", () => {
	...
	it ("Tests input value while group item is focused", async () => {
		...
		groupItem = await popover.$("kengine-list").$$("kengine-li-groupheader")[1];
...
describe("Accessibility", async () => {
	...
	it ("Announce item on selection", async () => {
		...
		const arrow = await combo.shadow$("[input-icon]");
		const input = await combo.shadow$("#kengine-combobox-input");
		const invisibleMessageSpan = await browser.$(".kengine-invisiblemessage-polite");
	...
	it("Value state type should be added to the screen readers default value states announcement", async () => {
		...
		ariaHiddenText = await cbInformation.shadow$(".kengine-hidden-text").getHTML(false);
		...
		ariaHiddenText = await cbSuccess.shadow$(".kengine-hidden-text").getHTML(false);
		assert.strictEqual(ariaHiddenText.includes("Value State"), true, "Hidden screen reader text is correct");
	});
...
describe("Keyboard navigation", async () => {
	it ("Should focus the first item on arrow down and then the input on arrow up",  async () => {
		...
		listItem = await popover.$("kengine-list").$$("kengine-li")[0];
	...
	it ("Should focus the value state header and then the input", async () => {
		...
		valueStateHeader = await popover.$(".kengine-responsive-popover-header.kengine-valuestatemessage-root");
	...
	it ("Previous focus should not remain on the item after reopening the picker and choosing another one", async () => {
		...
		listItem = await popover.$("kengine-list").$$("kengine-li")[1];
		...
		listItem = await popover.$("kengine-list").$$("kengine-li")[3];

		await listItem.click();

		await arrow.click();
		prevListItem = await popover.$("kengine-list").$$("kengine-li")[5];
	...
	it ("Should select the corresponding item on home/pgup/pgdown/end",  async () => {
		...
		const input = await comboBox.shadow$("#kengine-combobox-input");
		...
		listItem = await respPopover.$("kengine-list").$("kengine-li");
		...
		listItem = await respPopover.$("kengine-list").$$("kengine-li")[10];
