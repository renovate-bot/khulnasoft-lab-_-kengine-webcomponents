...
const getScripts = (options) => {
	...
	const tsCrossEnv = tsOption ? "cross-env KENGINE_TS=true" : "";
	...
	if (fs.existsSync("config/vite.config.js")) {
		...
		viteConfig = `-c "${require.resolve("@kengine/webcomponents-tools/components-package/vite.config.js")}"`;
	}
	...
	if (fs.existsSync(".eslintrc.js") || fs.existsSync(".eslintrc.cjs")) {
		...
		// no custom configuration - use default from tools project
		eslintConfig = `--config  "${require.resolve("@kengine/webcomponents-tools/components-package/eslint.js")}"`;
	}
	...
	const scripts = {
		...
		build: {
			...
			templates: `mkdirp dist/generated/templates && ${tsCrossEnv} node "${LIB}/hbs2kengine/index.js" -d src/ -o src/generated/templates`,
			styles: {
