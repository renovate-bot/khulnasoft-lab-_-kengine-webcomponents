/*
 * JSDoc3 plugin for KENGINE documentation generation.
 *
...
/**
 * KENGINE plugin for JSDoc3 (3.3.0-alpha5)
 *
 * The plugin adds the following KHULNASOFTKENGINE specific tag definitions to JSDoc3
 *
 *   disclaimer
 *
 *   experimental
 ...
 *
 *
 * Last but not least, it implements an astNodeVisitor to detect KENGINE specific "extend" calls and to create
 * documentation for the properties, aggregations etc. that are created with the "extend" call.
 *
 * @module plugins/ui-jsdoc
 */
...
var pluginConfig = (env.conf && env.conf.templates && env.conf.templates.kengine) || {};
...
/**
 * Prefixes of the KENGINE unified resource name for the source files is NOT part of the file name.
...
/**
 * A KENGINE specific unique Id for all doclets.
 */
var docletUid = 0;
 ...
 * via ui.resourceNamePrefixes, for KENGINE libraries it is empty by default).
 *
...
/**
 * Cached KENGINE metadata for encountered KENGINE classes.
 *
...
function kenginedata(doclet) {
	return doclet.__kengine || (doclet.__kengine = { id: ++docletUid });
}
	...
	if ( p >= 0 && !hasSettingsDocs ) {
		...
		if ( isManagedObject ) { // only a ManagedObject has settings
			...
			if ( !isEmpty(oClassInfo.properties)
				...
				lines.push(
					"",
					includeSettings ? "" : "@kengine-settings",
				...
				lines.push(
					"",
					newStyle && !includeSettings ? "@kengine-settings" : "",
		...
		var enhancedComment =
			...
			(commentAlreadyProcessed ? "@kengine-updated-doclet\n * " : "") +
...
function location(doclet) {
	...
	return " #" + kenginedata(doclet).id + "@" + filename + (doclet.meta.lineno != null ? ":" + doclet.meta.lineno : "") + (doclet.synthetic ? "(synthetic)" : "");
}
...
/**
 * Preprocesses a JSDoc comment string to ensure some KENGINE standards.
 *
...
function preprocessComment(e) {
	...
	if ( !/@private|@public|@protected|@sap-restricted|@kengine-restricted/.test(src) ) {
		src = unwrap(src);
		src = src + "\n@private";
		src = wrap(src);
...
exports.defineTags = function(dictionary) {
	...
	/**
	 * A ui specific tag to add a disclaimer to a symbol
	 */
	...
	/**
	 * A ui specific tag to mark a symbol as experimental.
	 */
	 ...
	 * Support for 'interface' might not be complete (only standard KENGINE use cases tested)
	 */
	 ...
	 */
	dictionary.defineTag('kengine-restricted', {
		onTagged: function(doclet, tag) {
			doclet.access = 'restricted';
			if ( tag.value ) {
				kenginedata(doclet).stakeholders = tag.value.trim().split(/(?:\s*,\s*|\s+)/);
			}
		}
	});
	dictionary.defineSynonym('kengine-restricted', 'sap-restricted');
	 ...
	 */
	dictionary.defineTag('kengine-updated-doclet', {
		mustNotHaveValue: true,
		onTagged: function(doclet, tag) {
			kenginedata(doclet).updatedDoclet = true;
		}
	});
...
exports.handlers = {
	...
	newDoclet: function(e) {
		...
		var _kenginedata = kenginedata(e.doclet);
		...
		if ( e.doclet.meta ) {
			...
			_kenginedata.resource = currentModule.resource;
			_kenginedata.module = currentModule.name || currentModule.module;
		}
	...
	parseComplete : function(e) {
		...
		doclets.sort(function(a,b) {
			if ( a.longname === b.longname ) {
				if ( a.synthetic === b.synthetic ) {
					...
					return a.__kengine.id - b.__kengine.id;
				}
		...
		for (i = 0, j = 0; i < l; i++) {
			...
			if ( classInfos[doclet.longname] ) {
				doclet.__kengine.metadata = classInfos[doclet.longname];

				// add designtime infos, if configured
				var designtimeModule = doclet.__kengine.metadata.designtime;
				if ( designtimeModule && typeof designtimeModule !== 'string' ) {
					designtimeModule = doclet.__kengine.module + ".designtime";
				}
				if ( designtimeModule && designtimeInfos[designtimeModule] ) {
					...
					doclet.__kengine.metadata.annotations = designtimeInfos[designtimeModule].annotations;
				}

				// derive extends from KENGINE APIs
				if ( doclet.__kengine.metadata.baseType
					 && !(doclet.augments && doclet.augments.length > 0) ) {
					doclet.augments = doclet.augments || [];
					info("  @extends " + doclet.__kengine.metadata.baseType + " derived from KENGINE APIs (" + doclet.longname + ")");
					doclet.augments.push(doclet.__kengine.metadata.baseType);
				}

				// derive interface implementations from KENGINE metadata
				if ( doclet.__kengine.metadata.interfaces && doclet.__kengine.metadata.interfaces.length ) {
					doclet.__kengine.metadata.interfaces.forEach(function(intf) {
						doclet.implements = doclet.implements || [];
						if ( doclet.implements.indexOf(intf) < 0 ) {
							info("  @implements " + intf + " derived from KENGINE metadata (" + doclet.longname + ")");
			...
			if ( typeInfos[doclet.longname] ) {
				doclet.__kengine.stereotype = 'datatype';
				doclet.__kengine.metadata = {
			...
			if ( j > 0 && doclets[j - 1].longname === doclet.longname ) {
				if ( !doclets[j - 1].synthetic && !doclet.__kengine.updatedDoclet ) {
