/*
 * JSDoc3 template for KENGINE documentation generation.
 *
 * (c) Copyright 2009-2018 KHULNASOFT SE or an KHULNASOFT affiliate company.
...
function publish(symbolSet) {
	...
	info("entering ui template");

	// create output dir
	fs.mkPath(env.opts.destination);

//	if ( symbolSet().count() < 20000 ) {
//		info("writing raw symbols to " + path.join(env.opts.destination, "symbols-unpruned-kengine.json"));
//		fs.writeFileSync(path.join(env.opts.destination, "symbols-unpruned-kengine.json"), JSON.stringify(symbolSet().get(), filter, "\t"), 'utf8');
//	}
	...
	if ( symbolSet().count() < 20000 ) {
		info("writing raw symbols to " + path.join(env.opts.destination, "symbols-pruned-kengine.json"));
		// skip this for now
		// fs.writeFileSync(path.join(env.opts.destination, "symbols-pruned-kengine.json"), JSON.stringify(symbolSet().get(), filter, "\t"), 'utf8');
	}
	...
	for (var i = 0; i < templateConf.variants.length; i++) {
		...
		if ( PUBLISHING_VARIANTS[vVariant.variant] ) {
			...
			//  - user configuration for ui template
...
function createNamespaceTree() {
	...
	for (var i = 0; i < aTypes.length; i++) { // loop with a for-loop as it can handle concurrent modifications
		...
		if ( symbol.memberof ) {
			...
			symbol.__kengine.parent = parent;
			parent.__kengine.children = parent.__kengine.children || [];
			parent.__kengine.children.push(symbol);
...
function createInheritanceTree() {
	...
	function getOrCreateClass(sClass, sExtendingClass) {
		...
		if ( !oClass ) {
			...
			oClass.__kengine.base = oBaseClass;
			oBaseClass.__kengine.derived = oBaseClass.__kengine.derived || [];
			oBaseClass.__kengine.derived.push(oClass);
		}
		return oClass;
	}
		...
		if ( oClass.implements ) {
			for (var j = 0; j < oClass.implements.length; j++) {
				...
				if ( !oInterface ) {
					...
					oInterface.__kengine.base = oObject;
					oObject.__kengine.derived = oObject.__kengine.derived || [];
					oObject.__kengine.derived.push(oInterface);
				}
				oInterface.__kengine.implementations = oInterface.__kengine.implementations || [];
				oInterface.__kengine.implementations.push(oClass);
			}
		}
	});
	...
	function setStereotype(oSymbol, sStereotype) {
		...
		oSymbol.__kengine.stereotype = sStereotype;
		var derived = oSymbol.__kengine.derived;
		if ( derived ) {
			for (var i = 0; i < derived.length; i++ ) {
				if ( !derived[i].__kengine.stereotype ) {
	...
	oTypes.each(function(oStartClass) {
		...
		function visit(oClass) {
			...
			if ( oClass.__kengine.base ) {
				visited[oClass.longname] = true;
				visit(oClass.__kengine.base);
	...
	return __db(function() {
		return R_KINDS.test(this.kind) && this.__kengine && this.__kengine.base == null;
	}).get();
	*/
}
	...
	__db().each(function($) {
		if ( $.memberof ) {
			...
			if ( parent /* && supportsInheritance(parent) */ ) {
				parent.__kengine.members = parent.__kengine.members || [];
				parent.__kengine.members.push($);
			}
		}
	});
}
...
function mergeEventDocumentation() {
	...
	console.log("merging JSDoc event documentation into KENGINE metadata");

	var oTypes = __db(function() { return isaClass(this); });

	oTypes.each(function(symbol) {

		var metadata = symbol.__kengine.metadata;
		var members = symbol.__kengine.members;
		...
		members.forEach(function($) {
			if ( $.kind === 'event' && !$.inherited
				...
				$.params.forEach(function(param) {
					...
					if ( m ) {
						var pname = m[1];
						var kengineparam = event.parameters[pname] || ( event.parameters[pname] = {});
						if ( kengineparam.type == null ) {
							kengineparam.type = listTypes(param.type);
							modified = true;
						}
						if ( kengineparam.doc == null ) {
							kengineparam.doc = param.description;
							modified = true;
						}
					}
				});
...
function groupByModule(symbols) {
	...
	symbols.forEach(function(symbol) {
		...
		var key = symbol.__kengine.module;

		if ( key ) {
			add(key, symbol);
			if ( symbol.__kengine.members ) {
				symbol.__kengine.members.forEach(function($) {
					if ( !$.inherited && $.__kengine.module && $.__kengine.module !== key && conf.filter($) ) {
						add($.__kengine.module, $);
					}
				});
			}
		}
...
function childrenOfKind(data, kind) {
	...
	if ( data.__kengine.members ) {
		data.__kengine.members.forEach(function($) {
...
function createAPIJSON(symbols, filename) {
	...
	var api = {
		"$schema-ref": "http://schemas.khulnasoft.com/ui/designtime/api.json/1.0"
	}
...
function createAPIJSON4Symbol(symbol, omitDefaults) {
	...
	function hasSettings($, visited) {
		...
		var metadata = $.__kengine.metadata;
		return metadata &&
			(
	...
	function writeMethod(member, name) {
		...
		if ( member.__kengine.module && member.__kengine.module !== symbol.__kengine.module ) {
			attrib("module", member.__kengine.module);
		...
		if ( member.tags && member.tags.some(function(tag) { return tag.title === 'kengine-metamodel'; }) ) {
			attrib('kengine-metamodel', true, false, /* raw = */true);
		}
		...
		//secTags(member);
		if ( member.__kengine.resource && member.__kengine.resource !== symbol.__kengine.resource ) {
			attrib("resource", member.__kengine.resource);
		}
		closeTag("method");
	...
	*/
	...
	if ( symbol.__kengine.resource ) {
		attrib("resource", symbol.__kengine.resource);
	}
	if ( symbol.__kengine.module ) {
		attrib("module", symbol.__kengine.module);
		attrib("export", undefined, '', true);
	}
	if ( symbol.virtual ) {
	...
	if ( symbol.tags && symbol.tags.some(function(tag) { return tag.title === 'kengine-metamodel'; }) ) {
		attrib('kengine-metamodel', true, false, /* raw = */true);
	}
	...
	if ( kind === 'class' ) {
		...
		if ( symbol.__kengine.stereotype || hasSettings(symbol) ) {

			tag("kengine-metadata");

			if ( symbol.__kengine.stereotype ) {
				attrib("stereotype", symbol.__kengine.stereotype);
			}

			writeMetadata(symbol);

			closeTag("kengine-metadata");
		}
		...
		if ( symbol.__kengine.stereotype || symbol.__kengine.metadata ) {
			tag("kengine-metadata");
			...
			if ( symbol.__kengine.metadata && symbol.__kengine.metadata.basetype ) {
				attrib("basetype", symbol.__kengine.metadata.basetype);
			}

			if ( symbol.__kengine.metadata && symbol.__kengine.metadata.pattern ) {
				attrib("pattern", symbol.__kengine.metadata.pattern);
			}

			if ( symbol.__kengine.metadata && symbol.__kengine.metadata.range ) {
				attrib("range", symbol.__kengine.metadata.range, null, /* raw = */ true);
			}
		...
		if ( ownMethods.length > 0 ) {
			...
			for ( i = 0; i < ownMethods.length; i++ ) {
				...
				if ( member.__kengine.members ) {
					...
					member.__kengine.members.forEach(function($) {
	...
	//	if ( roots && symbol.__kengine.children && symbol.__kengine.children.length ) {
	//		collection("children", "kind");
	//		symbol.__kengine.children.forEach(writeSymbol);
...
function createAPIXML(symbols, filename, options) {
	...
	function writeMetadata(symbolAPI, inherited) {
		...
		var kengineMetadata = symbolAPI["kengine-metadata"];
		if ( !kengineMetadata ) {
			return;
		}
		...
		if ( kengineMetadata.specialSettings ) {
			kengineMetadata.specialSettings.forEach(function(special) {
				tag("specialSetting");
				attrib("name", special.name);
				attrib("type", special.type);
				attrib("visibility", special.visibility, 'public');
		...
		if ( kengineMetadata.properties ) {
			kengineMetadata.properties.forEach(function(prop) {
				tag("property");
				attrib("name", prop.name);
				attrib("type", prop.type, 'string');
		...
		if ( kengineMetadata.defaultProperty ) {
			tag("defaultProperty", kengineMetadata.defaultProperty);
		}

		if ( kengineMetadata.aggregations ) {
			kengineMetadata.aggregations.forEach(function(aggr) {
				tag("aggregation");
				attrib("name", aggr.name);
				attrib("singularName", aggr.singularName); // TODO omit default?
				attrib("type", aggr.type, 'sap.ui.core.Control');
		...
		if ( kengineMetadata.defaultAggregation ) {
			tag("defaultAggregation", kengineMetadata.defaultAggregation);
		}

		if ( kengineMetadata.associations ) {
			kengineMetadata.associations.forEach(function(assoc) {
				tag("association");
				attrib("name", assoc.name);
				attrib("singularName", assoc.singularName); // TODO omit default?
				attrib("type", assoc.type, 'sap.ui.core.Control');
		...
		if ( kengineMetadata.events ) {
			kengineMetadata.events.forEach(function(event) {
				tag("event");
				attrib("name", event.name);
				attrib("visibility", event.visibility, 'public');
				if ( event.since ) {
		...
		if ( kengineMetadata.annotations ) {
			kengineMetadata.annotations.forEach(function(anno) {
				tag("annotation");
				attrib("name", anno.name);
...
//			}
			...
			if ( kind === 'class' ) {
				...
				var hasSettings = symbolAPI["kengine-metadata"];

				if ( !legacyContent && symbolAPI["kengine-metadata"] ) {

					tag("kengine-metadata");

					if ( symbolAPI["kengine-metadata"].stereotype ) {
						attrib("stereotype", symbolAPI["kengine-metadata"].stereotype);
					}
			...
			var ownSubspaces = ( symbol.__kengine.children || [] ).filter(function($) { return $.kind === 'namespace' }).sort(sortByAlias);
	...
	rootTag("api");
	if ( !legacyContent ) {
		namespace("xmlns", "http://www.khulnasoft.com/sap.ui.library.api.xsd");
...
function getConstructorDescription(symbol) {
	...
	if ( tags ) {
		for (var i = 0; i < tags.length; i++) {
			if ( tags[i].title === "kengine-settings" && tags[i].text) {
				description += "\n</p><p>\n" + tags[i].text;
				break;
			}
		}
