...
function getSVGBlock(input, blockCounter) {
	const definitionTS = `\nfunction blockSVG${blockCounter} (this: any, context: KENGINEElement, tags: string[], suffix: string | undefined) {
		return svg\`${input}\`;
	};`;
	...
	return {
		...
		definition: process.env.KENGINE_TS ? definitionTS : definitionJS,
	};
}
