...
function HTMLLitVisitor(componentName, debug) {
	...
	const blockParametersDefinitionTS = [`this: ${componentName}`, "context: KENGINEElement", "tags: string[]", "suffix: string | undefined"];
	const blockParametersDefinitionJS = ["context", "tags", "suffix"];
	this.blockParametersDefinition = process.env.KENGINE_TS ? blockParametersDefinitionTS : blockParametersDefinitionJS;
...
function visitEachBlock(block) {
	...
	const repleatDirectiveParams = process.env.KENGINE_TS ? reapeatDirectiveParamsTS : reapeatDirectiveParamsJS;
